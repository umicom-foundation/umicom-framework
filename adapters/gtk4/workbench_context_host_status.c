...
UmiStatus umi_workbench_context_host_gtk4_status_refresh(GtkWidget *status,UmiWorkbenchContextHost *host)
{
    ...
    s=umi_workbench_context_host_status_projection_build(host,&p);/* Preserve the original failure result so the caller can respond to the correct cause. */ if(s!=UMI_STATUS_OK)return s;
