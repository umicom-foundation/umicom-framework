...
protected: void paintEvent(QPaintEvent *event) override { (void)event; QPainter p(this); p.fillRect(rect(),QColor(16,23,32)); if(graph_==nullptr)return; p.setRenderHint(QPainter::Antialiasing,true); p.setPen(QPen(QColor(100,110,140),1.5)); for(size_t e=0U;e<graph_->edge_count;++e){const UmiWsGraphNode *a=nullptr,*b=nullptr; for(size_t n=0U;n<graph_->node_count;++n){if(std::strcmp(graph_->nodes[n].node_id,graph_->edges[e].source_id)==0)a=&graph_->nodes[n]; if(std::strcmp(graph_->nodes[n].node_id,graph_->edges[e].target_id)==0)b=&graph_->nodes[n];} if(a!=nullptr&&b!=nullptr)p.drawLine(QPointF(a->x,a->y),QPointF(b->x,b->y));} p.setBrush(QColor(55,85,145)); p.setPen(QColor(125,155,220)); for(size_t n=0U;n<graph_->node_count;++n){QRectF r(graph_->nodes[n].x-42.0,graph_->nodes[n].y-18.0,84.0,36.0);p.drawRoundedRect(r,4.0,4.0);p.drawText(r,Qt::AlignCenter,QString::fromUtf8(graph_->nodes[n].label));}}
