 ...
 *
 * PURPOSE:
 *   Implement the split host qt6 behaviour used by its public contract and
 *   client applications.
 *
...
extern "C" UmiQt6WidgetHandle umi_qt6_ws_split_host_create(const UmiWsSplitRegion *split, UmiQt6WidgetHandle first, UmiQt6WidgetHandle second) {
    ...
    auto *host = new QSplitter(orientation); /* Protect caller-owned memory by checking that required state is available before it is used. */ if (first != nullptr) host->addWidget(static_cast<QWidget *>(first)); /* Protect caller-owned memory by checking that required state is available before it is used. */ if (second != nullptr) host->addWidget(static_cast<QWidget *>(second));
