...
int main(void) {
    ...
    UMI_TEST_REQUIRE(umi_application_production_lifecycle_gate_evaluate(UMI_APPLICATION_PRODUCTION_GATE_ACCEPTANCE, &runtime->binding, &runtime->readiness, &runtime->manifest_drift, &runtime->recovery, &runtime->acceptance, &gate) == UMI_STATUS_OK);
    UMI_TEST_REQUIRE(gate.passed);
    free(runtime);
    return 0;
}
