/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/studio_runtime/test_activation_profile_ai_validation.c
 *
 * PURPOSE:
 *   Verify command activation mapping for AI Validation.
 *
 * Created by: Sammy Hegab
 * Organisation: Umicom Foundation
 * Licence: MIT
 *---------------------------------------------------------------------------*/
#include <assert.h>
#include "umicom/studio_runtime/activation_profiles/ai_validation.h"

int main(void)
{
    const UmiStudioRuntimeActivationProfile *profile =
        umi_studio_activation_profile_ai_validation();

    assert(profile != NULL);
    assert(profile->rule_count == 1U);
    assert(profile->rules[0].surface == UMI_STUDIO_SURFACE_AI_VALIDATION);
    assert(umi_studio_activation_rule_matches(
        &profile->rules[0],
        "ai.developer.open-validation"));
    return 0;
}
