/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_workspace_memory.c
 *
 * PURPOSE:
 *   Verify the AI Workspace Memory developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiWorkspaceMemory(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.workspace-memory") == 0);
