/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_change_review.c
 *
 * PURPOSE:
 *   Verify the AI Change Review developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiChangeReview(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.change-review") == 0);
