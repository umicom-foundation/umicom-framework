/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_signature_active_parameter.c
 *
 * PURPOSE:
 *   Verify the Signature Active Parameter developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageSignatureActiveParameter(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.signature-active-parameter") == 0);
