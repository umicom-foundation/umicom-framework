/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_provider_failover.c
 *
 * PURPOSE:
 *   Verify the Language Provider Failover developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageProviderFailover(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.provider-failover") == 0);
