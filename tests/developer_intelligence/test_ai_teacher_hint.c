/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_teacher_hint.c
 *
 * PURPOSE:
 *   Verify the AI Teacher Hint developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiTeacherHint(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.teacher-hint") == 0);
