/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_semantic_token_delta.c
 *
 * PURPOSE:
 *   Verify the Semantic Token Delta developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageSemanticTokenDelta(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.semantic-token-delta") == 0);
