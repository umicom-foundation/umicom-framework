/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_code_test_generation.c
 *
 * PURPOSE:
 *   Verify the AI Test Generation developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiCodeTestGeneration(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.code-test-generation") == 0);
