/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_assembly_profile.c
 *
 * PURPOSE:
 *   Verify the Assembly Language Profile developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageAssemblyProfile(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.assembly-profile") == 0);
