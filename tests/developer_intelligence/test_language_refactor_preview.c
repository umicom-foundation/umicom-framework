/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_refactor_preview.c
 *
 * PURPOSE:
 *   Verify the Refactor Preview developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageRefactorPreview(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.refactor-preview") == 0);
