/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_rag_collection_rank.c
 *
 * PURPOSE:
 *   Verify the RAG Collection Rank developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiRagCollectionRank(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.rag-collection-rank") == 0);
