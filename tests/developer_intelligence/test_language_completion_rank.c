/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_completion_rank.c
 *
 * PURPOSE:
 *   Verify the Completion Rank developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageCompletionRank(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.completion-rank") == 0);
