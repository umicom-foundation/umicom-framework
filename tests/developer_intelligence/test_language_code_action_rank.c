/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_code_action_rank.c
 *
 * PURPOSE:
 *   Verify the Code Action Rank developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageCodeActionRank(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.code-action-rank") == 0);
