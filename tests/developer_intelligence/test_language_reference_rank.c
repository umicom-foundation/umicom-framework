/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_reference_rank.c
 *
 * PURPOSE:
 *   Verify the Reference Rank developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageReferenceRank(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.reference-rank") == 0);
