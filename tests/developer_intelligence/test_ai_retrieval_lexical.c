/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_retrieval_lexical.c
 *
 * PURPOSE:
 *   Verify the RAG Lexical Retrieval developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiRetrievalLexical(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.retrieval-lexical") == 0);
