/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_context_budget.c
 *
 * PURPOSE:
 *   Verify the AI Context Budget developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiContextBudget(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.context-budget") == 0);
