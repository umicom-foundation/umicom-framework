/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_code_review.c
 *
 * PURPOSE:
 *   Verify the AI Code Review developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiCodeReview(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.code-review") == 0);
