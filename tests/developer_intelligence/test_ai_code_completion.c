/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_code_completion.c
 *
 * PURPOSE:
 *   Verify the AI Code Completion developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiCodeCompletion(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.code-completion") == 0);
