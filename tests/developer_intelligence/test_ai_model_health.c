/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_model_health.c
 *
 * PURPOSE:
 *   Verify the Model Health developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiModelHealth(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.model-health") == 0);
