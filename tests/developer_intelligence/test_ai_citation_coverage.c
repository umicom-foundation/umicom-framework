/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_citation_coverage.c
 *
 * PURPOSE:
 *   Verify the Citation Coverage developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiCitationCoverage(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.citation-coverage") == 0);
