/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_repository_change.c
 *
 * PURPOSE:
 *   Verify the AI Repository Change developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiRepositoryChange(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.repository-change") == 0);
