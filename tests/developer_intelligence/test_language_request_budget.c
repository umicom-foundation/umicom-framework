/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_request_budget.c
 *
 * PURPOSE:
 *   Verify the Language Request Budget developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageRequestBudget(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.request-budget") == 0);
