/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_retrieval_fusion.c
 *
 * PURPOSE:
 *   Verify the RAG Hybrid Fusion developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiRetrievalFusion(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.retrieval-fusion") == 0);
