/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_checkpoint_policy.c
 *
 * PURPOSE:
 *   Verify the AI Checkpoint Policy developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiCheckpointPolicy(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.checkpoint-policy") == 0);
