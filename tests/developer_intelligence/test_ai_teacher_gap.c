/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_teacher_gap.c
 *
 * PURPOSE:
 *   Verify the AI Teacher Skill Gap developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiTeacherGap(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.teacher-gap") == 0);
