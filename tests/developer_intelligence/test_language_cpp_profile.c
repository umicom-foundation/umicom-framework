/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_cpp_profile.c
 *
 * PURPOSE:
 *   Verify the C++ Language Profile developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageCppProfile(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.cpp-profile") == 0);
