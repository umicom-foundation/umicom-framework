/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_document_link.c
 *
 * PURPOSE:
 *   Verify the Document Link developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageDocumentLink(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.document-link") == 0);
