/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_selection_range.c
 *
 * PURPOSE:
 *   Verify the Selection Range developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageSelectionRange(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.selection-range") == 0);
