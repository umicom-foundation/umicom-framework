/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_compilation_database.c
 *
 * PURPOSE:
 *   Verify the Compilation Database developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageCompilationDatabase(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.compilation-database") == 0);
