/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_code_lens.c
 *
 * PURPOSE:
 *   Verify the Code Lens developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageCodeLens(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.code-lens") == 0);
