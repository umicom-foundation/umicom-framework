/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_completion_context.c
 *
 * PURPOSE:
 *   Verify the Completion Context developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageCompletionContext(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.completion-context") == 0);
