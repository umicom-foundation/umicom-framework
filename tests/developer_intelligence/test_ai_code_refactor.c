/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_code_refactor.c
 *
 * PURPOSE:
 *   Verify the AI Code Refactor developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiCodeRefactor(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.code-refactor") == 0);
