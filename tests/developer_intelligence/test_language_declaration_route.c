/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_declaration_route.c
 *
 * PURPOSE:
 *   Verify the Declaration Route developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageDeclarationRoute(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.declaration-route") == 0);
