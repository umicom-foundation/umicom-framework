/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_repository_index.c
 *
 * PURPOSE:
 *   Verify the AI Repository Index developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiRepositoryIndex(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.repository-index") == 0);
