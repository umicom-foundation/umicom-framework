/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_context_source_rank.c
 *
 * PURPOSE:
 *   Verify the AI Context Source Rank developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiContextSourceRank(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.context-source-rank") == 0);
