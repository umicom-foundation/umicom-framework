/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_inlay_hint_rank.c
 *
 * PURPOSE:
 *   Verify the Inlay Hint Rank developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageInlayHintRank(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.inlay-hint-rank") == 0);
