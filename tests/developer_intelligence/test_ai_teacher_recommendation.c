/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_teacher_recommendation.c
 *
 * PURPOSE:
 *   Verify the AI Teacher Recommendation developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiTeacherRecommendation(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.teacher-recommendation") == 0);
