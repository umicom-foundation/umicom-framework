/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_include_graph.c
 *
 * PURPOSE:
 *   Verify the Include Graph developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageIncludeGraph(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.include-graph") == 0);
