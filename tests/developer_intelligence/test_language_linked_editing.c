/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_linked_editing.c
 *
 * PURPOSE:
 *   Verify the Linked Editing developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageLinkedEditing(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.linked-editing") == 0);
