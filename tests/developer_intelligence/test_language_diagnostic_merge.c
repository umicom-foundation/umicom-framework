/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_diagnostic_merge.c
 *
 * PURPOSE:
 *   Verify the Diagnostic Merge developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageDiagnosticMerge(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.diagnostic-merge") == 0);
