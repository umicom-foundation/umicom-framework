/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_retrieval_vector.c
 *
 * PURPOSE:
 *   Verify the RAG Vector Retrieval developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiRetrievalVector(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.retrieval-vector") == 0);
