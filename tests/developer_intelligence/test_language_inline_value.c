/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_inline_value.c
 *
 * PURPOSE:
 *   Verify the Inline Value developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageInlineValue(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.inline-value") == 0);
