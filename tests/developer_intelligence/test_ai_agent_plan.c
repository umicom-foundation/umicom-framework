/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_agent_plan.c
 *
 * PURPOSE:
 *   Verify the AI Agent Plan developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiAgentPlan(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.agent-plan") == 0);
