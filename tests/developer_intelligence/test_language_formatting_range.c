/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_formatting_range.c
 *
 * PURPOSE:
 *   Verify the Formatting Range developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageFormattingRange(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.formatting-range") == 0);
