/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_call_hierarchy.c
 *
 * PURPOSE:
 *   Verify the Call Hierarchy developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageCallHierarchy(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.call-hierarchy") == 0);
