/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_local_runtime_route.c
 *
 * PURPOSE:
 *   Verify the Local AI Runtime Route developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiLocalRuntimeRoute(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.local-runtime-route") == 0);
