/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_model_route.c
 *
 * PURPOSE:
 *   Verify the Model Route developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiModelRoute(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.model-route") == 0);
