/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_conversation_memory.c
 *
 * PURPOSE:
 *   Verify the AI Conversation Memory developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiConversationMemory(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.conversation-memory") == 0);
