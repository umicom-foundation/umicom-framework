/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_rename_plan.c
 *
 * PURPOSE:
 *   Verify the Rename Plan developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageRenamePlan(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.rename-plan") == 0);
