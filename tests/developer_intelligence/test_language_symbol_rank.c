/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_symbol_rank.c
 *
 * PURPOSE:
 *   Verify the Workspace Symbol Rank developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageSymbolRank(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.symbol-rank") == 0);
