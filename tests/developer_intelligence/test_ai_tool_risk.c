/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_tool_risk.c
 *
 * PURPOSE:
 *   Verify the AI Tool Risk developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiToolRisk(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.tool-risk") == 0);
