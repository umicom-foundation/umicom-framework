/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_type_hierarchy.c
 *
 * PURPOSE:
 *   Verify the Type Hierarchy developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageTypeHierarchy(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.type-hierarchy") == 0);
