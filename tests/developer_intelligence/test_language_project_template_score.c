/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_project_template_score.c
 *
 * PURPOSE:
 *   Verify the Project Template Selection developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageProjectTemplateScore(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.project-template-score") == 0);
