/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_code_explanation.c
 *
 * PURPOSE:
 *   Verify the AI Code Explanation developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiCodeExplanation(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.code-explanation") == 0);
