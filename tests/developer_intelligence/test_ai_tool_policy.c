/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_tool_policy.c
 *
 * PURPOSE:
 *   Verify the AI Tool Policy developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiToolPolicy(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.tool-policy") == 0);
