/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_prompt_budget.c
 *
 * PURPOSE:
 *   Verify the Prompt Budget developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiPromptBudget(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.prompt-budget") == 0);
