/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_provider_fallback.c
 *
 * PURPOSE:
 *   Verify the Provider Fallback developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiProviderFallback(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.provider-fallback") == 0);
