/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_implementation_route.c
 *
 * PURPOSE:
 *   Verify the Implementation Route developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageImplementationRoute(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.implementation-route") == 0);
