/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_rerank_score.c
 *
 * PURPOSE:
 *   Verify the RAG Rerank developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiRerankScore(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.rerank-score") == 0);
