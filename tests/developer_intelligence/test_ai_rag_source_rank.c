/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_rag_source_rank.c
 *
 * PURPOSE:
 *   Verify the RAG Source Rank developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiRagSourceRank(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.rag-source-rank") == 0);
