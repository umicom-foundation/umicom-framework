/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_type_definition_route.c
 *
 * PURPOSE:
 *   Verify the Type Definition Route developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageTypeDefinitionRoute(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.type-definition-route") == 0);
    ...
    assert(snapshot.localPreferred == 1);
    return 0;
}
