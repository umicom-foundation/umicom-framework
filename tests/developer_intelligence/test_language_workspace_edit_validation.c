/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_workspace_edit_validation.c
 *
 * PURPOSE:
 *   Verify the Workspace Edit Validation developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageWorkspaceEditValidation(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.workspace-edit-validation") == 0);
