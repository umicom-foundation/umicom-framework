/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_rename_conflict.c
 *
 * PURPOSE:
 *   Verify the Rename Conflict developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageRenameConflict(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.rename-conflict") == 0);
