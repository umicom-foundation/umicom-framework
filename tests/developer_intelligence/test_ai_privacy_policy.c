/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_privacy_policy.c
 *
 * PURPOSE:
 *   Verify the AI Privacy Policy developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiPrivacyPolicy(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.privacy-policy") == 0);
