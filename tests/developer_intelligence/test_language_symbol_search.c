/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_symbol_search.c
 *
 * PURPOSE:
 *   Verify the Workspace Symbol Search developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageSymbolSearch(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.symbol-search") == 0);
