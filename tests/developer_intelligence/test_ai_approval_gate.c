/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_approval_gate.c
 *
 * PURPOSE:
 *   Verify the AI Approval Gate developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiApprovalGate(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.approval-gate") == 0);
