/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_hover_merge.c
 *
 * PURPOSE:
 *   Verify the Hover Merge developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageHoverMerge(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.hover-merge") == 0);
