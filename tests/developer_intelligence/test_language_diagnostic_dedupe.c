/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_diagnostic_dedupe.c
 *
 * PURPOSE:
 *   Verify the Diagnostic Deduplication developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageDiagnosticDedupe(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.diagnostic-dedupe") == 0);
