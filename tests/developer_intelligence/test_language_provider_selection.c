/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_provider_selection.c
 *
 * PURPOSE:
 *   Verify the Language Provider Selection developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageProviderSelection(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.provider-selection") == 0);
