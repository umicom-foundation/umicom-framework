/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_c23_profile.c
 *
 * PURPOSE:
 *   Verify the C23 Language Profile developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageC23Profile(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.c23-profile") == 0);
