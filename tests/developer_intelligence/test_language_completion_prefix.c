/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_completion_prefix.c
 *
 * PURPOSE:
 *   Verify the Completion Prefix developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageCompletionPrefix(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.completion-prefix") == 0);
