/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_ai_edit_risk.c
 *
 * PURPOSE:
 *   Verify the AI Edit Risk developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateAiEditRisk(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ai.edit-risk") == 0);
