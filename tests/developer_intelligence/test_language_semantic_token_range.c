/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_intelligence/test_language_semantic_token_range.c
 *
 * PURPOSE:
 *   Verify the Semantic Token Range developer-intelligence policy.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperIntelligenceEvaluateLanguageSemanticTokenRange(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "language.semantic-token-range") == 0);
