/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/debug_runtime/test_request_set_breakpoints.c
 *
 * PURPOSE:
 *   Verify DAP setBreakpoints is emitted through the persistent framed transport.
 *
...
int main(void)
{
    ...
    assert(umi_debug_runtime_request_set_breakpoints(f.adapter, "C:/work/main.c", NULL, 0U, 0, &sequence) == UMI_STATUS_OK);
    assert(sequence != 0U);
    assert(debug_request_test_fixture_written(
        &f, written, sizeof(written)) == UMI_STATUS_OK);
    assert(strstr(written, "setBreakpoints") != NULL);
