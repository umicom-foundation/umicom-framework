/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/debug_runtime/test_request_scopes.c
 *
 * PURPOSE:
 *   Verify DAP scopes is framed and written through the real adapter path.
 *
...
int main(void)
{
    ...
    assert(umi_debug_runtime_request_scopes(f.adapter, 11U, &sequence) == UMI_STATUS_OK);
    assert(sequence != 0U);
    assert(debug_request_test_fixture_written(
        &f, written, sizeof(written)) == UMI_STATUS_OK);
    assert(strstr(written, "scopes") != NULL);
