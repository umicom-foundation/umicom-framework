/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/debug_runtime/test_request_breakpoint_locations.c
 *
 * PURPOSE:
 *   Verify DAP breakpointLocations is emitted through the persistent framed transport.
 *
...
int main(void)
{
    ...
    assert(umi_debug_runtime_request_breakpoint_locations(f.adapter, "C:/work/a.c", 4U, 1U, 8U, 1U, &sequence) == UMI_STATUS_OK);
    assert(sequence != 0U);
    assert(debug_request_test_fixture_written(
        &f, written, sizeof(written)) == UMI_STATUS_OK);
    assert(strstr(written, "breakpointLocations") != NULL);
