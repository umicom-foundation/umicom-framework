/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/debug_runtime/test_request_continue.c
 *
 * PURPOSE:
 *   Verify DAP continue is framed and written through the real adapter path.
 *
...
int main(void)
{
    ...
    assert(umi_debug_runtime_request_continue(f.adapter, 7U, &sequence) == UMI_STATUS_OK);
    assert(sequence != 0U);
    assert(debug_request_test_fixture_written(
        &f, written, sizeof(written)) == UMI_STATUS_OK);
    assert(strstr(written, "continue") != NULL);
