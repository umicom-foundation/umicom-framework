/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/debug_runtime/test_request_disassemble.c
 *
 * PURPOSE:
 *   Verify DAP disassemble is emitted through the persistent framed transport.
 *
...
int main(void)
{
    ...
    assert(umi_debug_runtime_request_disassemble(f.adapter, "0x1000", 0, 0, 20U, 1, &sequence) == UMI_STATUS_OK);
    assert(sequence != 0U);
    assert(debug_request_test_fixture_written(
        &f, written, sizeof(written)) == UMI_STATUS_OK);
    assert(strstr(written, "disassemble") != NULL);
