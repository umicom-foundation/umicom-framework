/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/debug_runtime/test_request_threads.c
 *
 * PURPOSE:
 *   Verify DAP threads is framed and written through the real adapter path.
 *
...
int main(void)
{
    ...
    assert(umi_debug_runtime_request_threads(f.adapter, &sequence) == UMI_STATUS_OK);
    assert(sequence != 0U);
    assert(debug_request_test_fixture_written(
        &f, written, sizeof(written)) == UMI_STATUS_OK);
    assert(strstr(written, "threads") != NULL);
