/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/debug_runtime/test_request_set_function_breakpoints.c
 *
 * PURPOSE:
 *   Verify DAP setFunctionBreakpoints is emitted through the persistent framed transport.
 *
...
int main(void)
{
    ...
    assert(umi_debug_runtime_request_set_function_breakpoints(f.adapter, NULL, 0U, &sequence) == UMI_STATUS_OK);
    assert(sequence != 0U);
    assert(debug_request_test_fixture_written(
        &f, written, sizeof(written)) == UMI_STATUS_OK);
    assert(strstr(written, "setFunctionBreakpoints") != NULL);
