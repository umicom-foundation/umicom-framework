/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/runtime_bootstrap/test_graph_ordering.c
 *
 * PURPOSE:
 *   Implement the test graph ordering behavior for
 *   Umicom Framework.
 *
 ...
 *
 * PURPOSE:
 *   Focused regression coverage for Produce deterministic topological service order from dependency graphs.
 *---------------------------------------------------------------------------*/
#include <stdint.h>
#include <string.h>
...
int main(void) {
    ...
    CHECK(umi_bootstrap_graph_order(&g,&order)==UMI_STATUS_OK);
    CHECK(order.count==2U && strcmp(order.ids[0],"svc.a")==0);
    return 0;
}
