/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/test_delivery_package_manifest_v2.c
 *
 * PURPOSE:
...
int main(void)
{
    ...
    assert(umi_package_manifest_v2_validate(&manifest) ==
           UMI_STATUS_INVALID_STATE);
    assert(umi_package_manifest_v2_add(
               &manifest, "build/bin/umicom-studio-console",
               "bin/umicom-studio-console", UMI_PACKAGE_FILE_EXECUTABLE_V2,
               1) == UMI_STATUS_OK);
    assert(umi_package_manifest_v2_add(
               &manifest, "LICENSE", "share/licenses/LICENSE",
               UMI_PACKAGE_FILE_LICENCE_V2, 1) == UMI_STATUS_OK);
    assert(umi_package_manifest_v2_validate(&manifest) == UMI_STATUS_OK);
    assert(umi_package_manifest_v2_find(
