/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_trade_counterparty.c
 *
 * PURPOSE:
 *   Verify Trade Counterparty shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_trade_counterparty(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "trade.counterparty") == 0);
