/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_settlement_reconciliation.c
 *
 * PURPOSE:
 *   Verify Settlement Reconciliation shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_settlement_reconciliation(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "settlement.reconciliation") == 0);
