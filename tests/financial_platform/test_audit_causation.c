/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_audit_causation.c
 *
 * PURPOSE:
 *   Verify Financial Audit Causation shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_audit_causation(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "audit.causation") == 0);
