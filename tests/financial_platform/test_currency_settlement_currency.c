/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_currency_settlement_currency.c
 *
 * PURPOSE:
 *   Verify Settlement Currency Readiness shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_currency_settlement_currency(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "currency.settlement-currency") == 0);
