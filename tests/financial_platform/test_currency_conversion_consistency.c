/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_currency_conversion_consistency.c
 *
 * PURPOSE:
 *   Verify FX Conversion Consistency shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_currency_conversion_consistency(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "currency.conversion-consistency") == 0);
