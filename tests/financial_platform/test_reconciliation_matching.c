/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_reconciliation_matching.c
 *
 * PURPOSE:
 *   Verify Reconciliation Matching shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_reconciliation_matching(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "reconciliation.matching") == 0);
