/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_trade_identity.c
 *
 * PURPOSE:
 *   Verify Trade Identity shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_trade_identity(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "trade.identity") == 0);
