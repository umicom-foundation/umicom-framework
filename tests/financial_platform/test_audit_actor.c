/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_audit_actor.c
 *
 * PURPOSE:
 *   Verify Financial Audit Actor shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_audit_actor(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "audit.actor") == 0);
