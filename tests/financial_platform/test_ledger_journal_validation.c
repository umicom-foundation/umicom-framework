/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_ledger_journal_validation.c
 *
 * PURPOSE:
 *   Verify Journal Validation shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_ledger_journal_validation(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ledger.journal-validation") == 0);
