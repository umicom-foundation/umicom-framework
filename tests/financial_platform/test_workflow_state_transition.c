/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_workflow_state_transition.c
 *
 * PURPOSE:
 *   Verify Workflow State Transition shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_workflow_state_transition(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "workflow.state-transition") == 0);
