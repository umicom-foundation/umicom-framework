/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_currency_code_validation.c
 *
 * PURPOSE:
 *   Verify Currency Code Validation shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_currency_code_validation(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "currency.code-validation") == 0);
