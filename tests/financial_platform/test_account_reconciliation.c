/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_account_reconciliation.c
 *
 * PURPOSE:
 *   Verify Account Reconciliation shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_account_reconciliation(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "account.reconciliation") == 0);
