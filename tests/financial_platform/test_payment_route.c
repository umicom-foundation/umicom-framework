/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_payment_route.c
 *
 * PURPOSE:
 *   Verify Payment Route shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_payment_route(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "payment.route") == 0);
