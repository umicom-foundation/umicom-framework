/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_ledger_period.c
 *
 * PURPOSE:
 *   Verify Accounting Period Readiness shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_ledger_period(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ledger.period") == 0);
