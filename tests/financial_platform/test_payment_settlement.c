/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_payment_settlement.c
 *
 * PURPOSE:
 *   Verify Payment Settlement Readiness shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_payment_settlement(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "payment.settlement") == 0);
