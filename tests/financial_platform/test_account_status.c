/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_account_status.c
 *
 * PURPOSE:
 *   Verify Account Status shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_account_status(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "account.status") == 0);
