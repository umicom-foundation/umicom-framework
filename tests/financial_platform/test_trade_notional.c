/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_trade_notional.c
 *
 * PURPOSE:
 *   Verify Trade Notional shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_trade_notional(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "trade.notional") == 0);
