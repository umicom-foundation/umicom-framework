/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_position_gross_exposure.c
 *
 * PURPOSE:
 *   Verify Gross Exposure shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_position_gross_exposure(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "position.gross-exposure") == 0);
