/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_settlement_securities_leg.c
 *
 * PURPOSE:
 *   Verify Settlement Securities Leg shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_settlement_securities_leg(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "settlement.securities-leg") == 0);
