/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_audit_persistence.c
 *
 * PURPOSE:
 *   Verify Financial Audit Persistence shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_audit_persistence(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "audit.persistence") == 0);
