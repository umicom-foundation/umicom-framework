/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_account_balance_consistency.c
 *
 * PURPOSE:
 *   Verify Account Balance Consistency shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_account_balance_consistency(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "account.balance-consistency") == 0);
