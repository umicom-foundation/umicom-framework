/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_payment_cutoff.c
 *
 * PURPOSE:
 *   Verify Payment Cut-off shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_payment_cutoff(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "payment.cutoff") == 0);
