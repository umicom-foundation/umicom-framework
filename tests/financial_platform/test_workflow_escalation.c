/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_workflow_escalation.c
 *
 * PURPOSE:
 *   Verify Workflow Escalation shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_workflow_escalation(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "workflow.escalation") == 0);
