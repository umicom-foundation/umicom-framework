/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_account_currency.c
 *
 * PURPOSE:
 *   Verify Account Currency shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_account_currency(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "account.currency") == 0);
