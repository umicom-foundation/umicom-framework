/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_instrument_market_data.c
 *
 * PURPOSE:
 *   Verify Instrument Market Data shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_instrument_market_data(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "instrument.market-data") == 0);
