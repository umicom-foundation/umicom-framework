/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_payment_instruction.c
 *
 * PURPOSE:
 *   Verify Payment Instruction shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_payment_instruction(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "payment.instruction") == 0);
