/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_ledger_posting.c
 *
 * PURPOSE:
 *   Verify Ledger Posting Readiness shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_ledger_posting(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ledger.posting") == 0);
