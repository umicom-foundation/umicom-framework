/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_currency_rate_tolerance.c
 *
 * PURPOSE:
 *   Verify Exchange Rate Tolerance shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_currency_rate_tolerance(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "currency.rate-tolerance") == 0);
