/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_instrument_maturity.c
 *
 * PURPOSE:
 *   Verify Instrument Maturity shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_instrument_maturity(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "instrument.maturity") == 0);
