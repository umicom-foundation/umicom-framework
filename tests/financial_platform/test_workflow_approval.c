/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_workflow_approval.c
 *
 * PURPOSE:
 *   Verify Workflow Approval shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_workflow_approval(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "workflow.approval") == 0);
