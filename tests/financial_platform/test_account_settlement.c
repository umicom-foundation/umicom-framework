/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_account_settlement.c
 *
 * PURPOSE:
 *   Verify Account Settlement Readiness shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_account_settlement(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "account.settlement") == 0);
