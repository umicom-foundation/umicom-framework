/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_party_identity.c
 *
 * PURPOSE:
 *   Verify Party Identity shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_party_identity(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "party.identity") == 0);
