/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_audit_readiness.c
 *
 * PURPOSE:
 *   Verify Financial Audit Readiness shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_audit_readiness(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "audit.readiness") == 0);
