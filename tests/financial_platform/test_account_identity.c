/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_account_identity.c
 *
 * PURPOSE:
 *   Verify Account Identity shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_account_identity(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "account.identity") == 0);
