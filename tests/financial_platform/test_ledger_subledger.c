/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_ledger_subledger.c
 *
 * PURPOSE:
 *   Verify Subledger Readiness shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_ledger_subledger(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ledger.subledger") == 0);
