/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_payment_currency.c
 *
 * PURPOSE:
 *   Verify Payment Currency shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_payment_currency(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "payment.currency") == 0);
