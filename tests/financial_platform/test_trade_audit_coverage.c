/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_trade_audit_coverage.c
 *
 * PURPOSE:
 *   Verify Trade Audit Coverage shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_trade_audit_coverage(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "trade.audit-coverage") == 0);
