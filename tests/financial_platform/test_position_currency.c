/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_position_currency.c
 *
 * PURPOSE:
 *   Verify Position Currency shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_position_currency(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "position.currency") == 0);
