/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_instrument_price_precision.c
 *
 * PURPOSE:
 *   Verify Instrument Price Precision shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_instrument_price_precision(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "instrument.price-precision") == 0);
