/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_audit_sequence.c
 *
 * PURPOSE:
 *   Verify Financial Audit Sequence shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_audit_sequence(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "audit.sequence") == 0);
