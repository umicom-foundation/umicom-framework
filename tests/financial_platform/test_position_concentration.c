/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_position_concentration.c
 *
 * PURPOSE:
 *   Verify Position Concentration shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_position_concentration(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "position.concentration") == 0);
