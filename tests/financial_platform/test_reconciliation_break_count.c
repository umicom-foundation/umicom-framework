/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_reconciliation_break_count.c
 *
 * PURPOSE:
 *   Verify Reconciliation Break Count shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_reconciliation_break_count(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "reconciliation.break-count") == 0);
