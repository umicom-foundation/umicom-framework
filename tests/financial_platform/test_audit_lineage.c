/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_audit_lineage.c
 *
 * PURPOSE:
 *   Verify Financial Audit Lineage shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_audit_lineage(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "audit.lineage") == 0);
