/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_position_net_exposure.c
 *
 * PURPOSE:
 *   Verify Net Exposure shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_position_net_exposure(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "position.net-exposure") == 0);
