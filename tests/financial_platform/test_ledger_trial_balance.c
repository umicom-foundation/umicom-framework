/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_ledger_trial_balance.c
 *
 * PURPOSE:
 *   Verify Trial Balance shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_ledger_trial_balance(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ledger.trial-balance") == 0);
