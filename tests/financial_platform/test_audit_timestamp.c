/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_audit_timestamp.c
 *
 * PURPOSE:
 *   Verify Financial Audit Timestamp shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_audit_timestamp(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "audit.timestamp") == 0);
