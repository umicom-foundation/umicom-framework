/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_workflow_role_coverage.c
 *
 * PURPOSE:
 *   Verify Workflow Role Coverage shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_workflow_role_coverage(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "workflow.role-coverage") == 0);
