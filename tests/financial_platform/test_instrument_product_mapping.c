/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_instrument_product_mapping.c
 *
 * PURPOSE:
 *   Verify Instrument Product Mapping shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_instrument_product_mapping(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "instrument.product-mapping") == 0);
