/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_payment_beneficiary.c
 *
 * PURPOSE:
 *   Verify Payment Beneficiary shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_payment_beneficiary(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "payment.beneficiary") == 0);
