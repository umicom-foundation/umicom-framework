/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_currency_catalogue_coverage.c
 *
 * PURPOSE:
 *   Verify Currency Catalogue Coverage shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_currency_catalogue_coverage(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "currency.catalogue-coverage") == 0);
