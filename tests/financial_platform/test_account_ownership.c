/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_account_ownership.c
 *
 * PURPOSE:
 *   Verify Account Ownership shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_account_ownership(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "account.ownership") == 0);
