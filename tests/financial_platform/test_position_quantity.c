/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_position_quantity.c
 *
 * PURPOSE:
 *   Verify Position Quantity shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_position_quantity(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "position.quantity") == 0);
