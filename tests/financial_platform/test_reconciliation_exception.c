/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_reconciliation_exception.c
 *
 * PURPOSE:
 *   Verify Reconciliation Exception Approval shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_reconciliation_exception(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "reconciliation.exception") == 0);
