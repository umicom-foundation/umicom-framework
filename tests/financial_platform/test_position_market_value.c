/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_position_market_value.c
 *
 * PURPOSE:
 *   Verify Position Market Value shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_position_market_value(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "position.market-value") == 0);
