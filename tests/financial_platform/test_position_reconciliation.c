/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_position_reconciliation.c
 *
 * PURPOSE:
 *   Verify Position Reconciliation shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_position_reconciliation(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "position.reconciliation") == 0);
