/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_position_pnl.c
 *
 * PURPOSE:
 *   Verify Position P&L shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_position_pnl(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "position.pnl") == 0);
