/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_trade_economics.c
 *
 * PURPOSE:
 *   Verify Trade Economics shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_trade_economics(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "trade.economics") == 0);
