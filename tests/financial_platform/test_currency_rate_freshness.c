/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_currency_rate_freshness.c
 *
 * PURPOSE:
 *   Verify Exchange Rate Freshness shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_currency_rate_freshness(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "currency.rate-freshness") == 0);
