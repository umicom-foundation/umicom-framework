/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_trade_lifecycle.c
 *
 * PURPOSE:
 *   Verify Trade Lifecycle shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_trade_lifecycle(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "trade.lifecycle") == 0);
