/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_instrument_identity.c
 *
 * PURPOSE:
 *   Verify Instrument Identity shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_instrument_identity(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "instrument.identity") == 0);
