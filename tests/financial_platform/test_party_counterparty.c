/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_party_counterparty.c
 *
 * PURPOSE:
 *   Verify Counterparty Readiness shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_party_counterparty(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "party.counterparty") == 0);
