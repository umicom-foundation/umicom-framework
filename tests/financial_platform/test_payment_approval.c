/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_payment_approval.c
 *
 * PURPOSE:
 *   Verify Payment Approval shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_payment_approval(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "payment.approval") == 0);
