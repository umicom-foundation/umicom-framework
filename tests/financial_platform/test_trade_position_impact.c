/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_trade_position_impact.c
 *
 * PURPOSE:
 *   Verify Trade Position Impact shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_trade_position_impact(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "trade.position-impact") == 0);
