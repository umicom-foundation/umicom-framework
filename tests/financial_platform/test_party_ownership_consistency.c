/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_party_ownership_consistency.c
 *
 * PURPOSE:
 *   Verify Ownership Consistency shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_party_ownership_consistency(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "party.ownership-consistency") == 0);
