/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_audit_immutability.c
 *
 * PURPOSE:
 *   Verify Financial Audit Immutability shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_audit_immutability(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "audit.immutability") == 0);
