/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_payment_amount.c
 *
 * PURPOSE:
 *   Verify Payment Amount shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_payment_amount(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "payment.amount") == 0);
