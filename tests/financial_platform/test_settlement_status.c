/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_settlement_status.c
 *
 * PURPOSE:
 *   Verify Settlement Status shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_settlement_status(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "settlement.status") == 0);
