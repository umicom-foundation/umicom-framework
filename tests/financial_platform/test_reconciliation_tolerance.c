/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_reconciliation_tolerance.c
 *
 * PURPOSE:
 *   Verify Reconciliation Tolerance shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_reconciliation_tolerance(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "reconciliation.tolerance") == 0);
