/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_party_legal_entity.c
 *
 * PURPOSE:
 *   Verify Legal Entity Readiness shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_party_legal_entity(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "party.legal-entity") == 0);
