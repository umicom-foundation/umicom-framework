/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_party_lifecycle.c
 *
 * PURPOSE:
 *   Verify Party Lifecycle shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_party_lifecycle(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "party.lifecycle") == 0);
