/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_currency_precision_validation.c
 *
 * PURPOSE:
 *   Verify Currency Precision Validation shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_currency_precision_validation(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "currency.precision-validation") == 0);
