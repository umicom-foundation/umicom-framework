/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/financial_platform/test_trade_settlement.c
 *
 * PURPOSE:
 *   Verify Trade Settlement Readiness shared-financial-platform behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_financial_platform_evaluate_trade_settlement(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "trade.settlement") == 0);
