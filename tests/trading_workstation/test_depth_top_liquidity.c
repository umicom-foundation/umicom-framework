/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_depth_top_liquidity.c
 *
 * PURPOSE:
 *   Verify the Top Liquidity professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_depth_top_liquidity(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "depth.top-liquidity") == 0);
