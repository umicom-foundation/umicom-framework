/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_chart_tape_activity.c
 *
 * PURPOSE:
 *   Verify the Time and Sales Activity professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_chart_tape_activity(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "chart_tape.activity") == 0);
