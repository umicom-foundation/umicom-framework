/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_research_alert_attention.c
 *
 * PURPOSE:
 *   Verify the Alert Attention professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_research_alert_attention(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "research.alert-attention") == 0);
