/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_watchlist_capacity.c
 *
 * PURPOSE:
 *   Verify the Watchlist Capacity professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_watchlist_capacity(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "watchlist.capacity") == 0);
