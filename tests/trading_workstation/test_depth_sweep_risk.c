/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_depth_sweep_risk.c
 *
 * PURPOSE:
 *   Verify the Liquidity Sweep Risk professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_depth_sweep_risk(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "depth.sweep-risk") == 0);
