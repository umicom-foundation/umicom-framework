/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_depth_spread_ticks.c
 *
 * PURPOSE:
 *   Verify the Depth Spread Ticks professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_depth_spread_ticks(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "depth.spread-ticks") == 0);
