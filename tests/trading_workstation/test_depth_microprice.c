/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_depth_microprice.c
 *
 * PURPOSE:
 *   Verify the Depth Microprice professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_depth_microprice(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "depth.microprice") == 0);
    assert(snapshot.revision == 55U);
    assert(snapshot.score >= 0.0 && snapshot.score <= 100.0);
    assert(snapshot.blocked == 0);
    assert(snapshot.value > 99.9 && snapshot.value < 100.1);
    return 0;
}
