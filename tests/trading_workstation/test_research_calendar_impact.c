/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_research_calendar_impact.c
 *
 * PURPOSE:
 *   Verify the Economic Calendar Impact professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_research_calendar_impact(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "research.calendar-impact") == 0);
