/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_market_feed_health.c
 *
 * PURPOSE:
 *   Verify the Market Feed Health professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_market_feed_health(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "market.feed-health") == 0);
