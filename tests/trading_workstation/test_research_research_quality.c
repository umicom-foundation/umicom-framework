/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_research_research_quality.c
 *
 * PURPOSE:
 *   Verify the Research Evidence Quality professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_research_research_quality(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "research.research-quality") == 0);
