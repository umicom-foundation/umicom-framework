/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_market_range_position.c
 *
 * PURPOSE:
 *   Verify the Range Position professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_market_range_position(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "market.range-position") == 0);
