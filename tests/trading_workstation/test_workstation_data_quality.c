/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_workstation_data_quality.c
 *
 * PURPOSE:
 *   Verify the Trading Data Quality professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_workstation_data_quality(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "workstation.data-quality") == 0);
