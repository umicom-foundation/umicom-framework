/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_execution_submit_readiness.c
 *
 * PURPOSE:
 *   Verify the Submit Readiness professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_execution_submit_readiness(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "execution.submit-readiness") == 0);
