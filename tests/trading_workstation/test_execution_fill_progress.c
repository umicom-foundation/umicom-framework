/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_execution_fill_progress.c
 *
 * PURPOSE:
 *   Verify the Fill Progress professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_execution_fill_progress(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "execution.fill-progress") == 0);
    assert(snapshot.revision == 55U);
    assert(snapshot.score >= 0.0 && snapshot.score <= 100.0);
    assert(snapshot.blocked == 0);
    assert(snapshot.ratio > 49.9 && snapshot.ratio < 50.1);
    return 0;
}
