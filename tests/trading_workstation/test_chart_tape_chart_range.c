/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_chart_tape_chart_range.c
 *
 * PURPOSE:
 *   Verify the Chart Range professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_chart_tape_chart_range(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "chart_tape.chart-range") == 0);
