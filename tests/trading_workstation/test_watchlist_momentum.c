/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_watchlist_momentum.c
 *
 * PURPOSE:
 *   Verify the Watchlist Momentum professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_watchlist_momentum(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "watchlist.momentum") == 0);
