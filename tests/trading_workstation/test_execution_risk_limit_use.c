/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_execution_risk_limit_use.c
 *
 * PURPOSE:
 *   Verify the Risk Limit Utilisation professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_execution_risk_limit_use(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "execution.risk-limit-use") == 0);
