/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_research_scanner_score.c
 *
 * PURPOSE:
 *   Verify the Market Scanner Score professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_research_scanner_score(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "research.scanner-score") == 0);
