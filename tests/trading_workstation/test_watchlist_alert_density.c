/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_watchlist_alert_density.c
 *
 * PURPOSE:
 *   Verify the Watchlist Alert Density professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_watchlist_alert_density(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "watchlist.alert-density") == 0);
