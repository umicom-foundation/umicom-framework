/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_execution_cancel_readiness.c
 *
 * PURPOSE:
 *   Verify the Cancel Readiness professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_execution_cancel_readiness(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "execution.cancel-readiness") == 0);
