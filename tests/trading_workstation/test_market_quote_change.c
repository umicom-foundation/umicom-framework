/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_market_quote_change.c
 *
 * PURPOSE:
 *   Verify the Quote Change professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_market_quote_change(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "market.quote-change") == 0);
    assert(snapshot.revision == 55U);
    assert(snapshot.score >= 0.0 && snapshot.score <= 100.0);
    assert(snapshot.blocked == 0);
    assert(fabs(snapshot.value - 2.0) < 0.0001);
    return 0;
}
