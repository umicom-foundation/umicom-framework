/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_execution_limit_distance.c
 *
 * PURPOSE:
 *   Verify the Limit Distance professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_execution_limit_distance(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "execution.limit-distance") == 0);
