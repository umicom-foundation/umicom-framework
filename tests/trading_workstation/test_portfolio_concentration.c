/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_portfolio_concentration.c
 *
 * PURPOSE:
 *   Verify the Position Concentration professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_portfolio_concentration(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "portfolio.concentration") == 0);
