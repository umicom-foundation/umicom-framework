/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_chart_tape_linked_context.c
 *
 * PURPOSE:
 *   Verify the Chart and Tape Linked Context professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_chart_tape_linked_context(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "chart_tape.linked-context") == 0);
