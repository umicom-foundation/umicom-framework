/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_research_scanner_momentum.c
 *
 * PURPOSE:
 *   Verify the Scanner Momentum professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_research_scanner_momentum(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "research.scanner-momentum") == 0);
