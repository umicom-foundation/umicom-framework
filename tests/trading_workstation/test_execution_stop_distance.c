/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_execution_stop_distance.c
 *
 * PURPOSE:
 *   Verify the Stop Distance professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_execution_stop_distance(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "execution.stop-distance") == 0);
