/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_market_quote_midprice.c
 *
 * PURPOSE:
 *   Verify the Quote Midprice professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_market_quote_midprice(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "market.quote-midprice") == 0);
    assert(snapshot.revision == 55U);
    assert(snapshot.score >= 0.0 && snapshot.score <= 100.0);
    assert(snapshot.blocked == 0);
    assert(fabs(snapshot.value - 100.0) < 0.0001);
    return 0;
}
