/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_research_alert_density.c
 *
 * PURPOSE:
 *   Verify the Alert Density professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_research_alert_density(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "research.alert-density") == 0);
