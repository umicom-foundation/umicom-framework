/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_watchlist_context.c
 *
 * PURPOSE:
 *   Verify the Watchlist Linked Context professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_watchlist_context(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "watchlist.context") == 0);
