/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_market_quote_change_percent.c
 *
 * PURPOSE:
 *   Verify the Quote Change Percent professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_market_quote_change_percent(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "market.quote-change-percent") == 0);
    assert(snapshot.revision == 55U);
    assert(snapshot.score >= 0.0 && snapshot.score <= 100.0);
    assert(snapshot.blocked == 0);
    assert(snapshot.value > 2.03 && snapshot.value < 2.05);
    return 0;
}
