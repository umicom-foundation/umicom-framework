/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_depth_readiness.c
 *
 * PURPOSE:
 *   Verify the Depth Readiness professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_depth_readiness(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "depth.readiness") == 0);
