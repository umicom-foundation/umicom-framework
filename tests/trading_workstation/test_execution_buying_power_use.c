/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_execution_buying_power_use.c
 *
 * PURPOSE:
 *   Verify the Buying Power Utilisation professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_execution_buying_power_use(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "execution.buying-power-use") == 0);
