/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_chart_tape_momentum.c
 *
 * PURPOSE:
 *   Verify the Chart Momentum professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_chart_tape_momentum(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "chart_tape.momentum") == 0);
