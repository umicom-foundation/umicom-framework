/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_research_calendar_proximity.c
 *
 * PURPOSE:
 *   Verify the Economic Event Proximity professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_research_calendar_proximity(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "research.calendar-proximity") == 0);
