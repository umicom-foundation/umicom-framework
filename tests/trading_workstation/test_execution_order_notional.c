/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_execution_order_notional.c
 *
 * PURPOSE:
 *   Verify the Order Notional professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_execution_order_notional(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "execution.order-notional") == 0);
