/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_depth_ladder_density.c
 *
 * PURPOSE:
 *   Verify the Depth Ladder Density professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_depth_ladder_density(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "depth.ladder-density") == 0);
