/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_watchlist_readiness.c
 *
 * PURPOSE:
 *   Verify the Watchlist Readiness professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_watchlist_readiness(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "watchlist.readiness") == 0);
