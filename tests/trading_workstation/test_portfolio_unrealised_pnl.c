/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_portfolio_unrealised_pnl.c
 *
 * PURPOSE:
 *   Verify the Unrealised P&L professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_portfolio_unrealised_pnl(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "portfolio.unrealised-pnl") == 0);
    assert(snapshot.revision == 55U);
    assert(snapshot.score >= 0.0 && snapshot.score <= 100.0);
    assert(snapshot.blocked == 0);
    assert(fabs(snapshot.pnl - 250.0) < 0.01);
    return 0;
}
