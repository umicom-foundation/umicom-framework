/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_depth_bid_pressure.c
 *
 * PURPOSE:
 *   Verify the Bid Pressure professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_depth_bid_pressure(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "depth.bid-pressure") == 0);
