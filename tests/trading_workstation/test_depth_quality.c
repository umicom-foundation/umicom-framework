/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_depth_quality.c
 *
 * PURPOSE:
 *   Verify the Depth Quality professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_depth_quality(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "depth.quality") == 0);
