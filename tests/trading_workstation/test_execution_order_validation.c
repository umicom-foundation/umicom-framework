/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_execution_order_validation.c
 *
 * PURPOSE:
 *   Verify the Order Validation professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_execution_order_validation(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "execution.order-validation") == 0);
