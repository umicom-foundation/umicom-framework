/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_portfolio_risk_budget.c
 *
 * PURPOSE:
 *   Verify the Risk Budget professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_portfolio_risk_budget(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "portfolio.risk-budget") == 0);
