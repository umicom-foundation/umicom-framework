/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_portfolio_net_exposure.c
 *
 * PURPOSE:
 *   Verify the Net Exposure professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_portfolio_net_exposure(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "portfolio.net-exposure") == 0);
