/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_workstation_timeframe_sync.c
 *
 * PURPOSE:
 *   Verify the Timeframe Synchronisation professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_workstation_timeframe_sync(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "workstation.timeframe-sync") == 0);
