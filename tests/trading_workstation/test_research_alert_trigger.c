/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_research_alert_trigger.c
 *
 * PURPOSE:
 *   Verify the Price Alert Trigger professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_research_alert_trigger(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "research.alert-trigger") == 0);
