/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_market_session_range.c
 *
 * PURPOSE:
 *   Verify the Session Range professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_market_session_range(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "market.session-range") == 0);
