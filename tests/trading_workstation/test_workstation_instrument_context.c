/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_workstation_instrument_context.c
 *
 * PURPOSE:
 *   Verify the Instrument Context professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_workstation_instrument_context(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "workstation.instrument-context") == 0);
