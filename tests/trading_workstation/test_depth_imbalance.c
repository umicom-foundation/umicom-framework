/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_depth_imbalance.c
 *
 * PURPOSE:
 *   Verify the Depth Imbalance professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_depth_imbalance(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "depth.imbalance") == 0);
    assert(snapshot.revision == 55U);
    assert(snapshot.score >= 0.0 && snapshot.score <= 100.0);
    assert(snapshot.blocked == 0);
    assert(fabs(snapshot.value - 0.04) < 0.0001);
    return 0;
}
