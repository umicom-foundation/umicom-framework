/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_market_spread_basis_points.c
 *
 * PURPOSE:
 *   Verify the Spread Basis Points professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_market_spread_basis_points(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "market.spread-basis-points") == 0);
    assert(snapshot.revision == 55U);
    assert(snapshot.score >= 0.0 && snapshot.score <= 100.0);
    assert(snapshot.blocked == 0);
    assert(snapshot.value > 19.9 && snapshot.value < 20.1);
    return 0;
}
