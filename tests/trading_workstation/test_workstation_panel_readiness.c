/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_workstation_panel_readiness.c
 *
 * PURPOSE:
 *   Verify the Panel Readiness professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_workstation_panel_readiness(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "workstation.panel-readiness") == 0);
