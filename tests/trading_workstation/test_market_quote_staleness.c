/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_market_quote_staleness.c
 *
 * PURPOSE:
 *   Verify the Quote Staleness professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_market_quote_staleness(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "market.quote-staleness") == 0);
