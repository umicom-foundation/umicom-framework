/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_workstation_link_group_health.c
 *
 * PURPOSE:
 *   Verify the Link Group Health professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_workstation_link_group_health(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "workstation.link-group-health") == 0);
