/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_research_scanner_volume.c
 *
 * PURPOSE:
 *   Verify the Scanner Volume Signal professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_research_scanner_volume(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "research.scanner-volume") == 0);
