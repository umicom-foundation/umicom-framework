/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_depth_ask_pressure.c
 *
 * PURPOSE:
 *   Verify the Ask Pressure professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_depth_ask_pressure(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "depth.ask-pressure") == 0);
