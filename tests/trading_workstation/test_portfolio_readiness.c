/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/trading_workstation/test_portfolio_readiness.c
 *
 * PURPOSE:
 *   Verify the Portfolio Readiness professional workstation calculation.
 *
...
int main(void)
{
    ...
    assert(umi_trading_professional_evaluate_portfolio_readiness(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "portfolio.readiness") == 0);
