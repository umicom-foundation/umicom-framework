/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_pricing_market_data.c
 *
 * PURPOSE:
 *   Verify Pricing Market Data Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_pricing_market_data(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "pricing.market-data") == 0);
