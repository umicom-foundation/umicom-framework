/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_dashboard_cash_health.c
 *
 * PURPOSE:
 *   Verify Cash Dashboard Health Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_dashboard_cash_health(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "dashboard.cash-health") == 0);
