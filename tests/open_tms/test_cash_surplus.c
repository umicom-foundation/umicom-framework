/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_cash_surplus.c
 *
 * PURPOSE:
 *   Verify Cash Surplus Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_cash_surplus(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "cash.surplus") == 0);
