/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_settlement_window.c
 *
 * PURPOSE:
 *   Verify Settlement Window Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_settlement_window(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "settlement.window") == 0);
