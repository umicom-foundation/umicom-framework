/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_pricing_accrual.c
 *
 * PURPOSE:
 *   Verify Accrued Amount Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_pricing_accrual(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "pricing.accrual") == 0);
