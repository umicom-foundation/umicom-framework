/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_risk_credit.c
 *
 * PURPOSE:
 *   Verify Credit Risk Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_risk_credit(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "risk.credit") == 0);
