/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_dashboard_settlement_health.c
 *
 * PURPOSE:
 *   Verify Settlement Dashboard Health Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_dashboard_settlement_health(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "dashboard.settlement-health") == 0);
