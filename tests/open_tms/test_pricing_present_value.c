/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_pricing_present_value.c
 *
 * PURPOSE:
 *   Verify Present Value Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_pricing_present_value(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "pricing.present-value") == 0);
