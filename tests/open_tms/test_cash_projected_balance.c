/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_cash_projected_balance.c
 *
 * PURPOSE:
 *   Verify Projected Cash Balance Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_cash_projected_balance(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "cash.projected-balance") == 0);
