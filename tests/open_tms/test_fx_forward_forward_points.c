/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_fx_forward_forward_points.c
 *
 * PURPOSE:
 *   Verify FX Forward Points Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_fx_forward_forward_points(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "fx_forward.forward-points") == 0);
    assert(snapshot.revision == 111U);
    assert(snapshot.score >= 0.0 && snapshot.score <= 100.0);
    assert(snapshot.blocked == 0);
    assert(snapshot.value > 0.012 && snapshot.value < 0.013);
    return 0;
}
