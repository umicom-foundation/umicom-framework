/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_liquidity_funding_capacity.c
 *
 * PURPOSE:
 *   Verify Funding Capacity Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_liquidity_funding_capacity(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "liquidity.funding-capacity") == 0);
