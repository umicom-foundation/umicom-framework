/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_liquidity_gap.c
 *
 * PURPOSE:
 *   Verify Liquidity Gap Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_liquidity_gap(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "liquidity.gap") == 0);
    assert(snapshot.revision == 111U);
    assert(snapshot.score >= 0.0 && snapshot.score <= 100.0);
    assert(snapshot.blocked == 0);
    assert(snapshot.value == 400.0);
    return 0;
}
