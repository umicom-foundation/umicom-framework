/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_position_trade_coverage.c
 *
 * PURPOSE:
 *   Verify Position Trade Coverage Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_position_trade_coverage(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "position.trade-coverage") == 0);
