/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_reconciliation_cash.c
 *
 * PURPOSE:
 *   Verify Cash Reconciliation Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_reconciliation_cash(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "reconciliation.cash") == 0);
