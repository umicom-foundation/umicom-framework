/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_liquidity_concentration.c
 *
 * PURPOSE:
 *   Verify Funding Concentration Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_liquidity_concentration(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "liquidity.concentration") == 0);
