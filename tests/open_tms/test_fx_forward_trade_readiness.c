/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_fx_forward_trade_readiness.c
 *
 * PURPOSE:
 *   Verify FX Forward Trade Readiness Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_fx_forward_trade_readiness(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "fx_forward.trade-readiness") == 0);
