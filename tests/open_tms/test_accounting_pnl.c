/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_accounting_pnl.c
 *
 * PURPOSE:
 *   Verify Treasury Accounting P&L Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_accounting_pnl(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "accounting.pnl") == 0);
