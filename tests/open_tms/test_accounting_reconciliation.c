/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_accounting_reconciliation.c
 *
 * PURPOSE:
 *   Verify Treasury Ledger Reconciliation Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_accounting_reconciliation(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "accounting.reconciliation") == 0);
