/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_risk_breach.c
 *
 * PURPOSE:
 *   Verify Risk Limit Breach Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_risk_breach(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "risk.breach") == 0);
