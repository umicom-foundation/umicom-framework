/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_risk_stress.c
 *
 * PURPOSE:
 *   Verify Treasury Stress Score Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_risk_stress(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "risk.stress") == 0);
