/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_risk_counterparty.c
 *
 * PURPOSE:
 *   Verify Counterparty Risk Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_risk_counterparty(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "risk.counterparty") == 0);
