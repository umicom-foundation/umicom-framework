/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_risk_limit_utilisation.c
 *
 * PURPOSE:
 *   Verify Risk Limit Utilisation Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_risk_limit_utilisation(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "risk.limit-utilisation") == 0);
