/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_position_currency_exposure.c
 *
 * PURPOSE:
 *   Verify Currency Exposure Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_position_currency_exposure(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "position.currency-exposure") == 0);
