/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_pricing_curve_coverage.c
 *
 * PURPOSE:
 *   Verify Curve Coverage Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_pricing_curve_coverage(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "pricing.curve-coverage") == 0);
