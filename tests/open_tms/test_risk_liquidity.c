/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_risk_liquidity.c
 *
 * PURPOSE:
 *   Verify Liquidity Risk Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_risk_liquidity(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "risk.liquidity") == 0);
