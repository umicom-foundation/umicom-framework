/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_settlement_failure.c
 *
 * PURPOSE:
 *   Verify Settlement Failure Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_settlement_failure(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "settlement.failure") == 0);
