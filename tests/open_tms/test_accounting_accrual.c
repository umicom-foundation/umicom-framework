/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_accounting_accrual.c
 *
 * PURPOSE:
 *   Verify Treasury Accrual Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_accounting_accrual(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "accounting.accrual") == 0);
