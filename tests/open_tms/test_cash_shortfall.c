/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_cash_shortfall.c
 *
 * PURPOSE:
 *   Verify Cash Shortfall Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_cash_shortfall(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "cash.shortfall") == 0);
