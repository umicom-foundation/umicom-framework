/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_position_mtm.c
 *
 * PURPOSE:
 *   Verify Position Mark-to-Market Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_position_mtm(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "position.mtm") == 0);
