/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_accounting_event.c
 *
 * PURPOSE:
 *   Verify Treasury Accounting Event Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_accounting_event(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "accounting.event") == 0);
