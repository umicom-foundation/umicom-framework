/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_dashboard_liquidity_health.c
 *
 * PURPOSE:
 *   Verify Liquidity Dashboard Health Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_dashboard_liquidity_health(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "dashboard.liquidity-health") == 0);
