/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_liquidity_bucket_coverage.c
 *
 * PURPOSE:
 *   Verify Liquidity Bucket Coverage Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_liquidity_bucket_coverage(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "liquidity.bucket-coverage") == 0);
