/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_position_aggregation.c
 *
 * PURPOSE:
 *   Verify Position Aggregation Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_position_aggregation(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "position.aggregation") == 0);
