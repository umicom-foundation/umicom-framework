/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_fx_forward_forward_rate.c
 *
 * PURPOSE:
 *   Verify FX Forward Rate Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_fx_forward_forward_rate(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "fx_forward.forward-rate") == 0);
    assert(snapshot.revision == 111U);
    assert(snapshot.score >= 0.0 && snapshot.score <= 100.0);
    assert(snapshot.blocked == 0);
    assert(snapshot.value > 1.26 && snapshot.value < 1.27);
    return 0;
}
