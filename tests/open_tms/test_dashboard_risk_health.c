/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_dashboard_risk_health.c
 *
 * PURPOSE:
 *   Verify Risk Dashboard Health Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_dashboard_risk_health(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "dashboard.risk-health") == 0);
