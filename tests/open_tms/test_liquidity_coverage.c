/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_liquidity_coverage.c
 *
 * PURPOSE:
 *   Verify Liquidity Coverage Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_liquidity_coverage(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "liquidity.coverage") == 0);
