/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_fx_forward_lifecycle.c
 *
 * PURPOSE:
 *   Verify FX Forward Lifecycle Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_fx_forward_lifecycle(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "fx_forward.lifecycle") == 0);
