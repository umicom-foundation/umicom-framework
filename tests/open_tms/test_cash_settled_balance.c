/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_cash_settled_balance.c
 *
 * PURPOSE:
 *   Verify Settled Cash Balance Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_cash_settled_balance(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "cash.settled-balance") == 0);
