/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_cash_minimum_balance.c
 *
 * PURPOSE:
 *   Verify Minimum Cash Buffer Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_cash_minimum_balance(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "cash.minimum-balance") == 0);
