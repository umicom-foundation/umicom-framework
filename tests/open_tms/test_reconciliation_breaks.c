/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_reconciliation_breaks.c
 *
 * PURPOSE:
 *   Verify Treasury Reconciliation Breaks Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_reconciliation_breaks(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "reconciliation.breaks") == 0);
