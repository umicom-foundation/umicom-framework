/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_dashboard_operations_health.c
 *
 * PURPOSE:
 *   Verify Treasury Operations Health Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_dashboard_operations_health(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "dashboard.operations-health") == 0);
