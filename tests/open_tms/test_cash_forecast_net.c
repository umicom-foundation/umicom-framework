/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_cash_forecast_net.c
 *
 * PURPOSE:
 *   Verify Cash Forecast Net Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_cash_forecast_net(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "cash.forecast-net") == 0);
    assert(snapshot.revision == 111U);
    assert(snapshot.score >= 0.0 && snapshot.score <= 100.0);
    assert(snapshot.blocked == 0);
    assert(snapshot.value == 200.0);
    return 0;
}
