/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_accounting_posting.c
 *
 * PURPOSE:
 *   Verify Treasury Posting Readiness Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_accounting_posting(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "accounting.posting") == 0);
