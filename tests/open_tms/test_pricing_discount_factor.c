/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_pricing_discount_factor.c
 *
 * PURPOSE:
 *   Verify Discount Factor Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_pricing_discount_factor(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "pricing.discount-factor") == 0);
