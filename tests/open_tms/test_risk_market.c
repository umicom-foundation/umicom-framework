/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_risk_market.c
 *
 * PURPOSE:
 *   Verify Market Risk Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_risk_market(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "risk.market") == 0);
