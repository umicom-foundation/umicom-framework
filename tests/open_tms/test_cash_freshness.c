/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_cash_freshness.c
 *
 * PURPOSE:
 *   Verify Cash Data Freshness Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_cash_freshness(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "cash.freshness") == 0);
