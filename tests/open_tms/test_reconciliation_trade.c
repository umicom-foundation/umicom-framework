/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_reconciliation_trade.c
 *
 * PURPOSE:
 *   Verify Trade Reconciliation Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_reconciliation_trade(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "reconciliation.trade") == 0);
