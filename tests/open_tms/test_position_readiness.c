/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_position_readiness.c
 *
 * PURPOSE:
 *   Verify Position Readiness Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_position_readiness(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "position.readiness") == 0);
