/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/open_tms/test_settlement_confirmation.c
 *
 * PURPOSE:
 *   Verify Confirmation Matching Open TMS behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_open_tms_evaluate_settlement_confirmation(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "settlement.confirmation") == 0);
