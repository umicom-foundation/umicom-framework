/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/toolchain_binding_acceptance/test_markdown_default.c
 *
 * PURPOSE:
 *   Verify the existing markdown.default developer toolchain binding remains registered and reports its current readiness accurately.
 *
...
int test_toolchain_binding_acceptance_markdown_default(void)
{
    ...
    assert(strcmp(binding.language_id, "markdown") == 0);
    ...
    assert(strcmp(readiness.binding_id, "markdown.default") == 0);
