...
int main(void){UmiNativeIrFunction f;UmiNativeIrType t=umi_nc_ir_type_make(UMI_NC_IR_I64,64U,1U);if(umi_nc_ir_function_init(&f,"f",t)!=UMI_STATUS_OK)return 1;uint32_t id=0U;if(umi_nc_ir_function_add_block(&f,"e",&id)!=UMI_STATUS_OK)return 2;UmiNativeIrBuilder b;if(umi_nc_ir_builder_begin(&b,&f,id)!=UMI_STATUS_OK)return 3;uint32_t v=0U;if(umi_nc_ir_builder_const_i64(&b,99,&v)!=UMI_STATUS_OK||umi_nc_ir_builder_return(&b,0U)!=UMI_STATUS_OK)return 4;size_t n=0U;if(umi_nc_dead_code_eliminate(&f,&n)!=UMI_STATUS_OK||n!=1U||f.blocks[0].instruction_count!=1U)return 5;return 0;}
