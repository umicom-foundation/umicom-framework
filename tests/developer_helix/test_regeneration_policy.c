 ...
 */
int main(void){ UmiHelixRegenerationPolicy r; umi_helix_regeneration_policy_init(&r,"regeneration_policy"); /* Keep the operation inside its valid bounds before reading, writing or adding data. */ if(umi_helix_regeneration_policy_validate(&r)!=UMI_STATUS_OK){ return 1; } r.threshold=0.8; r.blocking=true; double s=umi_helix_regeneration_policy_score(&r); /* Keep the operation inside its valid bounds before reading, writing or adding data. */ if(s<=0.0 || s>1.0){ return 2; } return 0; }
