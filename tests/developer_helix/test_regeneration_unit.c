 ...
 */
int main(void){ UmiHelixRegenerationUnit r; umi_helix_regeneration_unit_init(&r,"regeneration_unit"); /* Keep the operation inside its valid bounds before reading, writing or adding data. */ if(umi_helix_regeneration_unit_validate(&r)!=UMI_STATUS_OK){ return 1; } r.progress=0.75; double s=umi_helix_regeneration_unit_score(&r); /* Keep the operation inside its valid bounds before reading, writing or adding data. */ if(s<=0.0 || s>1.0){ return 2; } return 0; }
