/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_vcs_repository.c
 *
 * PURPOSE:
 *   Verify the VCS Repository operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateVcsRepository(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "vcs.repository") == 0);
