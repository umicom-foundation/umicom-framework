/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_terminal_transcript.c
 *
 * PURPOSE:
 *   Verify the Terminal Transcript operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateTerminalTranscript(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "terminal.transcript") == 0);
