/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_debug_snapshot.c
 *
 * PURPOSE:
 *   Verify the Debug Snapshot operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateDebugSnapshot(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "debug.snapshot") == 0);
