/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_coverage_filter.c
 *
 * PURPOSE:
 *   Verify the Coverage Filter operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateCoverageFilter(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "coverage.filter") == 0);
