/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_coverage_test_failure.c
 *
 * PURPOSE:
 *   Verify the Test Failure operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateCoverageTestFailure(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "coverage.test-failure") == 0);
