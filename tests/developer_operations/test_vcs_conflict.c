/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_vcs_conflict.c
 *
 * PURPOSE:
 *   Verify the VCS Conflict operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateVcsConflict(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "vcs.conflict") == 0);
