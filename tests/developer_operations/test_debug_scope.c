/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_debug_scope.c
 *
 * PURPOSE:
 *   Verify the Debug Scope operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateDebugScope(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "debug.scope") == 0);
