/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_debug_source.c
 *
 * PURPOSE:
 *   Verify the Debug Source operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateDebugSource(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "debug.source") == 0);
