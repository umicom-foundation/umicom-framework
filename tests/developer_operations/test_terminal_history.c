/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_terminal_history.c
 *
 * PURPOSE:
 *   Verify the Terminal History operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateTerminalHistory(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "terminal.history") == 0);
