/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_debug_process.c
 *
 * PURPOSE:
 *   Verify the Debug Process operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateDebugProcess(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "debug.process") == 0);
