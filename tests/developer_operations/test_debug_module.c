/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_debug_module.c
 *
 * PURPOSE:
 *   Verify the Debug Module operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateDebugModule(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "debug.module") == 0);
