/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_debug_disassembly.c
 *
 * PURPOSE:
 *   Verify the Debug Disassembly operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateDebugDisassembly(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "debug.disassembly") == 0);
