/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_debug_breakpoint.c
 *
 * PURPOSE:
 *   Verify the Debug Breakpoint operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateDebugBreakpoint(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "debug.breakpoint") == 0);
