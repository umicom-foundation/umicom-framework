/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_debug_register.c
 *
 * PURPOSE:
 *   Verify the Debug Register operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateDebugRegister(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "debug.register") == 0);
