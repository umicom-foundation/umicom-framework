/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_debug_frame.c
 *
 * PURPOSE:
 *   Verify the Debug Frame operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateDebugFrame(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "debug.frame") == 0);
