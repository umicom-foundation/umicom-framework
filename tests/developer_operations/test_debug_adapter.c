/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_debug_adapter.c
 *
 * PURPOSE:
 *   Verify the Debug Adapter operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateDebugAdapter(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "debug.adapter") == 0);
