/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_debug_variable.c
 *
 * PURPOSE:
 *   Verify the Debug Variable operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateDebugVariable(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "debug.variable") == 0);
