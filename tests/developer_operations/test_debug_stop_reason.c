/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_debug_stop_reason.c
 *
 * PURPOSE:
 *   Verify the Debug Stop Reason operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateDebugStopReason(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "debug.stop-reason") == 0);
