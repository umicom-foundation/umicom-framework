/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_coverage_navigation.c
 *
 * PURPOSE:
 *   Verify the Coverage Navigation operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateCoverageNavigation(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "coverage.navigation") == 0);
