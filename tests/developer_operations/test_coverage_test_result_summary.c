/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_coverage_test_result_summary.c
 *
 * PURPOSE:
 *   Verify the Test Result Summary operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateCoverageTestResultSummary(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "coverage.test-result-summary") == 0);
