/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_debug_memory.c
 *
 * PURPOSE:
 *   Verify the Debug Memory operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateDebugMemory(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "debug.memory") == 0);
