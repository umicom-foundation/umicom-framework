/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_debug_call_stack.c
 *
 * PURPOSE:
 *   Verify the Debug Call Stack operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateDebugCallStack(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "debug.call-stack") == 0);
