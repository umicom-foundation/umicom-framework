/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_terminal_output_search.c
 *
 * PURPOSE:
 *   Verify the Output Search operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateTerminalOutputSearch(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "terminal.output-search") == 0);
