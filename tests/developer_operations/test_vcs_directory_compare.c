/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_vcs_directory_compare.c
 *
 * PURPOSE:
 *   Verify the VCS Directory Compare operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateVcsDirectoryCompare(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "vcs.directory-compare") == 0);
