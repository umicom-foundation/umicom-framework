/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_terminal_process.c
 *
 * PURPOSE:
 *   Verify the Terminal Process operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateTerminalProcess(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "terminal.process") == 0);
