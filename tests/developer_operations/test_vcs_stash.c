/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_vcs_stash.c
 *
 * PURPOSE:
 *   Verify the VCS Stash operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateVcsStash(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "vcs.stash") == 0);
