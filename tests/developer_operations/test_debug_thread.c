/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_debug_thread.c
 *
 * PURPOSE:
 *   Verify the Debug Thread operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateDebugThread(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "debug.thread") == 0);
