/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_debug_toolbar.c
 *
 * PURPOSE:
 *   Verify the Debug Toolbar operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateDebugToolbar(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "debug.toolbar") == 0);
