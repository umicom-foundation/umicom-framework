/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_debug_session.c
 *
 * PURPOSE:
 *   Verify the Debug Session operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateDebugSession(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "debug.session") == 0);
