/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_debug_navigation.c
 *
 * PURPOSE:
 *   Verify the Debug Navigation operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateDebugNavigation(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "debug.navigation") == 0);
