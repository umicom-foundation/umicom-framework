/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/developer_operations/test_vcs_blame.c
 *
 * PURPOSE:
 *   Verify the VCS Blame operational projection.
 *
...
int main(void)
{
    ...
    assert(UmiDeveloperOperationsEvaluateVcsBlame(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "vcs.blame") == 0);
