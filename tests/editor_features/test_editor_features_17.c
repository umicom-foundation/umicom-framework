/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/editor_features/test_editor_features_17.c
 *
 * PURPOSE:
...
int main(void)
{
    ...
    {
        const UmiEditorFeatureDescriptor *descriptor = umi_editor_feature_save_participants();
        if (descriptor == NULL ||
            descriptor->struct_size != sizeof(*descriptor) ||
            descriptor->api_version != UMI_EDITOR_FEATURE_API_VERSION ||
            descriptor->category != UMI_EDITOR_FEATURE_CATEGORY_WORKSPACE ||
    ...
    {
        const UmiEditorFeatureDescriptor *descriptor = umi_editor_feature_working_copy();
        if (descriptor == NULL ||
            descriptor->struct_size != sizeof(*descriptor) ||
            descriptor->api_version != UMI_EDITOR_FEATURE_API_VERSION ||
            descriptor->category != UMI_EDITOR_FEATURE_CATEGORY_WORKSPACE ||
    ...
    {
        const UmiEditorFeatureDescriptor *descriptor = umi_editor_feature_hot_exit();
        if (descriptor == NULL ||
            descriptor->struct_size != sizeof(*descriptor) ||
            descriptor->api_version != UMI_EDITOR_FEATURE_API_VERSION ||
            descriptor->category != UMI_EDITOR_FEATURE_CATEGORY_WORKSPACE ||
    ...
    {
        const UmiEditorFeatureDescriptor *descriptor = umi_editor_feature_language_binding();
        if (descriptor == NULL ||
            descriptor->struct_size != sizeof(*descriptor) ||
            descriptor->api_version != UMI_EDITOR_FEATURE_API_VERSION ||
            descriptor->category != UMI_EDITOR_FEATURE_CATEGORY_LANGUAGE ||
            descriptor->maturity != UMI_EDITOR_FEATURE_MATURITY_STABLE ||
            descriptor->feature_id[0] == '\0' ||
            descriptor->title[0] == '\0' ||
            descriptor->summary[0] == '\0') {
    ...
    {
        const UmiEditorFeatureDescriptor *descriptor = umi_editor_feature_completion_session();
        if (descriptor == NULL ||
            descriptor->struct_size != sizeof(*descriptor) ||
            descriptor->api_version != UMI_EDITOR_FEATURE_API_VERSION ||
            descriptor->category != UMI_EDITOR_FEATURE_CATEGORY_LANGUAGE ||
