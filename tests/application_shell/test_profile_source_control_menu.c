/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/application_shell/test_profile_source_control_menu.c
 *
 * PURPOSE:
 *   Verify the built-in Source Control Menu profile is valid and installable.
 *
...
int main(void)
{
    ...
    assert(profile != NULL);
    assert(profile->contribution_count == 8U);
    ...
    assert(umi_application_shell_registry_find(
        registry,
        "umicom.shell.source-control-menu.root",
