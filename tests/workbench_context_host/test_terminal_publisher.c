/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/workbench_context_host/test_terminal_publisher.c
 *
 * PURPOSE:
 *   Verify terminal publication uses selection context with process metadata.
 *
...
int main(void)
{
    ...
    assert(umi_workbench_context_host_publish_terminal(
        fixture.host, "blue", "test.source", "terminal-context",
        "terminal-1", "powershell", "C:/dev", "cmake --build .", 0, 100U) == UMI_STATUS_OK);
