/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/workbench_context_host/test_account_publisher.c
 *
 * PURPOSE:
 *   Verify canonical account publication.
 *
...
int main(void)
{
    ...
    assert(umi_workbench_context_host_publish_account(
        fixture.host, "red", "test.trading", "account-context",
        "account", "org", "book", "USD", "trading", "paper", 100U) == UMI_STATUS_OK);
    active = umi_workbench_context_link_service_current(
        fixture.host->link_service, "red");
    assert(active != NULL);
    assert(active->payload.kind == UMI_CONTEXT_KIND_ACCOUNT);
