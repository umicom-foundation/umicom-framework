/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/workbench_context_host/test_ai_publisher.c
 *
 * PURPOSE:
 *   Verify AI conversation publication preserves evidence metadata.
 *
...
int main(void)
{
    ...
    assert(umi_workbench_context_host_publish_ai(
        fixture.host, "blue", "test.source", "ai-context",
        "conversation", "message", "provider", "model", "evidence", 100U) == UMI_STATUS_OK);
