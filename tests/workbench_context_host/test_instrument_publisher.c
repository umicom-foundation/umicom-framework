/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/workbench_context_host/test_instrument_publisher.c
 *
 * PURPOSE:
 *   Verify canonical instrument publication.
 *
...
int main(void)
{
    ...
    assert(umi_workbench_context_host_publish_instrument(
        fixture.host, "red", "test.trading", "instrument-context",
        "instrument", "ES", "CME", "USD", "future", "ESU6", 100U) == UMI_STATUS_OK);
    active = umi_workbench_context_link_service_current(
        fixture.host->link_service, "red");
    assert(active != NULL);
    assert(active->payload.kind == UMI_CONTEXT_KIND_INSTRUMENT);
