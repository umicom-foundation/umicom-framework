/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/workbench_context_host/test_media_publisher.c
 *
 * PURPOSE:
 *   Verify canonical media publication.
 *
...
int main(void)
{
    ...
    assert(umi_workbench_context_host_publish_media(
        fixture.host, "blue", "test.source", "media-context",
        "asset", "timeline", "track", 1000U, 5000U, "video", 100U) == UMI_STATUS_OK);
    active = umi_workbench_context_link_service_current(
        fixture.host->link_service, "blue");
    assert(active != NULL);
    assert(active->payload.kind == UMI_CONTEXT_KIND_MEDIA);
