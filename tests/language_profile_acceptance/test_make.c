/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/language_profile_acceptance/test_make.c
 *
 * PURPOSE:
 *   Verify the existing make language profile remains registered with its established core capability.
 *
...
int test_language_profile_acceptance_make(void)
{
    ...
    assert(strcmp(profile.id, "make") == 0);
