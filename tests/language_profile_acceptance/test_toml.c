/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/language_profile_acceptance/test_toml.c
 *
 * PURPOSE:
 *   Verify the existing toml language profile remains registered with its established core capability.
 *
...
int test_language_profile_acceptance_toml(void)
{
    ...
    assert(strcmp(profile.id, "toml") == 0);
