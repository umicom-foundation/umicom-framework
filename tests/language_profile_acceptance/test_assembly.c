/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/language_profile_acceptance/test_assembly.c
 *
 * PURPOSE:
 *   Verify the existing assembly language profile remains registered with its established core capability.
 *
...
int test_language_profile_acceptance_assembly(void)
{
    ...
    assert(strcmp(profile.id, "assembly") == 0);
