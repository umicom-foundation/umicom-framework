/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/language_profile_acceptance/test_umicc.c
 *
 * PURPOSE:
 *   Verify the existing umicc language profile remains registered with its established core capability.
 *
...
int test_language_profile_acceptance_umicc(void)
{
    ...
    assert(strcmp(profile.id, "umicc") == 0);
