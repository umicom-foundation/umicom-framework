/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/workbench_context_source/test_deduplication.c
 *
 * PURPOSE:
 *   Verify immediate duplicate live samples are suppressed before Event Service.
 *
...
int main(void)
{
    ...
    definition.coalescing_window_ms = 50U;
    ...
    assert(runtime->duplicate_count == 1U);
