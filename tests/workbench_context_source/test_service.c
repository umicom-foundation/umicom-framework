/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/workbench_context_source/test_service.c
 *
 * PURPOSE:
 *   Verify a configured live source routes a sample through Event Service into the Context Host.
 *
...
int main(void)
{
    ...
    assert(umi_workbench_context_host_pop_delivery(
        fixture.host, "test.target.endpoint",
        &delivery) == UMI_STATUS_OK);
    assert(delivery.payload.kind ==
        UMI_CONTEXT_KIND_SOURCE_LOCATION);
    assert(sources->metrics.accepted_sample_count == 1U);
