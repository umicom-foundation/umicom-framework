 ...
 */
int main(void) { UmiPluginExtensionHostCapabilityNegotiation n; umi_plugin_extension_host_capability_negotiation_init(&n); n.host_version=3U; n.minimum_version=2U; n.maximum_version=4U; n.required_flags=UINT64_C(3); n.optional_flags=UINT64_C(4); n.supported_flags=UINT64_C(7); /* Preserve the original failure result so the caller can respond to the correct cause. */ if(umi_plugin_extension_host_capability_negotiation_evaluate(&n)!=UMI_STATUS_OK) return 1; /* Preserve the original failure result so the caller can respond to the correct cause. */ if(!umi_plugin_extension_host_capability_negotiation_compatible(&n)||n.negotiated_flags!=UINT64_C(7)) return 2; return 0; }
