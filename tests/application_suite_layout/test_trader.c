...
int main(void) {
    ...
    assert(umi_application_suite_layout_runtime_load(&r,"org.umicom.trader")==UMI_STATUS_OK);
    s=umi_application_suite_layout_runtime_snapshot(&r);
    assert(s.loaded); assert(s.window_count==8U); assert(strstr(s.active_layout_id,"trading")!=NULL);
    assert(umi_application_suite_layout_runtime_select(&r,"research")==UMI_STATUS_OK);
