...
int main(void) {
    ...
    const UmiExperienceLayoutDefinition *review;

    /* Read expected counts from the canonical recipe so adding a Studio panel
     * updates the test automatically instead of creating a stale assertion. */
    ...
    review = umi_application_experience_layout_find(experience, "review");
    UMI_TEST_REQUIRE(review != NULL);
    ...
    UMI_TEST_REQUIRE(umi_application_suite_layout_runtime_load(
                         r, "org.umicom.studio") == UMI_STATUS_OK);
    ...
    UMI_TEST_REQUIRE(umi_application_suite_layout_runtime_active(
                         r)->window_count == review->panel_count);
    free(r);
    return 0;
}
