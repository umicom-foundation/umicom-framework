/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/context_channel/integration_test_072.c
 *
 * PURPOSE:
...
int main(void)
{
    ...
    (void)umi_context_copy_text(context_id,sizeof(context_id),"integration-072");
    umi_context_payload_init(&payload,UMI_CONTEXT_KIND_SOURCE_LOCATION,context_id,"umicom.context/integration");
    umi_context_value_init(&value,"scenario");
    assert(umi_context_value_set_unsigned(&value,72U) == UMI_STATUS_OK);
    assert(umi_context_payload_add_value(&payload,&value) == UMI_STATUS_OK);
    assert(umi_context_colour_token_for(UMI_CONTEXT_COLOUR_RED,&token) == UMI_STATUS_OK);
