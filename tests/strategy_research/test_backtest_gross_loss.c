/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_backtest_gross_loss.c
 *
 * PURPOSE:
 *   Verify the Backtest Gross Loss strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_backtest_gross_loss(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "backtest.gross-loss") == 0);
