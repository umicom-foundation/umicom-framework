/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_study_bollinger_position.c
 *
 * PURPOSE:
 *   Verify the Bollinger Position strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_study_bollinger_position(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "study.bollinger-position") == 0);
