/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_study_rsi.c
 *
 * PURPOSE:
 *   Verify the Relative Strength Index strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_study_rsi(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "study.rsi") == 0);
