/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_performance_annualised_return.c
 *
 * PURPOSE:
 *   Verify the Annualised Return strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_performance_annualised_return(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "performance.annualised-return") == 0);
