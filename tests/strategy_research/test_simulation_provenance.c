/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_simulation_provenance.c
 *
 * PURPOSE:
 *   Verify the Simulation Provenance strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_simulation_provenance(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "simulation.provenance") == 0);
