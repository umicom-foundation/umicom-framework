/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_replay_event_rate.c
 *
 * PURPOSE:
 *   Verify the Replay Event Rate strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_replay_event_rate(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "replay.event-rate") == 0);
