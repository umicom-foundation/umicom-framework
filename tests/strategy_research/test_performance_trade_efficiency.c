/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_performance_trade_efficiency.c
 *
 * PURPOSE:
 *   Verify the Trade Efficiency strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_performance_trade_efficiency(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "performance.trade-efficiency") == 0);
