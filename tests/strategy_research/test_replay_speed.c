/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_replay_speed.c
 *
 * PURPOSE:
 *   Verify the Replay Speed strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_replay_speed(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "replay.speed") == 0);
