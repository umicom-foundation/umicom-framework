/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_replay_checkpoint.c
 *
 * PURPOSE:
 *   Verify the Replay Checkpoint Readiness strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_replay_checkpoint(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "replay.checkpoint") == 0);
