/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_performance_sharpe.c
 *
 * PURPOSE:
 *   Verify the Sharpe Ratio strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_performance_sharpe(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "performance.sharpe") == 0);
