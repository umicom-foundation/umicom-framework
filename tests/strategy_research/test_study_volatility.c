/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_study_volatility.c
 *
 * PURPOSE:
 *   Verify the Rolling Volatility strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_study_volatility(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "study.volatility") == 0);
