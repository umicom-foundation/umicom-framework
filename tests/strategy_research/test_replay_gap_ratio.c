/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_replay_gap_ratio.c
 *
 * PURPOSE:
 *   Verify the Replay Gap Ratio strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_replay_gap_ratio(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "replay.gap-ratio") == 0);
    assert(snapshot.revision == 88U);
    assert(snapshot.score >= 0.0 && snapshot.score <= 100.0);
    assert(snapshot.blocked == 0);
    assert(snapshot.ratio > 1.3 && snapshot.ratio < 1.4);
    return 0;
}
