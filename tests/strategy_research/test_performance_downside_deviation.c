/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_performance_downside_deviation.c
 *
 * PURPOSE:
 *   Verify the Downside Deviation strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_performance_downside_deviation(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "performance.downside-deviation") == 0);
