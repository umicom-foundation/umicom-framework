/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_optimisation_pareto.c
 *
 * PURPOSE:
 *   Verify the Pareto Fitness strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_optimisation_pareto(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "optimisation.pareto") == 0);
