/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_study_vwap.c
 *
 * PURPOSE:
 *   Verify the Volume Weighted Average Price strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_study_vwap(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "study.vwap") == 0);
