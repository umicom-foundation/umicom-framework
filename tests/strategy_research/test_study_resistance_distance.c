/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_study_resistance_distance.c
 *
 * PURPOSE:
 *   Verify the Resistance Distance strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_study_resistance_distance(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "study.resistance-distance") == 0);
