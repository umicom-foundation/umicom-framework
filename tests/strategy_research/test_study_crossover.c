/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_study_crossover.c
 *
 * PURPOSE:
 *   Verify the Moving Average Crossover strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_study_crossover(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "study.crossover") == 0);
