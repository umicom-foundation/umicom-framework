/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_strategy_signal_score.c
 *
 * PURPOSE:
 *   Verify the Strategy Signal Score strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_strategy_signal_score(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "strategy.signal-score") == 0);
