/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_study_ema.c
 *
 * PURPOSE:
 *   Verify the Exponential Moving Average strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_study_ema(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "study.ema") == 0);
