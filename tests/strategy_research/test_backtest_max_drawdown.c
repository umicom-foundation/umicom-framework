/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_backtest_max_drawdown.c
 *
 * PURPOSE:
 *   Verify the Backtest Maximum Drawdown strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_backtest_max_drawdown(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "backtest.max-drawdown") == 0);
