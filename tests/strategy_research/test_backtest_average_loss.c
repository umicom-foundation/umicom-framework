/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_backtest_average_loss.c
 *
 * PURPOSE:
 *   Verify the Backtest Average Loss strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_backtest_average_loss(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "backtest.average-loss") == 0);
