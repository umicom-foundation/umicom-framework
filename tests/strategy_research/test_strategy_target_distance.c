/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_strategy_target_distance.c
 *
 * PURPOSE:
 *   Verify the Strategy Target Distance strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_strategy_target_distance(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "strategy.target-distance") == 0);
