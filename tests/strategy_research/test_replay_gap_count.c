/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_replay_gap_count.c
 *
 * PURPOSE:
 *   Verify the Replay Gap Count strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_replay_gap_count(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "replay.gap-count") == 0);
