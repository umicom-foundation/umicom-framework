/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_simulation_fill_ratio.c
 *
 * PURPOSE:
 *   Verify the Simulation Fill Ratio strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_simulation_fill_ratio(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "simulation.fill-ratio") == 0);
