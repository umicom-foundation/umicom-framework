/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_performance_sortino.c
 *
 * PURPOSE:
 *   Verify the Sortino Ratio strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_performance_sortino(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "performance.sortino") == 0);
