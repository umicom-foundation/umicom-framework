/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_replay_progress.c
 *
 * PURPOSE:
 *   Verify the Replay Progress strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_replay_progress(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "replay.progress") == 0);
    assert(snapshot.revision == 88U);
    assert(snapshot.score >= 0.0 && snapshot.score <= 100.0);
    assert(snapshot.blocked == 0);
    assert(snapshot.ratio > 74.9 && snapshot.ratio < 75.1);
    return 0;
}
