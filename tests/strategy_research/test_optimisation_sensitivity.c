/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_optimisation_sensitivity.c
 *
 * PURPOSE:
 *   Verify the Parameter Sensitivity strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_optimisation_sensitivity(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "optimisation.sensitivity") == 0);
