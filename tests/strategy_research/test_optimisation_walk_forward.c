/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_optimisation_walk_forward.c
 *
 * PURPOSE:
 *   Verify the Walk Forward Score strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_optimisation_walk_forward(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "optimisation.walk-forward") == 0);
