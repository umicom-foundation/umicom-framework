/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_study_trend_strength.c
 *
 * PURPOSE:
 *   Verify the Trend Strength strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_study_trend_strength(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "study.trend-strength") == 0);
