/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_performance_annualised_volatility.c
 *
 * PURPOSE:
 *   Verify the Annualised Volatility strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_performance_annualised_volatility(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "performance.annualised-volatility") == 0);
