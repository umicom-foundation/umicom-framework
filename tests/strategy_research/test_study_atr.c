/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_study_atr.c
 *
 * PURPOSE:
 *   Verify the Average True Range strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_study_atr(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "study.atr") == 0);
