/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_replay_seek_distance.c
 *
 * PURPOSE:
 *   Verify the Replay Seek Distance strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_replay_seek_distance(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "replay.seek-distance") == 0);
