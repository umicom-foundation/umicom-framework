/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_simulation_position_change.c
 *
 * PURPOSE:
 *   Verify the Simulation Position Change strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_simulation_position_change(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "simulation.position-change") == 0);
