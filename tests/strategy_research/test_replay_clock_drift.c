/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_replay_clock_drift.c
 *
 * PURPOSE:
 *   Verify the Replay Clock Drift strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_replay_clock_drift(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "replay.clock-drift") == 0);
