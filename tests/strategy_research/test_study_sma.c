/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_study_sma.c
 *
 * PURPOSE:
 *   Verify the Simple Moving Average strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_study_sma(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "study.sma") == 0);
