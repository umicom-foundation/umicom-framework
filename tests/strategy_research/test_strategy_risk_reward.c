/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_strategy_risk_reward.c
 *
 * PURPOSE:
 *   Verify the Strategy Risk Reward strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_strategy_risk_reward(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "strategy.risk-reward") == 0);
    assert(snapshot.revision == 88U);
    assert(snapshot.score >= 0.0 && snapshot.score <= 100.0);
    assert(snapshot.blocked == 0);
    assert(snapshot.ratio > 2.99 && snapshot.ratio < 3.01);
    return 0;
}
