/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_strategy_entry_gate.c
 *
 * PURPOSE:
 *   Verify the Strategy Entry Gate strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_strategy_entry_gate(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "strategy.entry-gate") == 0);
