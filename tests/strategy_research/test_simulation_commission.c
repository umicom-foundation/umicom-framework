/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_simulation_commission.c
 *
 * PURPOSE:
 *   Verify the Simulation Commission strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_simulation_commission(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "simulation.commission") == 0);
