/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_performance_calmar.c
 *
 * PURPOSE:
 *   Verify the Calmar Ratio strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_performance_calmar(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "performance.calmar") == 0);
