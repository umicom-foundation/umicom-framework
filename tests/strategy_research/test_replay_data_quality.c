/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_replay_data_quality.c
 *
 * PURPOSE:
 *   Verify the Replay Data Quality strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_replay_data_quality(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "replay.data-quality") == 0);
