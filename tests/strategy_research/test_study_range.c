/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_study_range.c
 *
 * PURPOSE:
 *   Verify the Price Range strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_study_range(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "study.range") == 0);
