/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_study_rate_of_change.c
 *
 * PURPOSE:
 *   Verify the Rate of Change strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_study_rate_of_change(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "study.rate-of-change") == 0);
    assert(snapshot.revision == 88U);
    assert(snapshot.score >= 0.0 && snapshot.score <= 100.0);
    assert(snapshot.blocked == 0);
    assert(snapshot.value > 8.3 && snapshot.value < 8.4);
    return 0;
}
