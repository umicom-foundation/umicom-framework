/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_optimisation_budget.c
 *
 * PURPOSE:
 *   Verify the Optimisation Budget strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_optimisation_budget(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "optimisation.budget") == 0);
