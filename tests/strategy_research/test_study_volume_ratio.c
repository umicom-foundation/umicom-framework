/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_study_volume_ratio.c
 *
 * PURPOSE:
 *   Verify the Volume Ratio strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_study_volume_ratio(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "study.volume-ratio") == 0);
