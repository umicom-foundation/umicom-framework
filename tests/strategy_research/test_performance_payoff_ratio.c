/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_performance_payoff_ratio.c
 *
 * PURPOSE:
 *   Verify the Payoff Ratio strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_performance_payoff_ratio(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "performance.payoff-ratio") == 0);
