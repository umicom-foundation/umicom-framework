/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_study_support_distance.c
 *
 * PURPOSE:
 *   Verify the Support Distance strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_study_support_distance(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "study.support-distance") == 0);
