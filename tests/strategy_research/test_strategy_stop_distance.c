/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_strategy_stop_distance.c
 *
 * PURPOSE:
 *   Verify the Strategy Stop Distance strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_strategy_stop_distance(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "strategy.stop-distance") == 0);
