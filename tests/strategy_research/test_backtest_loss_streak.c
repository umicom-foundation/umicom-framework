/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_backtest_loss_streak.c
 *
 * PURPOSE:
 *   Verify the Backtest Loss Streak strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_backtest_loss_streak(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "backtest.loss-streak") == 0);
