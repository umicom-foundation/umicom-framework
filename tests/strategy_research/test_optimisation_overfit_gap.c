/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_optimisation_overfit_gap.c
 *
 * PURPOSE:
 *   Verify the Optimisation Overfit Gap strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_optimisation_overfit_gap(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "optimisation.overfit-gap") == 0);
    assert(snapshot.revision == 88U);
    assert(snapshot.score >= 0.0 && snapshot.score <= 100.0);
    assert(snapshot.blocked == 0);
    assert(fabs(snapshot.value - 6.0) < 0.001);
    return 0;
}
