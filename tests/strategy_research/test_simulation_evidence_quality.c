/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_simulation_evidence_quality.c
 *
 * PURPOSE:
 *   Verify the Simulation Evidence Quality strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_simulation_evidence_quality(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "simulation.evidence-quality") == 0);
