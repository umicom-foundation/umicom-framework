/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_performance_ulcer_index.c
 *
 * PURPOSE:
 *   Verify the Ulcer Index strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_performance_ulcer_index(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "performance.ulcer-index") == 0);
