/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/strategy_research/test_backtest_exposure.c
 *
 * PURPOSE:
 *   Verify the Backtest Exposure strategy research calculation.
 *
...
int main(void)
{
    ...
    assert(umi_strategy_research_evaluate_backtest_exposure(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "backtest.exposure") == 0);
