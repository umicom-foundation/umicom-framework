/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/language_support_acceptance/test_generic_c.c
 *
 * PURPOSE:
 *   Verify the existing language/toolchain support matrix generic c behavior.
 *
...
int test_language_support_generic_c(void)
{
    ...
    assert(row!=NULL);
    assert(strcmp(row->binding_id,"c.clang")==0);
