/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/language_support_acceptance/test_typescript.c
 *
 * PURPOSE:
 *   Verify the existing language/toolchain support matrix typescript behavior.
 *
...
int test_language_support_typescript(void)
{
    ...
    row=umi_developer_language_support_matrix_find(&matrix,"typescript");
    assert(row!=NULL&&row->binding_found);
    assert(strcmp(row->binding_id,"typescript.node")==0);
