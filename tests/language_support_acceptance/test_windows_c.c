/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/language_support_acceptance/test_windows_c.c
 *
 * PURPOSE:
 *   Verify the existing language/toolchain support matrix windows c behavior.
 *
...
int test_language_support_windows_c(void)
{
    ...
    assert(umi_developer_language_support_matrix_build(
        profiles,bindings,"windows","x86_64",available,NULL,&matrix)==UMI_STATUS_OK);
    row=umi_developer_language_support_matrix_find(&matrix,"c");
    assert(row!=NULL);
    assert(strcmp(row->binding_id,"c.msvc")==0);
