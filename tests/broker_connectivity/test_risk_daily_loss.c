/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_risk_daily_loss.c
 *
 * PURPOSE:
 *   Verify Daily Loss Boundary broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_risk_daily_loss(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "risk.daily-loss") == 0);
