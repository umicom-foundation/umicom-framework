/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_risk_price_band.c
 *
 * PURPOSE:
 *   Verify Price Band Gate broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_risk_price_band(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "risk.price-band") == 0);
