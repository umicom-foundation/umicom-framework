/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_risk_leverage.c
 *
 * PURPOSE:
 *   Verify Leverage Gate broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_risk_leverage(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "risk.leverage") == 0);
