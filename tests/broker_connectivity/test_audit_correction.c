/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_audit_correction.c
 *
 * PURPOSE:
 *   Verify Audit Correction Evidence broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_audit_correction(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "audit.correction") == 0);
