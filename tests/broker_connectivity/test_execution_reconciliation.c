/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_execution_reconciliation.c
 *
 * PURPOSE:
 *   Verify Execution Reconciliation broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_execution_reconciliation(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "execution.reconciliation") == 0);
