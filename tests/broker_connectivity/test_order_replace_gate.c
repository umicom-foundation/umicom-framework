/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_order_replace_gate.c
 *
 * PURPOSE:
 *   Verify Order Replace Gate broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_order_replace_gate(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "order.replace-gate") == 0);
