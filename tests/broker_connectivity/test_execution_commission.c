/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_execution_commission.c
 *
 * PURPOSE:
 *   Verify Execution Commission broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_execution_commission(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "execution.commission") == 0);
