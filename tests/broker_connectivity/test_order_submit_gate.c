/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_order_submit_gate.c
 *
 * PURPOSE:
 *   Verify Order Submit Gate broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_order_submit_gate(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "order.submit-gate") == 0);
