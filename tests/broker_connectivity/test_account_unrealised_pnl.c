/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_account_unrealised_pnl.c
 *
 * PURPOSE:
 *   Verify Unrealised P&L broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_account_unrealised_pnl(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "account.unrealised-pnl") == 0);
