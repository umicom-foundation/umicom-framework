/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_provider_live_mode.c
 *
 * PURPOSE:
 *   Verify Live Trading Mode broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_provider_live_mode(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "provider.live-mode") == 0);
