/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_execution_audit.c
 *
 * PURPOSE:
 *   Verify Execution Audit Coverage broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_execution_audit(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "execution.audit") == 0);
