/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_execution_fill_progress.c
 *
 * PURPOSE:
 *   Verify Execution Fill Progress broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_execution_fill_progress(&input, &snapshot) == UMI_STATUS_OK);
    ...
    assert(snapshot.ratio > 99.9 && snapshot.ratio <= 100.0);
    return 0;
}
