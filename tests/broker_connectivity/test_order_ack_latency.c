/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_order_ack_latency.c
 *
 * PURPOSE:
 *   Verify Order Ack Latency broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_order_ack_latency(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "order.ack-latency") == 0);
