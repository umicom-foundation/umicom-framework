/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_order_state_sync.c
 *
 * PURPOSE:
 *   Verify Order State Synchronisation broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_order_state_sync(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "order.state-sync") == 0);
