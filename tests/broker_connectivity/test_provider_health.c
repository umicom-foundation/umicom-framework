/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_provider_health.c
 *
 * PURPOSE:
 *   Verify Provider Health broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_provider_health(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "provider.health") == 0);
