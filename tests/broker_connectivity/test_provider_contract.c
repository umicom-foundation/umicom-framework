/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_provider_contract.c
 *
 * PURPOSE:
 *   Verify Provider Contract broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_provider_contract(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "provider.contract") == 0);
