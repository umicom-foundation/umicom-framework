/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_execution_late_detection.c
 *
 * PURPOSE:
 *   Verify Late Execution Detection broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_execution_late_detection(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "execution.late-detection") == 0);
