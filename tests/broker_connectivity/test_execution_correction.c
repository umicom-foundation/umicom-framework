/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_execution_correction.c
 *
 * PURPOSE:
 *   Verify Execution Correction broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_execution_correction(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "execution.correction") == 0);
