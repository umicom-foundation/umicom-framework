/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_account_buying_power.c
 *
 * PURPOSE:
 *   Verify Buying Power broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_account_buying_power(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "account.buying-power") == 0);
