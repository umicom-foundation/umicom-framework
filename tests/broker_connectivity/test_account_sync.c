/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_account_sync.c
 *
 * PURPOSE:
 *   Verify Account Synchronisation broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_account_sync(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "account.sync") == 0);
