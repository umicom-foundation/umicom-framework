/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_provider_paper_mode.c
 *
 * PURPOSE:
 *   Verify Paper Trading Mode broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_provider_paper_mode(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "provider.paper-mode") == 0);
