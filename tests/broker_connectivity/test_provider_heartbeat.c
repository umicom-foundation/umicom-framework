/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_provider_heartbeat.c
 *
 * PURPOSE:
 *   Verify Provider Heartbeat broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_provider_heartbeat(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "provider.heartbeat") == 0);
