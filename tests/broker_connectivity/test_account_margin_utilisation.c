/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_account_margin_utilisation.c
 *
 * PURPOSE:
 *   Verify Margin Utilisation broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_account_margin_utilisation(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "account.margin-utilisation") == 0);
