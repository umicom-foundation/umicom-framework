/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_risk_buying_power.c
 *
 * PURPOSE:
 *   Verify Buying Power Gate broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_risk_buying_power(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "risk.buying-power") == 0);
