/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_ibkr_execution_mapping.c
 *
 * PURPOSE:
 *   Verify IBKR Execution Mapping broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_ibkr_execution_mapping(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ibkr.execution-mapping") == 0);
