/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_ibkr_account_mapping.c
 *
 * PURPOSE:
 *   Verify IBKR Account Mapping broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_ibkr_account_mapping(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ibkr.account-mapping") == 0);
