/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_provider_authentication.c
 *
 * PURPOSE:
 *   Verify Provider Authentication broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_provider_authentication(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "provider.authentication") == 0);
