/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_order_working_notional.c
 *
 * PURPOSE:
 *   Verify Working Order Notional broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_order_working_notional(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "order.working-notional") == 0);
