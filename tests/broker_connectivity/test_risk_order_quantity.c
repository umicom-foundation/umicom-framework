/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_risk_order_quantity.c
 *
 * PURPOSE:
 *   Verify Maximum Order Quantity broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_risk_order_quantity(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "risk.order-quantity") == 0);
