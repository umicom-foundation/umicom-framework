/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_risk_kill_switch.c
 *
 * PURPOSE:
 *   Verify Kill Switch Gate broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_risk_kill_switch(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "risk.kill-switch") == 0);
