/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_risk_order_notional.c
 *
 * PURPOSE:
 *   Verify Maximum Order Notional broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_risk_order_notional(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "risk.order-notional") == 0);
