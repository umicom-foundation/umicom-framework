/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_ibkr_contract_mapping.c
 *
 * PURPOSE:
 *   Verify IBKR Contract Mapping broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_ibkr_contract_mapping(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ibkr.contract-mapping") == 0);
