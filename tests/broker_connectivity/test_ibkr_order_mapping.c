/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_ibkr_order_mapping.c
 *
 * PURPOSE:
 *   Verify IBKR Order Mapping broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_ibkr_order_mapping(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "ibkr.order-mapping") == 0);
