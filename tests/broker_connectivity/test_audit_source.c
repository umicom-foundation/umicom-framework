/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_audit_source.c
 *
 * PURPOSE:
 *   Verify Audit Source Coverage broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_audit_source(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "audit.source") == 0);
