/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_provider_latency.c
 *
 * PURPOSE:
 *   Verify Provider Latency broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_provider_latency(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "provider.latency") == 0);
