/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_execution_average_price.c
 *
 * PURPOSE:
 *   Verify Execution Average Price broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_execution_average_price(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "execution.average-price") == 0);
