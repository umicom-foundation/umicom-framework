/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_order_cancel_gate.c
 *
 * PURPOSE:
 *   Verify Order Cancel Gate broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_order_cancel_gate(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "order.cancel-gate") == 0);
