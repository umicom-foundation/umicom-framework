/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_position_average_cost.c
 *
 * PURPOSE:
 *   Verify Position Average Cost broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_position_average_cost(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "position.average-cost") == 0);
