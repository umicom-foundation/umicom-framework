/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_audit_gap.c
 *
 * PURPOSE:
 *   Verify Audit Sequence Gap broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_audit_gap(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "audit.gap") == 0);
