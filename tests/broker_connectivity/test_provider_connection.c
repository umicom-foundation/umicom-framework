/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_provider_connection.c
 *
 * PURPOSE:
 *   Verify Provider Connection broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_provider_connection(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "provider.connection") == 0);
