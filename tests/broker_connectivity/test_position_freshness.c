/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_position_freshness.c
 *
 * PURPOSE:
 *   Verify Position Freshness broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_position_freshness(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "position.freshness") == 0);
