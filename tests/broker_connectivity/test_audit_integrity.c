/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_audit_integrity.c
 *
 * PURPOSE:
 *   Verify Audit Integrity broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_audit_integrity(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "audit.integrity") == 0);
