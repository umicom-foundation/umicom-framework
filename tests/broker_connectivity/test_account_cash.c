/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_account_cash.c
 *
 * PURPOSE:
 *   Verify Account Cash broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_account_cash(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "account.cash") == 0);
