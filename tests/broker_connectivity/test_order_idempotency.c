/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_order_idempotency.c
 *
 * PURPOSE:
 *   Verify Order Idempotency broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_order_idempotency(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "order.idempotency") == 0);
