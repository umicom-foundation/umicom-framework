/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_account_net_liquidation.c
 *
 * PURPOSE:
 *   Verify Net Liquidation broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_account_net_liquidation(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "account.net-liquidation") == 0);
