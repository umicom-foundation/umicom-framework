/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_risk_concentration.c
 *
 * PURPOSE:
 *   Verify Concentration Gate broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_risk_concentration(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "risk.concentration") == 0);
