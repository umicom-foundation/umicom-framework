/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_risk_position_quantity.c
 *
 * PURPOSE:
 *   Verify Maximum Position Quantity broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_risk_position_quantity(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "risk.position-quantity") == 0);
