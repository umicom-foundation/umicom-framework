/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_position_unrealised_pnl.c
 *
 * PURPOSE:
 *   Verify Position Unrealised P&L broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_position_unrealised_pnl(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "position.unrealised-pnl") == 0);
