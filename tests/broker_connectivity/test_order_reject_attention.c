/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_order_reject_attention.c
 *
 * PURPOSE:
 *   Verify Order Reject Attention broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_order_reject_attention(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "order.reject-attention") == 0);
