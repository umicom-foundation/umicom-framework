/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_execution_duplicate_detection.c
 *
 * PURPOSE:
 *   Verify Execution Duplicate Detection broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_execution_duplicate_detection(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "execution.duplicate-detection") == 0);
