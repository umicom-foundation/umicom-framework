/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_provider_capabilities.c
 *
 * PURPOSE:
 *   Verify Provider Capabilities broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_provider_capabilities(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "provider.capabilities") == 0);
