/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_execution_quality.c
 *
 * PURPOSE:
 *   Verify Execution Quality broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_execution_quality(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "execution.quality") == 0);
