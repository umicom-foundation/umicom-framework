/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_execution_slippage.c
 *
 * PURPOSE:
 *   Verify Execution Slippage broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_execution_slippage(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "execution.slippage") == 0);
    assert(snapshot.revision == 123U);
    assert(snapshot.score >= 0.0 && snapshot.score <= 100.0);
    assert(snapshot.blocked == 0);
    assert(snapshot.ratio > 4.9 && snapshot.ratio < 5.1);
    return 0;
}
