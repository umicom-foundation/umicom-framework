/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/broker_connectivity/test_provider_reconnect.c
 *
 * PURPOSE:
 *   Verify Provider Reconnect broker-connectivity behaviour.
 *
...
int main(void)
{
    ...
    assert(umi_broker_connectivity_evaluate_provider_reconnect(&input, &snapshot) == UMI_STATUS_OK);
    assert(strcmp(snapshot.id, "provider.reconnect") == 0);
