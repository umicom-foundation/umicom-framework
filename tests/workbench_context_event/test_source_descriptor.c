/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: tests/workbench_context_event/test_source_descriptor.c
 *
 * PURPOSE:
 *   Verify source descriptor validation, mutation and stable hashing.
 *
...
int main(void)
{
    ...
    assert(umi_workbench_context_event_source_descriptor_validate(
        &record) == UMI_STATUS_OK);

    assert(umi_workbench_context_event_source_descriptor_set_source(
        &record,
        "source") == UMI_STATUS_OK);
    assert(umi_workbench_context_event_source_descriptor_set_subject(
        &record,
        "subject") == UMI_STATUS_OK);
    assert(umi_workbench_context_event_source_descriptor_set_group(
        &record,
        "blue") == UMI_STATUS_OK);
    assert(umi_workbench_context_event_source_descriptor_set_label(
        &record,
        "label") == UMI_STATUS_OK);
    ...
    first_hash = umi_workbench_context_event_source_descriptor_hash(
        &record);
    assert(first_hash != 0U);
    umi_workbench_context_event_source_descriptor_touch(
        &record,
        7U,
        1000U);
