/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: include/umicom/developer_workbench/developer_workbench.h
 *
 * PURPOSE:
 *   Aggregate the reusable professional developer-workbench platform.
 *
 * AUTHOR AND ORGANISATION:
 * Sammy Hegab
 * Umicom Foundation
 *
 * LICENCE:
 * MIT
 *---------------------------------------------------------------------------*/
#ifndef UMICOM_DEVELOPER_WORKBENCH_DEVELOPER_WORKBENCH_H
#define UMICOM_DEVELOPER_WORKBENCH_DEVELOPER_WORKBENCH_H

#include "umicom/developer_workbench/types.h"
#include "umicom/developer_workbench/configuration.h"
#include "umicom/developer_workbench/configuration_registry.h"
#include "umicom/developer_workbench/action_registry.h"
#include "umicom/developer_workbench/readiness.h"
#include "umicom/developer_workbench/operation_history.h"
#include "umicom/developer_workbench/lifecycle.h"
#include "umicom/developer_workbench/commands/commands.h"
#include "umicom/developer_workbench/command_catalogue.h"
#include "umicom/developer_workbench/command_service.h"
#include "umicom/developer_workbench/search_types.h"
#include "umicom/developer_workbench/search_provider.h"
#include "umicom/developer_workbench/search_engine.h"
#include "umicom/developer_workbench/command_search_provider.h"
#include "umicom/developer_workbench/file_search_provider.h"
#include "umicom/developer_workbench/recent_project_provider.h"
#include "umicom/developer_workbench/search_session.h"
#include "umicom/developer_workbench/command_palette.h"
#include "umicom/developer_workbench/quick_open.h"
#include "umicom/developer_workbench/recent_projects.h"
#include "umicom/developer_workbench/start_centre.h"
#include "umicom/developer_workbench/project_wizard.h"
#include "umicom/developer_workbench/perspective.h"
#include "umicom/developer_workbench/perspective_registry.h"
#include "umicom/developer_workbench/builtin_perspectives.h"
#include "umicom/developer_workbench/perspectives/perspectives.h"
#include "umicom/developer_workbench/workbench.h"

#endif
