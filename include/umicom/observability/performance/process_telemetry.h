/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: include/umicom/observability/performance/process_telemetry.h
 *
 * PURPOSE:
 *   Represent process-level CPU and memory telemetry for process telemetry.
 *
 * AUTHOR AND ORGANISATION:
 * Sammy Hegab
 * Umicom Foundation
 *
 * LICENCE:
 * MIT
 *---------------------------------------------------------------------------*/

#ifndef UMICOM_OBSERVABILITY_PERFORMANCE_PROCESS_TELEMETRY_H
#define UMICOM_OBSERVABILITY_PERFORMANCE_PROCESS_TELEMETRY_H

#include <stdbool.h>
#include <stddef.h>
#include <stdint.h>
#include "umicom/observability/performance/types.h"

#ifdef __cplusplus
extern "C" {
#endif

/**
 * Represent the performance process telemetry data shared with callers of this public
 * contract.
 */
typedef struct UmiPerformanceProcessTelemetry {
    uint32_t structure_size;
    uint32_t api_version;
    char id[UMI_PERFORMANCE_ID_CAPACITY];
    char subject_id[UMI_PERFORMANCE_ID_CAPACITY];
    UmiPerformanceState state;
    UmiPerformanceSeverity severity;
    uint64_t sequence;
    uint64_t timestamp_ns;
    double value;
    double auxiliary;
    uint64_t count;
    bool enabled;
} UmiPerformanceProcessTelemetry;

/* Initialise a versioned process telemetry record with stable identities. */
UmiStatus umi_performance_process_telemetry_init(UmiPerformanceProcessTelemetry *record, const char *id, const char *subject_id);
/* Validate structure/version/identity invariants before a record is consumed. */
UmiStatus umi_performance_process_telemetry_validate(const UmiPerformanceProcessTelemetry *record);
/* Update point-in-time measurement evidence and monotonically advance sequence. */
UmiStatus umi_performance_process_telemetry_observe(UmiPerformanceProcessTelemetry *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns);
/* Compare stable identities without comparing volatile measurement state. */
bool umi_performance_process_telemetry_same_identity(const UmiPerformanceProcessTelemetry *left, const UmiPerformanceProcessTelemetry *right);
/* Domain-specific policy helper for process telemetry. */
double umi_performance_process_telemetry_ratio(double numerator, double denominator);

#ifdef __cplusplus
}
#endif
#endif
