/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: include/umicom/desktop/control/context_direction.h
 *
 * PURPOSE:
 *   Represent source, destination or bidirectional context-link intent.
 *
 * ARCHITECTURE:
 *   This is additive Framework-owned Desk control state. It extends the
 *   established desktop/workbench/layout runtime; it does not replace existing
 *   models and applications remain thin consumers of this public contract.
 *
 * AUTHOR AND ORGANISATION:
 * Sammy Hegab
 * Umicom Foundation
 *
 * LICENCE:
 * MIT
 *---------------------------------------------------------------------------*/
#ifndef UMICOM_DESKTOP_CONTROL_CONTEXT_DIRECTION_H
#define UMICOM_DESKTOP_CONTROL_CONTEXT_DIRECTION_H

#include "umicom/desktop/control/types.h"

#ifdef __cplusplus
extern "C" {
#endif

#define UMI_DESKTOP_CONTEXT_DIRECTION_API_VERSION 1U

/**
 * Represent the desktop context direction snapshot data shared with callers of this public
 * contract.
 */
typedef struct UmiDesktopContextDirectionSnapshot {
    uint32_t struct_size;
    uint32_t api_version;
    char id[UMI_DESKTOP_CONTROL_ID_CAPACITY];
    char subject_id[UMI_DESKTOP_CONTROL_ID_CAPACITY];
    char detail[UMI_DESKTOP_CONTROL_TEXT_CAPACITY];
    uint32_t state;
    uint32_t priority;
    uint64_t revision;
    bool enabled;
} UmiDesktopContextDirectionSnapshot;

/* Initialise a valid bounded snapshot with stable identity. */
void umi_desktop_context_direction_init(UmiDesktopContextDirectionSnapshot *value, const char *id);
/* Validate structure version, identity and bounded string invariants. */
UmiStatus umi_desktop_context_direction_validate(const UmiDesktopContextDirectionSnapshot *value);
/* Associate the control record with an application, panel, workbench or layout identity. */
UmiStatus umi_desktop_context_direction_set_subject(UmiDesktopContextDirectionSnapshot *value, const char *subject_id);
/* Store human-readable diagnostic/policy detail without silent truncation. */
UmiStatus umi_desktop_context_direction_set_detail(UmiDesktopContextDirectionSnapshot *value, const char *detail);
/* Update numeric state and increment the monotonic local revision. */
UmiStatus umi_desktop_context_direction_set_state(UmiDesktopContextDirectionSnapshot *value, uint32_t state);
/* Update deterministic ordering/ranking priority. */
UmiStatus umi_desktop_context_direction_set_priority(UmiDesktopContextDirectionSnapshot *value, uint32_t priority);
/* Toggle the record while retaining identity for layout/session restoration. */
UmiStatus umi_desktop_context_direction_set_enabled(UmiDesktopContextDirectionSnapshot *value, bool enabled);
/* Identity comparison deliberately ignores mutable state. */
bool umi_desktop_context_direction_same_identity(const UmiDesktopContextDirectionSnapshot *left, const UmiDesktopContextDirectionSnapshot *right);

/* Feature-specific policy helper keeps this decision in Framework rather than a thin application. */
bool umi_desktop_context_direction_allows_publish(UmiDesktopControlDirection direction);

#ifdef __cplusplus
}
#endif
#endif
