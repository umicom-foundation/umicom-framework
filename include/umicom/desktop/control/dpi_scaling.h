/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: include/umicom/desktop/control/dpi_scaling.h
 *
 * PURPOSE:
 *   Store DPI/scale-factor policy used when restoring persisted geometry.
 *
 * ARCHITECTURE:
 *   This is additive Framework-owned Desk control state. It extends the
 *   established desktop/workbench/layout runtime; it does not replace existing
 *   models and applications remain thin consumers of this public contract.
 *
 * AUTHOR AND ORGANISATION:
 * Sammy Hegab
 * Umicom Foundation
 *
 * LICENCE:
 * MIT
 *---------------------------------------------------------------------------*/
#ifndef UMICOM_DESKTOP_CONTROL_DPI_SCALING_H
#define UMICOM_DESKTOP_CONTROL_DPI_SCALING_H

#include "umicom/desktop/control/types.h"

#ifdef __cplusplus
extern "C" {
#endif

#define UMI_DESKTOP_DPI_SCALING_API_VERSION 1U

/**
 * Represent the desktop dpi scaling snapshot data shared with callers of this public
 * contract.
 */
typedef struct UmiDesktopDpiScalingSnapshot {
    uint32_t struct_size;
    uint32_t api_version;
    char id[UMI_DESKTOP_CONTROL_ID_CAPACITY];
    char subject_id[UMI_DESKTOP_CONTROL_ID_CAPACITY];
    char detail[UMI_DESKTOP_CONTROL_TEXT_CAPACITY];
    uint32_t state;
    uint32_t priority;
    uint64_t revision;
    bool enabled;
} UmiDesktopDpiScalingSnapshot;

/* Initialise a valid bounded snapshot with stable identity. */
void umi_desktop_dpi_scaling_init(UmiDesktopDpiScalingSnapshot *value, const char *id);
/* Validate structure version, identity and bounded string invariants. */
UmiStatus umi_desktop_dpi_scaling_validate(const UmiDesktopDpiScalingSnapshot *value);
/* Associate the control record with an application, panel, workbench or layout identity. */
UmiStatus umi_desktop_dpi_scaling_set_subject(UmiDesktopDpiScalingSnapshot *value, const char *subject_id);
/* Store human-readable diagnostic/policy detail without silent truncation. */
UmiStatus umi_desktop_dpi_scaling_set_detail(UmiDesktopDpiScalingSnapshot *value, const char *detail);
/* Update numeric state and increment the monotonic local revision. */
UmiStatus umi_desktop_dpi_scaling_set_state(UmiDesktopDpiScalingSnapshot *value, uint32_t state);
/* Update deterministic ordering/ranking priority. */
UmiStatus umi_desktop_dpi_scaling_set_priority(UmiDesktopDpiScalingSnapshot *value, uint32_t priority);
/* Toggle the record while retaining identity for layout/session restoration. */
UmiStatus umi_desktop_dpi_scaling_set_enabled(UmiDesktopDpiScalingSnapshot *value, bool enabled);
/* Identity comparison deliberately ignores mutable state. */
bool umi_desktop_dpi_scaling_same_identity(const UmiDesktopDpiScalingSnapshot *left, const UmiDesktopDpiScalingSnapshot *right);

/* Feature-specific policy helper keeps this decision in Framework rather than a thin application. */
UmiStatus umi_desktop_dpi_scaling_scale_rect(const UmiDesktopControlRect *input, uint32_t from_scale, uint32_t to_scale, UmiDesktopControlRect *output);

#ifdef __cplusplus
}
#endif
#endif
