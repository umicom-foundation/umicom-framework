/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: include/umicom/desktop/desktop.h
 *
 * PURPOSE:
 *   Publish the public desktop contract for reusable Umicom applications.
 *
 * AUTHOR AND ORGANISATION:
 * Sammy Hegab
 * Umicom Foundation
 *
 * LICENCE:
 * MIT
 *---------------------------------------------------------------------------*/
#ifndef UMICOM_DESKTOP_DESKTOP_H
#define UMICOM_DESKTOP_DESKTOP_H

#include "umicom/desktop/types.h"
#include "umicom/desktop/monitor_topology.h"
#include "umicom/desktop/layout_tabs.h"
#include "umicom/desktop/window_manager.h"
#include "umicom/desktop/layout_catalogue.h"
#include "umicom/desktop/context_links.h"
#include "umicom/desktop/runtime.h"
#include "umicom/desktop/content_runtime.h"
#include "umicom/desktop/component_drag_drop.h"
#include "umicom/desktop/context_synchronizer.h"
#include "umicom/desktop/layout_history.h"
#include "umicom/desktop/monitor_interaction.h"
#include "umicom/desktop/persistence.h"
#include "umicom/desktop/session_recovery.h"
#include "umicom/desktop/ui_bridge.h"
#include "umicom/desktop/monitor_canvas.h"
#include "umicom/desktop/snap_engine.h"
#include "umicom/desktop/layout_designer.h"
#include "umicom/desktop/shell_actions.h"
#include "umicom/desktop/shell_model.h"
#include "umicom/desktop/views.h"
#include "umicom/desktop/master_authority.h"

#endif
