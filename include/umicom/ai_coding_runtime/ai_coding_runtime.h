/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: include/umicom/ai_coding_runtime/ai_coding_runtime.h
 *
 * PURPOSE:
 *   Aggregate the operational repository-aware AI coding and vibe-coding runtime.
 *
 * AUTHOR AND ORGANISATION:
 * Sammy Hegab
 * Umicom Foundation
 *
 * LICENCE:
 * MIT
 *---------------------------------------------------------------------------*/
#ifndef UMICOM_AI_CODING_RUNTIME_H
#define UMICOM_AI_CODING_RUNTIME_H

#include "umicom/ai_coding_runtime/types.h"
#include "umicom/ai_coding_runtime/config.h"
#include "umicom/ai_coding_runtime/path.h"
#include "umicom/ai_coding_runtime/workspace.h"
#include "umicom/ai_coding_runtime/local_workspace.h"
#include "umicom/ai_coding_runtime/language.h"
#include "umicom/ai_coding_runtime/ignore.h"
#include "umicom/ai_coding_runtime/scanner.h"
#include "umicom/ai_coding_runtime/repository_index.h"
#include "umicom/ai_coding_runtime/selection.h"
#include "umicom/ai_coding_runtime/context_materializer.h"
#include "umicom/ai_coding_runtime/context_summary.h"
#include "umicom/ai_coding_runtime/prompt_template.h"
#include "umicom/ai_coding_runtime/prompt.h"
#include "umicom/ai_coding_runtime/response.h"
#include "umicom/ai_coding_runtime/provider_executor.h"
#include "umicom/ai_coding_runtime/patch_preview.h"
#include "umicom/ai_coding_runtime/change_guard.h"
#include "umicom/ai_coding_runtime/validation_plan.h"
#include "umicom/ai_coding_runtime/validation_runner.h"
#include "umicom/ai_coding_runtime/validation_feedback.h"
#include "umicom/ai_coding_runtime/validation_selector.h"
#include "umicom/ai_coding_runtime/validation_profiles/profiles.h"
#include "umicom/ai_coding_runtime/repair_policy.h"
#include "umicom/ai_coding_runtime/cancellation.h"
#include "umicom/ai_coding_runtime/event_queue.h"
#include "umicom/ai_coding_runtime/history.h"
#include "umicom/ai_coding_runtime/audit.h"
#include "umicom/ai_coding_runtime/task_queue.h"
#include "umicom/ai_coding_runtime/session.h"
#include "umicom/ai_coding_runtime/agent.h"
#include "umicom/ai_coding_runtime/platform.h"
#include "umicom/ai_coding_runtime/workbench_bridge.h"

#endif
