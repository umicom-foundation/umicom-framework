/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: include/umicom/designer/visual_designer/types.h
 *
 * PURPOSE:
...
/*
 * Compatibility note:
 * The established UmiRad types and umi_rad functions keep their original C
 * names so this folder migration does not also change the binary interface.
 * Callers must update the include path, but they do not need to rewrite every
 * symbol. RAD means Rapid Application Development in those legacy names. New
 * files, build targets and documentation use the clearer visual_designer name.
 */
#ifndef UMICOM_DESIGNER_VISUAL_DESIGNER_TYPES_H
#define UMICOM_DESIGNER_VISUAL_DESIGNER_TYPES_H
...
/**
 * List the named visual designer orientation values accepted by this public contract.
 */
typedef enum UmiRadOrientation { UMI_RAD_HORIZONTAL = 1, UMI_RAD_VERTICAL = 2 } UmiRadOrientation;
/**
 * List the named visual designer severity values accepted by this public contract.
 */
typedef enum UmiRadSeverity { UMI_RAD_INFO = 1, UMI_RAD_WARNING = 2, UMI_RAD_ERROR = 3 } UmiRadSeverity;
/**
 * List the named visual designer target kind values accepted by this public contract.
 */
...
/**
 * List the named visual designer transaction state values accepted by this public contract.
 */
...
/**
 * Represent the visual designer point data shared with callers of this public contract.
 */
