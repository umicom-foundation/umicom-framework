/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: include/umicom/ui/mosaic/monitor_affinity.h
 *
 * PURPOSE:
 *   Define toolkit-neutral monitor affinity contracts for the Framework-owned workbench mosaic platform.
 *
 * AUTHOR AND ORGANISATION:
 * Sammy Hegab
 * Umicom Foundation
 *
 * LICENCE:
 * MIT
 *---------------------------------------------------------------------------*/
#ifndef UMICOM_UI_MOSAIC_MONITOR_AFFINITY_H
#define UMICOM_UI_MOSAIC_MONITOR_AFFINITY_H

#include <stdbool.h>
#include <stdint.h>
#include "umicom/ui/mosaic/types.h"

#ifdef __cplusplus
extern "C" {
#endif

/**
 * Represent the ui mosaic monitor affinity data shared with callers of this public
 * contract.
 */
typedef struct UmiUiMosaicMonitorAffinity {
    char id[UMI_UI_MOSAIC_ID_CAPACITY];
    char name[UMI_UI_MOSAIC_TEXT_CAPACITY];
    uint32_t revision;
    uint32_t item_count;
    bool locked;
} UmiUiMosaicMonitorAffinity;

/* Initializes versioned layout state used by monitor affinity. */
void umi_ui_mosaic_monitor_affinity_init(UmiUiMosaicMonitorAffinity *value);
/* Assigns the stable layout identity and display name. */
UmiStatus umi_ui_mosaic_monitor_affinity_set(UmiUiMosaicMonitorAffinity *value, const char *id, const char *name);
/* Validates identity and bounded layout cardinality. */
UmiStatus umi_ui_mosaic_monitor_affinity_validate(const UmiUiMosaicMonitorAffinity *value);
/* Advances the revision after a committed edit. */
UmiStatus umi_ui_mosaic_monitor_affinity_touch(UmiUiMosaicMonitorAffinity *value);

#ifdef __cplusplus
}
#endif
#endif
