/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: include/umicom/studio_runtime/surface_profiles/profiles.h
 *
 * PURPOSE:
 *   Aggregate Studio surface/Application Shell bindings.
 *
 * AUTHOR AND ORGANISATION:
 * Sammy Hegab
 * Umicom Foundation
 *
 * LICENCE:
 * MIT
 *---------------------------------------------------------------------------*/
#ifndef UMICOM_STUDIO_RUNTIME_SURFACE_PROFILES_H
#define UMICOM_STUDIO_RUNTIME_SURFACE_PROFILES_H
#include "umicom/studio_runtime/surface_profiles/explorer.h"
#include "umicom/studio_runtime/surface_profiles/editor.h"
#include "umicom/studio_runtime/surface_profiles/outline.h"
#include "umicom/studio_runtime/surface_profiles/search.h"
#include "umicom/studio_runtime/surface_profiles/problems.h"
#include "umicom/studio_runtime/surface_profiles/output.h"
#include "umicom/studio_runtime/surface_profiles/terminal.h"
#include "umicom/studio_runtime/surface_profiles/source_control.h"
#include "umicom/studio_runtime/surface_profiles/test_explorer.h"
#include "umicom/studio_runtime/surface_profiles/debug.h"
#include "umicom/studio_runtime/surface_profiles/build.h"
#include "umicom/studio_runtime/surface_profiles/ai_chat.h"
#include "umicom/studio_runtime/surface_profiles/ai_review.h"
#include "umicom/studio_runtime/surface_profiles/ai_approvals.h"
#include "umicom/studio_runtime/surface_profiles/ai_tools.h"
#include "umicom/studio_runtime/surface_profiles/ai_validation.h"
#endif
