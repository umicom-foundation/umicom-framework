/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: include/umicom/developer/helix/autonomy_policy.h
 *
 * PURPOSE:
 *   Define the permitted autonomy ceiling and mandatory review requirements for engineering actions.
 *
 * AUTHOR AND ORGANISATION:
 * Sammy Hegab
 * Umicom Foundation
 *
 * LICENCE:
 * MIT
 *---------------------------------------------------------------------------*/
#ifndef UMICOM_DEVELOPER_HELIX_AUTONOMY_POLICY_H
#define UMICOM_DEVELOPER_HELIX_AUTONOMY_POLICY_H
#include "umicom/developer/helix/types.h"
#ifdef __cplusplus
extern "C" {
#endif
/**
 * Represent the helix autonomy policy data shared with callers of this public contract.
 */
typedef struct UmiHelixAutonomyPolicy {
    uint32_t structure_size; uint32_t api_version; char id[UMI_HELIX_ID_CAPACITY]; char subject_id[UMI_HELIX_ID_CAPACITY];
    UmiHelixState state; UmiHelixSeverity severity; uint64_t sequence; uint32_t flags; double threshold; bool blocking;
} UmiHelixAutonomyPolicy;
/* Initialise a deterministic, empty autonomy policy record. */
void umi_helix_autonomy_policy_init(UmiHelixAutonomyPolicy *record,const char *id);
/* Validate structural invariants before the record is consumed by another Helix component. */
UmiStatus umi_helix_autonomy_policy_validate(const UmiHelixAutonomyPolicy *record);
/* Return a normalised evidence score used by higher-level planning and governance. */
double umi_helix_autonomy_policy_score(const UmiHelixAutonomyPolicy *record);
#ifdef __cplusplus
}
#endif
#endif
