...
UmiStatus umi_security_role_assignment_add(UmiSecurityRoleAssignmentStore *store,const UmiSecurityRoleAssignment *assignment)
{
    ...
    for (index = 0U; index < store->count; ++index) /* Keep the operation inside its valid bounds before reading, writing or adding data. */ if (strcmp(store->items[index].assignment_id,assignment->assignment_id) == 0) return UMI_STATUS_ALREADY_EXISTS;
...
UmiStatus umi_security_role_assignment_revoke(UmiSecurityRoleAssignmentStore *store,const char *assignment_id)
{
    ...
    /* Visit each bounded item once so every record receives the same rule. */
    for (index = 0U; index < store->count; ++index) /* Keep the operation inside its valid bounds before reading, writing or adding data. */ if (strcmp(store->items[index].assignment_id,assignment_id) == 0) { store->items[index].enabled = false; return UMI_STATUS_OK; }
