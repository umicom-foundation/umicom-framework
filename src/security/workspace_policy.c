...
UmiStatus umi_security_workspace_policy_evaluate(UmiWorkspaceTrustLevel trust,const UmiSecurityPermissionDescriptor *permission,UmiSecurityGovernanceDecision *out_decision)
{
    ...
    else /* Keep the operation inside its valid bounds before reading, writing or adding data. */ if (trust == UMI_WORKSPACE_UNTRUSTED && permission->risk >= UMI_SECURITY_RISK_HIGH) { out_decision->allowed = false; (void)snprintf(out_decision->reason,sizeof(out_decision->reason),"High-risk action denied in an untrusted workspace"); }
