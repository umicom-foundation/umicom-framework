...
UmiStatus umi_authorisation_service_create(UmiPolicyEngine *policy, UmiRoleRegistry *roles, UmiAuthorisationService **out_service)
{
    ...
    *out_service = NULL; service = (UmiAuthorisationService *)calloc(1U, sizeof(*service)); /* Protect caller-owned memory by checking that required state is available before it is used. */ if (service == NULL) return UMI_STATUS_OUT_OF_MEMORY;
