/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_selection/filter_record.c
 *
 * PURPOSE:
 *   Implement bounded mutation, validation and hashing for the selection filter record.
 *
...
/*
 * Initialise workbench selection filter record from caller-provided values so later
...
/*
 * Check that workbench selection filter record satisfies its contract before another
...
UmiStatus fn(UmiWorkbenchSelectionFilterRecord *record, const char *value) \
{ \
    UmiStatus status; \
...
UMI_SELECTION_SETTER(umi_workbench_selection_filter_record_set_source, source_id)
UMI_SELECTION_SETTER(umi_workbench_selection_filter_record_set_subject, subject_id)
UMI_SELECTION_SETTER(umi_workbench_selection_filter_record_set_secondary, secondary_id)
UMI_SELECTION_SETTER(umi_workbench_selection_filter_record_set_group, group_id)
UMI_SELECTION_SETTER(umi_workbench_selection_filter_record_set_label, label)

#undef UMI_SELECTION_SETTER

/*
 * Provide the workbench selection filter record hash operation used by this module and its
 * client applications.
 */
...
/*
 * Provide the workbench selection filter record touch operation used by this module and
