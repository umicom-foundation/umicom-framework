/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_selection/workspace_item.c
 *
 * PURPOSE:
 *   Implement bounded mutation, validation and hashing for the structured workspace item.
 *
...
/*
 * Initialise workbench selection workspace item from caller-provided values so later
...
/*
 * Check that workbench selection workspace item satisfies its contract before another
...
UmiStatus fn(UmiWorkbenchSelectionWorkspaceItem *record, const char *value) \
{ \
    UmiStatus status; \
...
UMI_SELECTION_SETTER(umi_workbench_selection_workspace_item_set_source, source_id)
UMI_SELECTION_SETTER(umi_workbench_selection_workspace_item_set_subject, subject_id)
UMI_SELECTION_SETTER(umi_workbench_selection_workspace_item_set_secondary, secondary_id)
UMI_SELECTION_SETTER(umi_workbench_selection_workspace_item_set_group, group_id)
UMI_SELECTION_SETTER(umi_workbench_selection_workspace_item_set_label, label)

#undef UMI_SELECTION_SETTER

/*
 * Provide the workbench selection workspace item hash operation used by this module and
...
/*
 * Provide the workbench selection workspace item touch operation used by this module and
 * its client applications.
 */
