/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_selection/selection_set.c
 *
 * PURPOSE:
 *   Implement bounded mutation, validation and hashing for the multi-item selection set.
 *
...
/*
 * Initialise workbench selection selection set from caller-provided values so later
...
/*
 * Check that workbench selection selection set satisfies its contract before another
...
UmiStatus fn(UmiWorkbenchSelectionSelectionSet *record, const char *value) \
{ \
    UmiStatus status; \
...
UMI_SELECTION_EXTRA_SETTER(umi_workbench_selection_selection_set_set_source, source_id)
UMI_SELECTION_EXTRA_SETTER(umi_workbench_selection_selection_set_set_subject, subject_id)
UMI_SELECTION_EXTRA_SETTER(umi_workbench_selection_selection_set_set_related, related_id)
UMI_SELECTION_EXTRA_SETTER(umi_workbench_selection_selection_set_set_group, group_id)
UMI_SELECTION_EXTRA_SETTER(umi_workbench_selection_selection_set_set_description, description)

#undef UMI_SELECTION_EXTRA_SETTER

/*
 * Provide the workbench selection selection set hash operation used by this module and its
 * client applications.
 */
...
/*
 * Provide the workbench selection selection set touch operation used by this module and
