/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_selection/node_identity.c
 *
 * PURPOSE:
 *   Implement bounded mutation, validation and hashing for the structured node identity.
 *
...
/*
 * Initialise workbench selection node identity from caller-provided values so later
...
/*
 * Check that workbench selection node identity satisfies its contract before another
...
UmiStatus fn(UmiWorkbenchSelectionNodeIdentity *record, const char *value) \
{ \
    UmiStatus status; \
...
UMI_SELECTION_EXTRA_SETTER(umi_workbench_selection_node_identity_set_source, source_id)
UMI_SELECTION_EXTRA_SETTER(umi_workbench_selection_node_identity_set_subject, subject_id)
UMI_SELECTION_EXTRA_SETTER(umi_workbench_selection_node_identity_set_related, related_id)
UMI_SELECTION_EXTRA_SETTER(umi_workbench_selection_node_identity_set_group, group_id)
UMI_SELECTION_EXTRA_SETTER(umi_workbench_selection_node_identity_set_description, description)

#undef UMI_SELECTION_EXTRA_SETTER

/*
 * Provide the workbench selection node identity hash operation used by this module and its
 * client applications.
 */
...
/*
 * Provide the workbench selection node identity touch operation used by this module and
