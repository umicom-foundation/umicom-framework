...
for(i=0U;i<store->count;++i)/* Keep the operation inside its valid bounds before reading, writing or adding data. */ if(strcmp(store->items[i].receipt_id,identity)==0)return &store->items[i];
return NULL;
}
/*
