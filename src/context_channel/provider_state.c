/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/context_channel/provider_state.c
 *
 * PURPOSE:
 *   Implement track health evidence for context providers.
 *
...
{
    UmiContextProviderState *existing;
uint64_t next_revision;
    ...
    /* Preserve the original failure result so the caller can respond to the correct cause. */
    if(umi_context_provider_state_validate(record)!=UMI_STATUS_OK)return UMI_STATUS_INVALID_ARGUMENT;
    existing=umi_context_provider_state_store_find(store,record->provider_id);
    /*
     * Protect caller-owned memory by checking that required state is available before it is
     * used.
     */
...
return UMI_STATUS_OK;
    ...
    if(store->count>=UMI_CONTEXT_PROVIDER_STATE_MAX_ITEMS)return UMI_STATUS_CAPACITY_EXCEEDED;
