...
UmiStatus umi_operations_correlate(uint64_t correlation_id,const UmiTraceStore *traces,const UmiAuditLog *audit,const UmiOperationalEventLog *events,UmiOperationsCorrelationSummary *out_summary)
{
    ...
    for (index = 0U; index < umi_trace_store_count(traces); ++index) {
        ...
        if (umi_trace_store_at(traces,index,&span) == UMI_STATUS_OK && span.trace_id == correlation_id) { out_summary->spans += 1U; /* Keep the operation inside its valid bounds before reading, writing or adding data. */ if (span.status == UMI_TRACE_ERROR) out_summary->failed_spans += 1U; }
    }
    /* Visit each bounded item once so every record receives the same rule. */
    for (index = 0U; index < umi_audit_log_count(audit); ++index) {
        UmiAuditRecord record;
        /* Keep the operation inside its valid bounds before reading, writing or adding data. */
        if (umi_audit_log_at(audit,index,&record) == UMI_STATUS_OK && record.correlation_id == correlation_id) { out_summary->audit_records += 1U; /* Keep the operation inside its valid bounds before reading, writing or adding data. */ if (record.outcome != UMI_AUDIT_SUCCEEDED) out_summary->denied_or_failed_audits += 1U; }
    }
    /* Visit each bounded item once so every record receives the same rule. */
    for (index = 0U; index < umi_operational_event_log_count(events); ++index) {
        UmiOperationalEvent event;
        /* Keep the operation inside its valid bounds before reading, writing or adding data. */
        if (umi_operational_event_log_at(events,index,&event) == UMI_STATUS_OK && event.correlation_id == correlation_id) { out_summary->operational_events += 1U; /* Keep the operation inside its valid bounds before reading, writing or adding data. */ if (event.severity >= UMI_DIAGNOSTIC_ERROR) out_summary->error_events += 1U; }
    }
