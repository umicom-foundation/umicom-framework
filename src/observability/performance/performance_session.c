/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/performance_session.c
 *
 * PURPOSE:
 *   Implement represent performance regression and budget control state for performance session.
 *
...
UmiStatus umi_performance_performance_session_observe(UmiPerformancePerformanceSession *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_performance_session_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
...
/* Calculate elapsed nanoseconds without unsigned underflow. */
