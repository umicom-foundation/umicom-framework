/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/coverage_regression.c
 *
 * PURPOSE:
 *   Implement represent code-coverage evidence, baselines and regressions for coverage regression.
 *
...
UmiStatus umi_performance_coverage_regression_observe(UmiPerformanceCoverageRegression *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_coverage_regression_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
