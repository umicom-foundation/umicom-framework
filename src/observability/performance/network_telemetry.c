/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/network_telemetry.c
 *
 * PURPOSE:
 *   Implement represent network throughput and latency telemetry for network telemetry.
 *
...
UmiStatus umi_performance_network_telemetry_observe(UmiPerformanceNetworkTelemetry *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_network_telemetry_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
