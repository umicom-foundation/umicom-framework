/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/trace_session.c
 *
 * PURPOSE:
 *   Implement represent distributed/runtime trace state and filtering policy for trace session.
 *
...
UmiStatus umi_performance_trace_session_observe(UmiPerformanceTraceSession *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_trace_session_validate(record);
    /* Preserve the original failure result so the caller can respond to the correct cause. */
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
