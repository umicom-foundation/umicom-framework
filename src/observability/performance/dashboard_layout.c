/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/dashboard_layout.c
 *
 * PURPOSE:
 *   Implement represent toolkit-neutral operational dashboard composition for dashboard layout.
 *
...
UmiStatus umi_performance_dashboard_layout_observe(UmiPerformanceDashboardLayout *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_dashboard_layout_validate(record);
    /* Preserve the original failure result so the caller can respond to the correct cause. */
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
