/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/leak_candidate.c
 *
 * PURPOSE:
 *   Implement classify potential memory leaks using bounded evidence for leak candidate.
 *
...
UmiStatus umi_performance_leak_candidate_observe(UmiPerformanceLeakCandidate *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_leak_candidate_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
