/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/benchmark_sample.c
 *
 * PURPOSE:
 *   Implement represent benchmark cases, runs, baselines and statistics for benchmark sample.
 *
...
UmiStatus umi_performance_benchmark_sample_observe(UmiPerformanceBenchmarkSample *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_benchmark_sample_validate(record);
    /* Preserve the original failure result so the caller can respond to the correct cause. */
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
