/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/resource_budget.c
 *
 * PURPOSE:
 *   Implement represent process/system resource observations and budgets for resource budget.
 *
...
UmiStatus umi_performance_resource_budget_observe(UmiPerformanceResourceBudget *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_resource_budget_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
