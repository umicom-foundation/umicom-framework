/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/performance_regression.c
 *
 * PURPOSE:
 *   Implement represent performance regression and budget control state for performance regression.
 *
...
UmiStatus umi_performance_performance_regression_observe(UmiPerformancePerformanceRegression *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_performance_regression_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
...
/* Express current-vs-baseline movement as a signed percentage. */
double umi_performance_performance_regression_regression_percent(double current, double baseline) {
    return umi_performance_percent_delta(current, baseline);
}
