/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/crash_correlation.c
 *
 * PURPOSE:
 *   Implement represent crash signatures, correlations and clustering evidence for crash correlation.
 *
...
UmiStatus umi_performance_crash_correlation_observe(UmiPerformanceCrashCorrelation *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_crash_correlation_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
