/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/benchmark_baseline.c
 *
 * PURPOSE:
 *   Implement represent benchmark cases, runs, baselines and statistics for benchmark baseline.
 *
...
UmiStatus umi_performance_benchmark_baseline_observe(UmiPerformanceBenchmarkBaseline *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_benchmark_baseline_validate(record);
    /* Preserve the original failure result so the caller can respond to the correct cause. */
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
