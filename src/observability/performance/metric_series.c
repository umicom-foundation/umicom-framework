/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/metric_series.c
 *
 * PURPOSE:
 *   Implement represent typed metric metadata, samples and time-series state for metric series.
 *
...
UmiStatus umi_performance_metric_series_observe(UmiPerformanceMetricSeries *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_metric_series_validate(record);
    /* Preserve the original failure result so the caller can respond to the correct cause. */
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
