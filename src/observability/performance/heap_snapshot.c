/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/heap_snapshot.c
 *
 * PURPOSE:
 *   Implement represent heap snapshot evidence for memory analysis for heap snapshot.
 *
...
UmiStatus umi_performance_heap_snapshot_observe(UmiPerformanceHeapSnapshot *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_heap_snapshot_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
