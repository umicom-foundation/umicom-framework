/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/io_telemetry.c
 *
 * PURPOSE:
 *   Implement represent I/O throughput and latency telemetry for io telemetry.
 *
...
UmiStatus umi_performance_io_telemetry_observe(UmiPerformanceIoTelemetry *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_io_telemetry_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
