/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/resource_sample.c
 *
 * PURPOSE:
 *   Implement represent process/system resource observations and budgets for resource sample.
 *
...
UmiStatus umi_performance_resource_sample_observe(UmiPerformanceResourceSample *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_resource_sample_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
