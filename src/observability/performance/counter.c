/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/counter.c
 *
 * PURPOSE:
 *   Implement represent monotonic counter observations for operational telemetry for counter.
 *
...
UmiStatus umi_performance_counter_observe(UmiPerformanceCounter *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_counter_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
