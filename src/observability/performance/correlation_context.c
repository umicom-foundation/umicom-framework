/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/correlation_context.c
 *
 * PURPOSE:
 *   Implement carry deterministic correlation state across telemetry domains for correlation context.
 *
...
UmiStatus umi_performance_correlation_context_observe(UmiPerformanceCorrelationContext *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_correlation_context_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
