/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/profiler_aggregation.c
 *
 * PURPOSE:
 *   Implement represent and evaluate reusable CPU/process profiling state for profiler aggregation.
 *
...
UmiStatus umi_performance_profiler_aggregation_observe(UmiPerformanceProfilerAggregation *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_profiler_aggregation_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
...
/* Compute a mean while treating an empty sample set as zero. */
