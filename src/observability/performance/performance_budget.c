/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/performance_budget.c
 *
 * PURPOSE:
 *   Implement represent performance regression and budget control state for performance budget.
 *
...
UmiStatus umi_performance_performance_budget_observe(UmiPerformancePerformanceBudget *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_performance_budget_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
...
/* Evaluate a non-negative observation against an inclusive budget. */
bool umi_performance_performance_budget_within_budget(double used, double limit) {
    return used >= 0.0 && limit >= 0.0 && used <= limit;
}
