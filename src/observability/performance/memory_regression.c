/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/memory_regression.c
 *
 * PURPOSE:
 *   Implement represent memory pressure, budgets and regression policy for memory regression.
 *
...
UmiStatus umi_performance_memory_regression_observe(UmiPerformanceMemoryRegression *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_memory_regression_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
