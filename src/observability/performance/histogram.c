/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/histogram.c
 *
 * PURPOSE:
 *   Implement represent distribution summary observations for operational telemetry for histogram.
 *
...
UmiStatus umi_performance_histogram_observe(UmiPerformanceHistogram *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_histogram_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
