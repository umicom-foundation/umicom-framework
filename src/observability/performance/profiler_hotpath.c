/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/profiler_hotpath.c
 *
 * PURPOSE:
 *   Implement represent and evaluate reusable CPU/process profiling state for profiler hotpath.
 *
...
UmiStatus umi_performance_profiler_hotpath_observe(UmiPerformanceProfilerHotpath *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_profiler_hotpath_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
