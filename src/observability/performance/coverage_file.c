/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/coverage_file.c
 *
 * PURPOSE:
 *   Implement represent code-coverage evidence, baselines and regressions for coverage file.
 *
...
UmiStatus umi_performance_coverage_file_observe(UmiPerformanceCoverageFile *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_coverage_file_validate(record);
    /* Preserve the original failure result so the caller can respond to the correct cause. */
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
