/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/metric_sample.c
 *
 * PURPOSE:
 *   Implement represent typed metric metadata, samples and time-series state for metric sample.
 *
...
UmiStatus umi_performance_metric_sample_observe(UmiPerformanceMetricSample *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_metric_sample_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
