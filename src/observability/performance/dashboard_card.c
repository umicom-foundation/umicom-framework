/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/dashboard_card.c
 *
 * PURPOSE:
 *   Implement represent toolkit-neutral operational dashboard composition for dashboard card.
 *
...
UmiStatus umi_performance_dashboard_card_observe(UmiPerformanceDashboardCard *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_dashboard_card_validate(record);
    /* Preserve the original failure result so the caller can respond to the correct cause. */
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
