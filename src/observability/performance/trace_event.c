/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/trace_event.c
 *
 * PURPOSE:
 *   Implement represent distributed/runtime trace state and filtering policy for trace event.
 *
...
UmiStatus umi_performance_trace_event_observe(UmiPerformanceTraceEvent *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_trace_event_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
