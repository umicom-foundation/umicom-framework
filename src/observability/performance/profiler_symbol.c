/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/profiler_symbol.c
 *
 * PURPOSE:
 *   Implement represent and evaluate reusable CPU/process profiling state for profiler symbol.
 *
...
UmiStatus umi_performance_profiler_symbol_observe(UmiPerformanceProfilerSymbol *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_profiler_symbol_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
