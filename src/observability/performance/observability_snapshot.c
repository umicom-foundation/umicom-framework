/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/observability_snapshot.c
 *
 * PURPOSE:
 *   Implement represent immutable point-in-time observability snapshots for observability snapshot.
 *
...
UmiStatus umi_performance_observability_snapshot_observe(UmiPerformanceObservabilitySnapshot *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_observability_snapshot_validate(record);
    /* Preserve the original failure result so the caller can respond to the correct cause. */
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
...
/* Produce a deterministic fingerprint for this evidence domain. */
uint64_t umi_performance_observability_snapshot_fingerprint(const char *text) {
    return umi_performance_hash_text(text);
}
