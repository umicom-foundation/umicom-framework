/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/crash_cluster.c
 *
 * PURPOSE:
 *   Implement represent crash signatures, correlations and clustering evidence for crash cluster.
 *
...
UmiStatus umi_performance_crash_cluster_observe(UmiPerformanceCrashCluster *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_crash_cluster_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
