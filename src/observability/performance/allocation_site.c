/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/allocation_site.c
 *
 * PURPOSE:
 *   Implement represent memory allocation evidence and allocation aggregation state for allocation site.
 *
...
UmiStatus umi_performance_allocation_site_observe(UmiPerformanceAllocationSite *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_allocation_site_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
