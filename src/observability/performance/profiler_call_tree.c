/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/profiler_call_tree.c
 *
 * PURPOSE:
 *   Implement represent and evaluate reusable CPU/process profiling state for profiler call tree.
 *
...
UmiStatus umi_performance_profiler_call_tree_observe(UmiPerformanceProfilerCallTree *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_profiler_call_tree_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
