/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/runtime_health.c
 *
 * PURPOSE:
 *   Implement summarise runtime health from performance evidence for runtime health.
 *
...
UmiStatus umi_performance_runtime_health_observe(UmiPerformanceRuntimeHealth *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_runtime_health_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
