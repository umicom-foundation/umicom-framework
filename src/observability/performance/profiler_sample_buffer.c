/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/profiler_sample_buffer.c
 *
 * PURPOSE:
 *   Implement represent and evaluate reusable CPU/process profiling state for profiler sample buffer.
 *
...
UmiStatus umi_performance_profiler_sample_buffer_observe(UmiPerformanceProfilerSampleBuffer *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_profiler_sample_buffer_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
...
/* Report whether a bounded collection can accept another item. */
