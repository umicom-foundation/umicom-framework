/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/benchmark_case.c
 *
 * PURPOSE:
 *   Implement represent benchmark cases, runs, baselines and statistics for benchmark case.
 *
...
UmiStatus umi_performance_benchmark_case_observe(UmiPerformanceBenchmarkCase *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_benchmark_case_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
