/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/trace_filter.c
 *
 * PURPOSE:
 *   Implement represent distributed/runtime trace state and filtering policy for trace filter.
 *
...
UmiStatus umi_performance_trace_filter_observe(UmiPerformanceTraceFilter *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_trace_filter_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
