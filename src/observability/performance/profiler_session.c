/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/profiler_session.c
 *
 * PURPOSE:
 *   Implement represent and evaluate reusable CPU/process profiling state for profiler session.
 *
...
UmiStatus umi_performance_profiler_session_observe(UmiPerformanceProfilerSession *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_profiler_session_validate(record);
    /* Preserve the original failure result so the caller can respond to the correct cause. */
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
