/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/observability/performance/timeline.c
 *
 * PURPOSE:
 *   Implement represent ordered operational timeline evidence for timeline.
 *
...
UmiStatus umi_performance_timeline_observe(UmiPerformanceTimeline *record, double value, double auxiliary, uint64_t count, uint64_t timestamp_ns) {
    UmiStatus status = umi_performance_timeline_validate(record);
    if (status != UMI_STATUS_OK) return status;
    record->value = value;
