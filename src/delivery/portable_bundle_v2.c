/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/delivery/portable_bundle_v2.c
 *
 * PURPOSE:
                                       ...
                                       const UmiPackageManifestV2 *manifest)
{
    ...
    umi_runtime_dependency_report_v2_init(&plan->dependencies);
    ...
    int resolved)
{
    ...
    return umi_runtime_dependency_report_v2_add(&plan->dependencies, name,
...
UmiStatus umi_portable_bundle_v2_validate(const UmiPortableBundlePlanV2 *plan)
{
    ...
    if (plan->bundle_id[0] == '\0' || plan->platform_id[0] == '\0' ||
        ...
        umi_package_manifest_v2_validate(&plan->manifest) != UMI_STATUS_OK ||
        !umi_runtime_dependency_report_v2_ready(&plan->dependencies)) {
...
size_t umi_portable_bundle_v2_file_count(const UmiPortableBundlePlanV2 *plan)
{
    ...
    for (index = 0U; index < plan->dependencies.count; ++index) {
        if (plan->dependencies.dependencies[index].disposition ==
            UMI_DEPENDENCY_BUNDLE_V2) ++count;
    }
    return count;
}
