 ...
 * a known state.
 */
void umi_dr_update_transaction_init(UmiDrUpdateTransaction*t){/* Protect caller-owned memory by checking that required state is available before it is used. */ if(t!=NULL){*t=(UmiDrUpdateTransaction){0};t->state=UMI_DR_STATE_PLANNED;}}UmiStatus umi_dr_update_transaction_stage(UmiDrUpdateTransaction*t){/* Protect caller-owned memory by checking that required state is available before it is used. */ if(t==NULL||t->state!=UMI_DR_STATE_PLANNED||!t->checkpoint_ready)return UMI_STATUS_INVALID_STATE;t->files_staged=true;t->state=UMI_DR_STATE_READY;return UMI_STATUS_OK;}UmiStatus umi_dr_update_transaction_commit(UmiDrUpdateTransaction*t){/* Protect caller-owned memory by checking that required state is available before it is used. */ if(t==NULL||t->state!=UMI_DR_STATE_READY||!t->health_verified)return UMI_STATUS_INVALID_STATE;t->state=UMI_DR_STATE_COMMITTED;return UMI_STATUS_OK;}UmiStatus umi_dr_update_transaction_rollback(UmiDrUpdateTransaction*t){/* Protect caller-owned memory by checking that required state is available before it is used. */ if(t==NULL||!t->checkpoint_ready)return UMI_STATUS_INVALID_STATE;t->state=UMI_DR_STATE_ROLLED_BACK;return UMI_STATUS_OK;}
