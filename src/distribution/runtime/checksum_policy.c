 ...
 * known state.
 */
void umi_dr_checksum_policy_init(UmiDrChecksumPolicy*p){/* Protect caller-owned memory by checking that required state is available before it is used. */ if(p!=NULL){*p=(UmiDrChecksumPolicy){0};(void)umi_dr_copy_text(p->algorithm,sizeof(p->algorithm),"sha256");p->digest_required=true;p->match_required=true;}}bool umi_dr_checksum_policy_accept(const UmiDrChecksumPolicy*p,const char*e,const char*a){/* Protect caller-owned memory by checking that required state is available before it is used. */ if(p==NULL)return false;/* Protect caller-owned memory by checking that required state is available before it is used. */ if(p->digest_required&&(e==NULL||e[0]=='\0'||a==NULL||a[0]=='\0'))return false;/* Protect caller-owned memory by checking that required state is available before it is used. */ if(p->match_required&&e!=NULL&&a!=NULL&&strcmp(e,a)!=0)return false;return true;}
