/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/developer_productivity/diagnostic_parsers/clang.c
 *
 * PURPOSE:
 *   Normalize Clang output into the shared Framework Problems model.
 *
...
const UmiDeveloperDiagnosticParser *umi_developer_diagnostic_parser_clang(void)
{
    static const UmiDeveloperDiagnosticParser parser = {
        ...
        .parser_id = "developer.parser.clang",
        .title = "Clang",
        .tool_id = "clang",
        .priority = 100,
        .parse = parse_line
