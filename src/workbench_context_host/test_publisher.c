    ...
    const char *source_uri,uint64_t source_line,uint64_t duration_ms,uint64_t now_ms)
{
    ...
    s=umi_workbench_context_link_build_selection(&p,context_id,host->application_id,
        source_panel_id?source_panel_id:"test-explorer",test_id,"test",test_id);if(s!=UMI_STATUS_OK)return s;
    s=umi_workbench_context_host_payload_add_text(&p,"suite-id",suite_id);if(s!=UMI_STATUS_OK)return s;
    s=umi_workbench_context_host_payload_add_text(&p,"outcome",outcome);if(s!=UMI_STATUS_OK)return s;
    s=umi_workbench_context_host_payload_add_text(&p,"source-uri",source_uri);if(s!=UMI_STATUS_OK)return s;
    s=umi_workbench_context_host_payload_add_unsigned(&p,"source-line",source_line);if(s!=UMI_STATUS_OK)return s;
    s=umi_workbench_context_host_payload_add_unsigned(&p,"duration-ms",duration_ms);if(s!=UMI_STATUS_OK)return s;
