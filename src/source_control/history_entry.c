/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/source_control/history_entry.c
 *
 * PURPOSE:
...
struct UmiSourceControlHistoryEntryRegistry {
    UmiSourceControlHistoryEntrySnapshot items[UMI_SOURCE_CONTROL_HISTORY_ENTRY_CAPACITY];
    size_t count;
    uint64_t revision;
};
...
UmiStatus umi_source_control_history_entry_registry_upsert(UmiSourceControlHistoryEntryRegistry *registry, const UmiSourceControlHistoryEntrySnapshot *item)
{
    ...
    registry->items[index].api_version = UMI_SOURCE_CONTROL_HISTORY_ENTRY_API_VERSION;
    registry->items[index].id[127U] = '\0';
    registry->items[index].repository_id[127U] = '\0';
    registry->items[index].revision_id[127U] = '\0';
    registry->items[index].summary[511U] = '\0';
