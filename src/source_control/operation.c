/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/source_control/operation.c
 *
 * PURPOSE:
...
struct UmiSourceControlOperationRegistry {
    UmiSourceControlOperationSnapshot items[UMI_SOURCE_CONTROL_OPERATION_CAPACITY];
    size_t count;
    uint64_t revision;
};

static size_t find_index(const UmiSourceControlOperationRegistry *registry, const char *id)
{
    size_t i;
...
{
    UmiSourceControlOperationRegistry *registry;
    if (out_registry == NULL) return UMI_STATUS_INVALID_ARGUMENT;
    *out_registry = NULL;
    registry = (UmiSourceControlOperationRegistry *)calloc(1U, sizeof(*registry));
...
void umi_source_control_operation_registry_destroy(UmiSourceControlOperationRegistry *registry) { free(registry); }
...
UmiStatus umi_source_control_operation_registry_upsert(UmiSourceControlOperationRegistry *registry, const UmiSourceControlOperationSnapshot *item)
{
    ...
    if (index == SIZE_MAX) {
        if (registry->count >= UMI_SOURCE_CONTROL_OPERATION_CAPACITY) return UMI_STATUS_CAPACITY_EXCEEDED;
        index = registry->count++;
    }
    registry->items[index] = *item;
    registry->items[index].struct_size = (uint32_t)sizeof(registry->items[index]);
    registry->items[index].api_version = UMI_SOURCE_CONTROL_OPERATION_API_VERSION;
...
size_t umi_source_control_operation_registry_count(const UmiSourceControlOperationRegistry *registry) { return registry != NULL ? registry->count : 0U; }
uint64_t umi_source_control_operation_registry_revision(const UmiSourceControlOperationRegistry *registry) { return registry != NULL ? registry->revision : 0U; }
