...
for(i=0U;i<store->count;++i)if(strcmp(store->items[i].contribution_id,identity)==0)return &store->items[i];
return NULL;
}
...
for(i=0U;i<store->count;++i)if(strcmp(store->items[i].contribution_id,identity)==0){
