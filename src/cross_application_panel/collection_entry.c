...
for(i=0U;i<store->count;++i)if(strcmp(store->items[i].entry_id,identity)==0)return &store->items[i];
return NULL;
}
...
for(i=0U;i<store->count;++i)if(strcmp(store->items[i].entry_id,identity)==0){
