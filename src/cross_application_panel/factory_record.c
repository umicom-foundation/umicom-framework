/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/cross_application_panel/factory_record.c
 *
 * PURPOSE:
 *   Implement cross-application panel factory record validation and storage.
 *
...
if(record==NULL||record->structure_size!=sizeof(*record))return UMI_STATUS_INVALID_ARGUMENT;
    ...
    if(!umi_context_text_is_valid(record->factory_id,sizeof(record->factory_id)))return UMI_STATUS_INVALID_ARGUMENT;
    /* Preserve the original failure result so the caller can respond to the correct cause. */
    ...
    /* Preserve the original failure result so the caller can respond to the correct cause. */
    if(record->factory_id[0]=='\0')return UMI_STATUS_INVALID_ARGUMENT;
...
for(i=0U;i<store->count;++i)/* Keep the operation inside its valid bounds before reading, writing or adding data. */ if(strcmp(store->items[i].factory_id,identity)==0)return &store->items[i];
return NULL;
}
/*
...
UmiPanelFactoryRecord *existing;
uint64_t next;
/*
 * Protect caller-owned memory by checking that required state is available before it is
 * used.
...
/* Preserve the original failure result so the caller can respond to the correct cause. */
if(umi_panel_factory_record_validate(record)!=UMI_STATUS_OK)return UMI_STATUS_INVALID_ARGUMENT;
existing=umi_panel_factory_record_store_find(store,record->factory_id);
/*
 * Protect caller-owned memory by checking that required state is available before it is
 * used.
 */
...
/* Visit each bounded item once so every record receives the same rule. */
for(i=0U;i<store->count;++i)/* Keep the operation inside its valid bounds before reading, writing or adding data. */ if(strcmp(store->items[i].factory_id,identity)==0){
/* Keep the operation inside its valid bounds before reading, writing or adding data. */
