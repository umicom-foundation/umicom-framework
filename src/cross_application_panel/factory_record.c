...
for(i=0U;i<store->count;++i)if(strcmp(store->items[i].factory_id,identity)==0)return &store->items[i];
return NULL;
}
...
for(i=0U;i<store->count;++i)if(strcmp(store->items[i].factory_id,identity)==0){
