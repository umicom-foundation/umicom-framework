/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/cross_application_panel/bookmark.c
 *
 * PURPOSE:
 *   Implement cross-application panel bookmark validation and storage.
 *
...
for(i=0U;i<store->count;++i)/* Keep the operation inside its valid bounds before reading, writing or adding data. */ if(strcmp(store->items[i].bookmark_id,identity)==0)return &store->items[i];
return NULL;
}
/*
...
/* Preserve the original failure result so the caller can respond to the correct cause. */
if(umi_panel_bookmark_validate(record)!=UMI_STATUS_OK)return UMI_STATUS_INVALID_ARGUMENT;
existing=umi_panel_bookmark_store_find(store,record->bookmark_id);
/*
 * Protect caller-owned memory by checking that required state is available before it is
 * used.
 */
...
/* Visit each bounded item once so every record receives the same rule. */
for(i=0U;i<store->count;++i)/* Keep the operation inside its valid bounds before reading, writing or adding data. */ if(strcmp(store->items[i].bookmark_id,identity)==0){
/* Keep the operation inside its valid bounds before reading, writing or adding data. */
