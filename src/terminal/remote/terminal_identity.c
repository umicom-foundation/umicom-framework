 ...
 */
void umi_terminal_remote_terminal_identity_init(UmiTerminalRemoteTerminalIdentity *value,const char *id,uint64_t limit) { /* Keep the operation inside its valid bounds before reading, writing or adding data. */ if(value) { umi_terminal_remote_named_entry_init(&value->identity,id,"terminal identity"); value->metric=0U; value->limit=limit; } }
/*
 * Provide the terminal remote terminal identity set metric operation used by this module
 * and its client applications.
 */
UmiStatus umi_terminal_remote_terminal_identity_set_metric(UmiTerminalRemoteTerminalIdentity *value,uint64_t metric) { /* Preserve the original failure result so the caller can respond to the correct cause. */ if(!value||!umi_terminal_remote_named_entry_valid(&value->identity)) return UMI_STATUS_INVALID_ARGUMENT; value->metric=metric; value->identity.revision++; return UMI_STATUS_OK; }
/*
 ...
 */
uint64_t umi_terminal_remote_terminal_identity_score(const UmiTerminalRemoteTerminalIdentity *value) { /* Keep the operation inside its valid bounds before reading, writing or adding data. */ if(!umi_terminal_remote_terminal_identity_within_limit(value)) return 0U; return umi_terminal_remote_fingerprint_text(value->identity.id) ^ value->metric ^ (value->limit<<1U); }
