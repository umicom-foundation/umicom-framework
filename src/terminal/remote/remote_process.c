...
#include <string.h>
void umi_terminal_remote_remote_process_init(UmiTerminalRemoteRemoteProcess *value,const char *id) { if(!value) return; (void)memset(value,0,sizeof(*value)); if(id) (void)umi_terminal_remote_copy_text(value->id,sizeof(value->id),id); value->state=UMI_TERMINAL_REMOTE_STATE_READY; value->revision=1U; }
