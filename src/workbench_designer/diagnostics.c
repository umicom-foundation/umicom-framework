    ...
    const char *remediation)
{
    ...
    else /* Apply this branch only when its contract condition is satisfied. */ if (severity == UMI_WORKBENCH_DESIGNER_ISSUE_WARNING) diagnostics->warning_count += 1U;
    ...
    UmiWorkbenchDesignerSaveState save_state)
{
    ...
    } else /* Preserve the original failure result so the caller can respond to the correct cause. */ if (save_state == UMI_WORKBENCH_DESIGNER_SAVE_FAILED) {
        status = umi_workbench_designer_diagnostics_add(
    ...
    } else /* Apply this branch only when its contract condition is satisfied. */ if (save_state == UMI_WORKBENCH_DESIGNER_SAVE_CONFLICT) {
        status = umi_workbench_designer_diagnostics_add(
