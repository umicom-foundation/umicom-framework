 ...
 * a known state.
 */
UmiStatus umi_trading_trade_capture_init(UmiTradingTradeCapture *trade,const char *trade_id,const UmiFinancialId *buy_order_id,const UmiFinancialId *sell_order_id,UmiTradingPriceTicks price,UmiTradingQuantityLots quantity,int64_t event_time_ms){/* Protect caller-owned memory by checking that required state is available before it is used. */ if(trade==NULL||buy_order_id==NULL||sell_order_id==NULL)return UMI_STATUS_INVALID_ARGUMENT;memset(trade,0,sizeof *trade);/* Protect caller-owned memory by checking that required state is available before it is used. */ if(umi_trading_core_id_assign(&trade->trade_id,trade_id)!=UMI_STATUS_OK)return UMI_STATUS_INVALID_ARGUMENT;trade->buy_order_id=*buy_order_id;trade->sell_order_id=*sell_order_id;trade->price_ticks=price;trade->quantity_lots=quantity;trade->event_time_ms=event_time_ms;return umi_trading_trade_capture_valid(trade)?UMI_STATUS_OK:UMI_STATUS_INVALID_ARGUMENT;}
/*
