...
UmiTradingCoreDecision umi_trading_order_validation_check(const UmiTradingOrderInstruction *instruction,const UmiTradingTickSizeRule *tick_rule,const UmiTradingLotSizeRule *lot_rule,const UmiTradingPriceBand *band){if(!umi_trading_order_instruction_valid(instruction)||!umi_trading_tick_size_rule_valid(tick_rule)||!umi_trading_lot_size_rule_valid(lot_rule))return decision(false,"invalid-input");if(instruction->quantity_lots<lot_rule->minimum_lots||instruction->quantity_lots>lot_rule->maximum_lots||((instruction->quantity_lots-lot_rule->minimum_lots)%lot_rule->step_lots)!=0)return decision(false,"invalid-lot");if(instruction->limit_ticks>0&&(instruction->limit_ticks%tick_rule->tick_size)!=0)return decision(false,"invalid-tick");if(instruction->limit_ticks>0&&band!=NULL&&!umi_trading_price_band_contains(band,instruction->limit_ticks))return decision(false,"outside-band");return decision(true,"accepted");}
