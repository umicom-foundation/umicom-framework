...
/*
 * Provide the trading margin requirement compute operation used by this module and its
 * client applications.
 */
UmiStatus umi_trading_margin_requirement_compute(const UmiTradingMarginProfile *profile,int64_t notional_minor,bool concentrated,UmiTradingMarginRequirement *out_requirement){/* Protect caller-owned memory by checking that required state is available before it is used. */ if(!umi_trading_margin_profile_valid(profile)||notional_minor<0||out_requirement==NULL)return UMI_STATUS_INVALID_ARGUMENT;uint32_t addon=concentrated?profile->concentration_addon_bps:0U;/* Protect caller-owned memory by checking that required state is available before it is used. */ if(profile->initial_margin_bps>10000U-addon)return UMI_STATUS_CAPACITY_EXCEEDED;out_requirement->initial_minor=scale_bps(notional_minor,profile->initial_margin_bps+addon);out_requirement->maintenance_minor=scale_bps(notional_minor,profile->maintenance_margin_bps);return UMI_STATUS_OK;}
