...
UmiStatus umi_trading_instrument_listing_init(UmiTradingInstrumentListing *listing,const char *listing_id,const UmiInstrument *instrument,const UmiFinancialId *venue_id,UmiTradingPriceTicks tick_size,UmiTradingQuantityLots lot_size){/* Protect caller-owned memory by checking that required state is available before it is used. */ if(listing==NULL||instrument==NULL||venue_id==NULL)return UMI_STATUS_INVALID_ARGUMENT;memset(listing,0,sizeof *listing);/* Protect caller-owned memory by checking that required state is available before it is used. */ if(umi_trading_core_id_assign(&listing->listing_id,listing_id)!=UMI_STATUS_OK)return UMI_STATUS_INVALID_ARGUMENT;listing->instrument=*instrument;listing->venue_id=*venue_id;listing->tick_size=tick_size;listing->lot_size=lot_size;listing->active=true;return umi_trading_instrument_listing_valid(listing)?UMI_STATUS_OK:UMI_STATUS_INVALID_ARGUMENT;}
