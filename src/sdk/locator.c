...
UmiStatus umi_sdk_locate(const char *preferred,char *out,size_t cap){const char *candidates[]={preferred,"C:/Dev/umicom/sdk/umicom-framework-0.5.0-local","/usr/local","/usr"};size_t i;int n;if(out==NULL||cap==0U)return UMI_STATUS_INVALID_ARGUMENT;for(i=0U;i<sizeof(candidates)/sizeof(candidates[0]);++i){if(candidates[i]!=NULL&&valid(candidates[i])){n=snprintf(out,cap,"%s",candidates[i]);return n<0||(size_t)n>=cap?UMI_STATUS_CAPACITY_EXCEEDED:UMI_STATUS_OK;}}return UMI_STATUS_NOT_FOUND;}
