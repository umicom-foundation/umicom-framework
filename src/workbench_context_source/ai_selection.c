/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_source/ai_selection.c
 *
 * PURPOSE:
 *   Implement validation, bounded mutation and stable hashing for the AI selection snapshot.
 *
...
#include "umicom/workbench_context_source/ai_selection.h"

#include <string.h>

/*
 * Initialise workbench context source ai selection from caller-provided values so later
...
/*
 * Check that workbench context source ai selection satisfies its contract before another
...
UmiStatus function_name(UmiWorkbenchContextSourceAiSelection *record, const char *value) \
{ \
    UmiStatus status; \
...
UMI_CONTEXT_SOURCE_SETTER(
    umi_workbench_context_source_ai_selection_set_source, source_id)
UMI_CONTEXT_SOURCE_SETTER(
    umi_workbench_context_source_ai_selection_set_panel, panel_id)
UMI_CONTEXT_SOURCE_SETTER(
    umi_workbench_context_source_ai_selection_set_subject, subject_id)
UMI_CONTEXT_SOURCE_SETTER(
    umi_workbench_context_source_ai_selection_set_group, group_id)
UMI_CONTEXT_SOURCE_SETTER(
    umi_workbench_context_source_ai_selection_set_label, label)

#undef UMI_CONTEXT_SOURCE_SETTER

/*
 * Provide the workbench context source ai selection hash operation used by this module and
...
/*
 * Provide the workbench context source ai selection touch operation used by this module
