/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_selection_provider/trace_record.c
 *
 * PURPOSE:
 *   Implement bounded mutation, validation and hashing for the provider trace record.
 *
...
/*
 * Initialise workbench selection provider trace record from caller-provided values so
...
/*
 * Check that workbench selection provider trace record satisfies its contract before
...
UmiStatus fn(UmiWorkbenchSelectionProviderTraceRecord *record, const char *value) \
{ \
    UmiStatus status; \
...
UMI_PROVIDER_SETTER(
    umi_workbench_selection_provider_trace_record_set_provider,
    provider_id)
UMI_PROVIDER_SETTER(
    umi_workbench_selection_provider_trace_record_set_source,
    source_id)
UMI_PROVIDER_SETTER(
    umi_workbench_selection_provider_trace_record_set_subject,
    subject_id)
UMI_PROVIDER_SETTER(
    umi_workbench_selection_provider_trace_record_set_related,
    related_id)
UMI_PROVIDER_SETTER(
    umi_workbench_selection_provider_trace_record_set_group,
    group_id)
UMI_PROVIDER_SETTER(
    umi_workbench_selection_provider_trace_record_set_description,
    description)

#undef UMI_PROVIDER_SETTER

/*
 * Provide the workbench selection provider trace record hash operation used by this module
...
/*
 * Provide the workbench selection provider trace record touch operation used by this
