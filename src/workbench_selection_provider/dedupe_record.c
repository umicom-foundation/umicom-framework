/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_selection_provider/dedupe_record.c
 *
 * PURPOSE:
 *   Implement bounded mutation, validation and hashing for the provider deduplication record.
 *
...
/*
 * Initialise workbench selection provider dedupe record from caller-provided values so
...
/*
 * Check that workbench selection provider dedupe record satisfies its contract before
...
UmiStatus fn(UmiWorkbenchSelectionProviderDedupeRecord *record, const char *value) \
{ \
    UmiStatus status; \
...
UMI_PROVIDER_SETTER(
    umi_workbench_selection_provider_dedupe_record_set_provider,
    provider_id)
UMI_PROVIDER_SETTER(
    umi_workbench_selection_provider_dedupe_record_set_source,
    source_id)
UMI_PROVIDER_SETTER(
    umi_workbench_selection_provider_dedupe_record_set_subject,
    subject_id)
UMI_PROVIDER_SETTER(
    umi_workbench_selection_provider_dedupe_record_set_related,
    related_id)
UMI_PROVIDER_SETTER(
    umi_workbench_selection_provider_dedupe_record_set_group,
    group_id)
UMI_PROVIDER_SETTER(
    umi_workbench_selection_provider_dedupe_record_set_description,
    description)

#undef UMI_PROVIDER_SETTER

/*
 * Provide the workbench selection provider dedupe record hash operation used by this
...
/*
 * Provide the workbench selection provider dedupe record touch operation used by this
 * module and its client applications.
 */
