...
UmiStatus umi_data_backup_manifest_init(UmiDataBackupManifest *item, const char *backup_id, uint64_t created_at, uint64_t schema_fingerprint, uint64_t content_fingerprint, uint64_t bytes_written) {
    ...
    UmiStatus s=umi_data_enterprise_copy_text(item->backup_id,sizeof(item->backup_id),backup_id);if(s!=UMI_STATUS_OK)return s;item->created_at=created_at;item->schema_fingerprint=schema_fingerprint;item->content_fingerprint=content_fingerprint;item->bytes_written=bytes_written;item->complete=true;
