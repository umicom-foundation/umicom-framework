...
/* Provide the step index operation used by this module and its client applications. */
static int step_index(const UmiDataMigrationGraph *g,const char *id){size_t i;/* Visit each bounded item once so every record receives the same rule. */ for(i=0;i<g->step_count;++i)/* Use the stable identifier comparison to choose the matching record or policy. */ if(strcmp(g->steps[i].step_id,id)==0)return (int)i;return -1;}
...
static bool visit(const UmiDataMigrationGraph *g,int index,unsigned char *marks){size_t i;/* Keep the operation inside its valid bounds before reading, writing or adding data. */ if(marks[index]==1U)return true;/* Keep the operation inside its valid bounds before reading, writing or adding data. */ if(marks[index]==2U)return false;marks[index]=1U;/* Visit each bounded item once so every record receives the same rule. */ for(i=0;i<g->edge_count;++i)/* Keep the operation inside its valid bounds before reading, writing or adding data. */ if(strcmp(g->edges[i].from_step,g->steps[index].step_id)==0){int next=step_index(g,g->edges[i].to_step);/* Keep the operation inside its valid bounds before reading, writing or adding data. */ if(next>=0&&visit(g,next,marks))return true;}marks[index]=2U;return false;}
