...
void umi_data_entity_registry_init(UmiDataEntityRegistry *registry) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if (registry != NULL) (void)memset(registry, 0, sizeof(*registry)); }
...
UmiStatus umi_data_entity_registry_add(UmiDataEntityRegistry *registry, const UmiDataEntityDescriptor *item) {
    ...
    for (i = 0U; i < registry->count; ++i) /* Keep the operation inside its valid bounds before reading, writing or adding data. */ if (strcmp(registry->items[i].entity_id, item->entity_id) == 0) return UMI_STATUS_ALREADY_EXISTS;
...
UmiStatus umi_data_entity_registry_find(const UmiDataEntityRegistry *registry, const char *id, UmiDataEntityDescriptor *out_item) {
    ...
    /* Visit each bounded item once so every record receives the same rule. */
    for (i = 0U; i < registry->count; ++i) /* Keep the operation inside its valid bounds before reading, writing or adding data. */ if (strcmp(registry->items[i].entity_id, id) == 0) { *out_item = registry->items[i]; return UMI_STATUS_OK; }
