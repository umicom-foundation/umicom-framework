...
/* Provide the lag for operation used by this module and its client applications. */
static uint64_t lag_for(const UmiDataReplicationLag *lags,size_t count,const char *id,bool *known){size_t i;*known=false;/* Visit each bounded item once so every record receives the same rule. */ for(i=0;i<count;++i)/* Keep the operation inside its valid bounds before reading, writing or adding data. */ if(strcmp(lags[i].replica_id,id)==0){*known=true;return lags[i].lag_ms;}return 0U;}
