...
void umi_data_replica_set_init(UmiDataReplicaSet *registry) { if (registry != NULL) (void)memset(registry, 0, sizeof(*registry)); }
...
UmiStatus umi_data_replica_set_add(UmiDataReplicaSet *registry, const UmiDataReplicaDescriptor *item) {
    ...
    for (i = 0U; i < registry->count; ++i) if (strcmp(registry->items[i].replica_id, item->replica_id) == 0) return UMI_STATUS_ALREADY_EXISTS;
...
UmiStatus umi_data_replica_set_find(const UmiDataReplicaSet *registry, const char *id, UmiDataReplicaDescriptor *out_item) {
    ...
    for (i = 0U; i < registry->count; ++i) if (strcmp(registry->items[i].replica_id, id) == 0) { *out_item = registry->items[i]; return UMI_STATUS_OK; }
