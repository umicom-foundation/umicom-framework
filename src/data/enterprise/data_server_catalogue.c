...
void umi_data_data_server_catalogue_init(UmiDataServerCatalogue *registry) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if (registry != NULL) (void)memset(registry, 0, sizeof(*registry)); }
...
UmiStatus umi_data_data_server_catalogue_add(UmiDataServerCatalogue *registry, const UmiDataServerProfile *item) {
    ...
    for (i = 0U; i < registry->count; ++i) /* Keep the operation inside its valid bounds before reading, writing or adding data. */ if (strcmp(registry->items[i].profile_id, item->profile_id) == 0) return UMI_STATUS_ALREADY_EXISTS;
...
UmiStatus umi_data_data_server_catalogue_find(const UmiDataServerCatalogue *registry, const char *id, UmiDataServerProfile *out_item) {
    ...
    /* Visit each bounded item once so every record receives the same rule. */
    for (i = 0U; i < registry->count; ++i) /* Keep the operation inside its valid bounds before reading, writing or adding data. */ if (strcmp(registry->items[i].profile_id, id) == 0) { *out_item = registry->items[i]; return UMI_STATUS_OK; }
