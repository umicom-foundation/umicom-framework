/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/product/marketplace.c
 *
 * PURPOSE:
 *   Implement a provider-neutral marketplace catalogue for Umicom applications, plug-ins and templates.
 *
...
struct UmiProductMarketplaceItemRegistry {
    UmiProductMarketplaceItemSnapshot items[UMI_PRODUCT_MARKETPLACE_CAPACITY];
    size_t count;
    uint64_t revision;
};

static size_t find_index(const UmiProductMarketplaceItemRegistry *registry, const char *id)
{
    size_t i;
...
{
    UmiProductMarketplaceItemRegistry *registry;
    if (out_registry == NULL) return UMI_STATUS_INVALID_ARGUMENT;
    *out_registry = NULL;
    registry = (UmiProductMarketplaceItemRegistry *)calloc(1U, sizeof(*registry));
...
UmiStatus umi_product_marketplace_registry_upsert(UmiProductMarketplaceItemRegistry *registry, const UmiProductMarketplaceItemSnapshot *item)
{
    ...
    if (index == SIZE_MAX) {
        if (registry->count >= UMI_PRODUCT_MARKETPLACE_CAPACITY) return UMI_STATUS_CAPACITY_EXCEEDED;
        index = registry->count++;
    }
    registry->items[index] = *item;
    registry->items[index].struct_size = (uint32_t)sizeof(UmiProductMarketplaceItemSnapshot);
