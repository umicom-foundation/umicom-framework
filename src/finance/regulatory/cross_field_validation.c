...
#include <math.h>
#include <string.h>
UmiStatus umi_reg_cross_field_validation_init(UmiCrossFieldValidation *rule,const char *rule_id,double tolerance){ UmiStatus s; if(rule==NULL||!umi_reg_number_valid(tolerance)||tolerance<0.0)return UMI_STATUS_INVALID_ARGUMENT; memset(rule,0,sizeof *rule); s=umi_reg_copy_text(rule->rule_id,sizeof rule->rule_id,rule_id); if(s==UMI_STATUS_OK)rule->tolerance=tolerance; return s; }
