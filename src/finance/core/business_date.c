...
UmiStatus umi_business_date_serial(UmiFinancialDate d,int64_t *out){if(out==NULL||!umi_financial_date_is_valid(d))return UMI_STATUS_INVALID_ARGUMENT;*out=days_from_civil(d.year,d.month,d.day);return UMI_STATUS_OK;}
/* Convert serial days back to a valid date. */
UmiStatus umi_business_date_from_serial(int64_t serial,UmiFinancialDate *out){UmiFinancialDate d;if(out==NULL)return UMI_STATUS_INVALID_ARGUMENT;d=civil_from_days(serial);if(!umi_financial_date_is_valid(d))return UMI_STATUS_INVALID_ARGUMENT;*out=d;return UMI_STATUS_OK;}
/* Add calendar days. */
UmiStatus umi_business_date_add(UmiFinancialDate d,int32_t days,UmiFinancialDate *out){int64_t s;if(umi_business_date_serial(d,&s)!=UMI_STATUS_OK)return UMI_STATUS_INVALID_ARGUMENT;return umi_business_date_from_serial(s+(int64_t)days,out);}
/* Return Monday=1 through Sunday=7. */
UmiStatus umi_business_date_weekday(UmiFinancialDate d,uint8_t *out){int64_t s,r;if(out==NULL||umi_business_date_serial(d,&s)!=UMI_STATUS_OK)return UMI_STATUS_INVALID_ARGUMENT;r=(s+3)%7;if(r<0)r+=7;*out=(uint8_t)(r+1);return UMI_STATUS_OK;}
