 ...
 */
void umi_treasury_risk_factor_catalogue_init(UmiTreasuryRiskFactorCatalogue *registry) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if (registry != NULL) memset(registry,0,sizeof *registry); }
/*
