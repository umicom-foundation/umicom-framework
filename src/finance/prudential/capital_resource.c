...
#include <string.h>
UmiStatus umi_pru_capital_resource_init(UmiPrudentialCapitalResource *record,const char *resource_id,UmiCapitalTierKind tier,double gross_amount,double deductions,UmiCurrency currency) { UmiStatus s; if(record==NULL||resource_id==NULL||!umi_pru_number_valid(gross_amount)||!umi_pru_number_valid(deductions)||gross_amount<0.0||deductions<0.0||deductions>gross_amount)return UMI_STATUS_INVALID_ARGUMENT; memset(record,0,sizeof *record); s=umi_pru_copy_text(record->resource_id,sizeof record->resource_id,resource_id); if(s!=UMI_STATUS_OK)return s; record->tier=tier; record->gross_amount=gross_amount; record->deductions=deductions; record->eligible_amount=gross_amount-deductions; record->currency=currency; return UMI_STATUS_OK; }
