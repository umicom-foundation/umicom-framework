...
UmiStatus umi_pru_survival_horizon_calculate(UmiPrudentialSurvivalHorizon *result,double opening,const double *flows,size_t count) { size_t i; double balance; if(result==NULL||flows==NULL||count==0U||count>UMI_PRU_MAX_PERIODS||opening<0.0)return UMI_STATUS_INVALID_ARGUMENT; balance=opening; result->opening_liquidity=opening; result->survived_days=0; result->exhausted=0; for(i=0U;i<count;++i) { if(!umi_pru_number_valid(flows[i]))return UMI_STATUS_INVALID_ARGUMENT; balance+=flows[i]; if(balance<0.0){result->exhausted=1;break;} ++result->survived_days; } result->closing_liquidity=balance; return UMI_STATUS_OK; }
