...
UmiStatus umi_pru_stress_shock_apply(UmiPrudentialStressShock *result,double base_value,double shock_rate) { if(result==NULL||!umi_pru_number_valid(base_value)||!umi_pru_number_valid(shock_rate)||shock_rate<-1.0)return UMI_STATUS_INVALID_ARGUMENT; result->base_value=base_value; result->shock_rate=shock_rate; result->stressed_value=base_value*(1.0+shock_rate); return umi_pru_number_valid(result->stressed_value)?UMI_STATUS_OK:UMI_STATUS_INVALID_STATE; }
