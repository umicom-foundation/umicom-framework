/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/test_platform/coverage.c
 *
 * PURPOSE:
...
struct UmiTestPlatformCoverageRegistry {
    UmiTestPlatformCoverageSnapshot items[UMI_TEST_PLATFORM_COVERAGE_CAPACITY];
    size_t count;
    uint64_t revision;
};

static size_t find_index(const UmiTestPlatformCoverageRegistry *registry, const char *id)
{
    size_t i;
...
{
    UmiTestPlatformCoverageRegistry *registry;
    if (out_registry == NULL) return UMI_STATUS_INVALID_ARGUMENT;
    *out_registry = NULL;
    registry = (UmiTestPlatformCoverageRegistry *)calloc(1U, sizeof(*registry));
...
void umi_test_platform_coverage_registry_destroy(UmiTestPlatformCoverageRegistry *registry) { free(registry); }
...
UmiStatus umi_test_platform_coverage_registry_upsert(UmiTestPlatformCoverageRegistry *registry, const UmiTestPlatformCoverageSnapshot *item)
{
    ...
    if (index == SIZE_MAX) {
        if (registry->count >= UMI_TEST_PLATFORM_COVERAGE_CAPACITY) return UMI_STATUS_CAPACITY_EXCEEDED;
        index = registry->count++;
    }
    registry->items[index] = *item;
    registry->items[index].struct_size = (uint32_t)sizeof(registry->items[index]);
    registry->items[index].api_version = UMI_TEST_PLATFORM_COVERAGE_API_VERSION;
...
size_t umi_test_platform_coverage_registry_count(const UmiTestPlatformCoverageRegistry *registry) { return registry != NULL ? registry->count : 0U; }
uint64_t umi_test_platform_coverage_registry_revision(const UmiTestPlatformCoverageRegistry *registry) { return registry != NULL ? registry->revision : 0U; }
