...
void umi_helix_plan_graph_init(UmiHelixPlanGraph *g){ if(g!=NULL) memset(g,0,sizeof(*g)); }
/* Add one unique bounded identifier. */
UmiStatus umi_helix_plan_graph_add(UmiHelixPlanGraph *g,const char *id){ if(g==NULL||!umi_helix_valid_id(id)){ return UMI_STATUS_INVALID_ARGUMENT; } if(g->count>=UMI_HELIX_MAX_ITEMS){ return UMI_STATUS_CAPACITY_EXCEEDED; } for(size_t i=0;i<g->count;i++) if(strcmp(g->ids[i],id)==0){ return UMI_STATUS_ALREADY_EXISTS; } UmiStatus s=umi_helix_copy_text(g->ids[g->count],sizeof(g->ids[g->count]),id); if(s==UMI_STATUS_OK) g->count++; return s; }
/* Record a prerequisite edge after validating both node indices. */
UmiStatus umi_helix_plan_graph_depend(UmiHelixPlanGraph *g,size_t a,size_t b){ if(g==NULL||a>=g->count||b>=g->count||a==b){ return UMI_STATUS_INVALID_ARGUMENT; } if(g->edge_count>=UMI_HELIX_MAX_EDGES){ return UMI_STATUS_CAPACITY_EXCEEDED; } for(size_t i=0;i<g->edge_count;i++) if(g->edges[i].from==a&&g->edges[i].to==b){ return UMI_STATUS_ALREADY_EXISTS; } g->edges[g->edge_count++]=(UmiHelixPlanGraphEdge){(uint16_t)a,(uint16_t)b}; return UMI_STATUS_OK; }
