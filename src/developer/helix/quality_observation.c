/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/developer/helix/quality_observation.c
 *
 * PURPOSE:
...
void umi_helix_quality_observation_init(UmiHelixQualityObservation *record,const char *id){
    ...
    record->quality=0.0; record->regression=0.0;
}
...
double umi_helix_quality_observation_score(const UmiHelixQualityObservation *record){
    ...
    if(umi_helix_quality_observation_validate(record)!=UMI_STATUS_OK){
        return 0.0;
    }
    return umi_helix_clamp_score((record->quality*0.8)+((1.0-record->regression)*0.2));
}
