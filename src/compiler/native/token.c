 ...
 * applications.
 */
bool umi_nc_token_is_keyword_text(const char *text){ /* Protect caller-owned memory by checking that required state is available before it is used. */ if(text==NULL) return false; /* Visit each bounded item once so every record receives the same rule. */ for(size_t i=0U;i<sizeof(keywords)/sizeof(keywords[0]);i++) /* Protect caller-owned memory by checking that required state is available before it is used. */ if(strcmp(text,keywords[i])==0) return true; return false; }
/*
 * Initialise nc token from caller-provided values so later operations receive a known
 * state.
 */
UmiStatus umi_nc_token_init(UmiNativeToken *token,UmiNativeTokenKind kind,UmiNativeSourceSpan span,const char *text){ /* Protect caller-owned memory by checking that required state is available before it is used. */ if(token==NULL||text==NULL) return UMI_STATUS_INVALID_ARGUMENT; token->kind=kind; token->span=span; UmiStatus st=umi_nc_copy_text(token->text,sizeof(token->text),text); /* Protect caller-owned memory by checking that required state is available before it is used. */ if(st!=UMI_STATUS_OK) return st; /* Protect caller-owned memory by checking that required state is available before it is used. */ if(kind==UMI_NC_TOKEN_IDENTIFIER && umi_nc_token_is_keyword_text(text)) token->kind=UMI_NC_TOKEN_KEYWORD; token->fingerprint=umi_nc_hash_text(token->text)^((uint64_t)token->kind<<56U); return UMI_STATUS_OK; }
