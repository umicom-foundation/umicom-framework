 ...
 * known state.
 */
UmiStatus umi_nc_ir_instruction_init(UmiNativeIrInstruction *i,uint32_t id,UmiNativeIrOpcode op,uint32_t result,UmiNativeIrType type){/* Protect caller-owned memory by checking that required state is available before it is used. */ if(i==NULL||id==0U||op>UMI_NC_IR_COPY)return UMI_STATUS_INVALID_ARGUMENT;memset(i,0,sizeof(*i));i->id=id;i->opcode=op;i->result_id=result;i->result_type=type;i->side_effect=(op==UMI_NC_IR_STORE||op==UMI_NC_IR_CALL||op==UMI_NC_IR_RET||op==UMI_NC_IR_BR||op==UMI_NC_IR_CBR);return UMI_STATUS_OK;}
/*
 * Provide the nc ir instruction add operand operation used by this module and its client
 * applications.
 */
UmiStatus umi_nc_ir_instruction_add_operand(UmiNativeIrInstruction *i,uint32_t v){/* Protect caller-owned memory by checking that required state is available before it is used. */ if(i==NULL||v==0U)return UMI_STATUS_INVALID_ARGUMENT;/* Protect caller-owned memory by checking that required state is available before it is used. */ if(i->operand_count>=UMI_NC_MAX_IR_OPERANDS)return UMI_STATUS_CAPACITY_EXCEEDED;i->operands[i->operand_count++]=v;return UMI_STATUS_OK;}
/*
