...
#include <string.h>
/*
 * Initialise project workspace application component selection from caller-provided values
...
/*
 * Provide the project workspace application component selection contains operation used by
...
/*
 * Add project workspace application component selection only after its inputs and
 * available capacity have been checked.
 */
UmiStatus umi_project_workspace_application_component_selection_add(UmiProjectWorkspaceApplicationComponentSelection *value, const char *text) {
    ...
    if(umi_project_workspace_application_component_selection_contains(value,text))return UMI_STATUS_ALREADY_EXISTS;
...
/*
 * Remove project workspace application component selection while keeping the remaining
