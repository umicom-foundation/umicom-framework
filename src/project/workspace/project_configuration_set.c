...
#include <string.h>
/*
 * Initialise project workspace project configuration set from caller-provided values so
...
/*
 * Provide the project workspace project configuration set contains operation used by this
...
/*
 * Add project workspace project configuration set only after its inputs and available
...
UmiStatus umi_project_workspace_project_configuration_set_add(UmiProjectWorkspaceProjectConfigurationSet *value, const char *text) {
    ...
    if(umi_project_workspace_project_configuration_set_contains(value,text))return UMI_STATUS_ALREADY_EXISTS;
...
/*
 * Remove project workspace project configuration set while keeping the remaining records
