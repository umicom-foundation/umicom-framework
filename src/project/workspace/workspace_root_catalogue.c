...
#include <string.h>
/*
 * Initialise project workspace workspace root catalogue from caller-provided values so
...
/*
 * Provide the project workspace workspace root catalogue contains operation used by this
...
/*
 * Add project workspace workspace root catalogue only after its inputs and available
...
UmiStatus umi_project_workspace_workspace_root_catalogue_add(UmiProjectWorkspaceWorkspaceRootCatalogue *value, const char *id)
{
    ...
    if (umi_project_workspace_workspace_root_catalogue_contains(value, id)) return UMI_STATUS_ALREADY_EXISTS;
...
/*
 * Find project workspace workspace root catalogue while leaving the underlying catalogue
