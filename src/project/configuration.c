/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/project/configuration.c
 *
 * PURPOSE:
...
struct UmiProjectConfigurationRegistry {
    UmiProjectConfigurationSnapshot items[UMI_PROJECT_CONFIGURATION_CAPACITY];
    size_t count;
    uint64_t revision;
};

static size_t find_index(const UmiProjectConfigurationRegistry *registry, const char *id)
{
    size_t i;
...
{
    UmiProjectConfigurationRegistry *registry;
    if (out_registry == NULL) return UMI_STATUS_INVALID_ARGUMENT;
    *out_registry = NULL;
    registry = (UmiProjectConfigurationRegistry *)calloc(1U, sizeof(*registry));
...
void umi_project_configuration_registry_destroy(UmiProjectConfigurationRegistry *registry) { free(registry); }
...
UmiStatus umi_project_configuration_registry_upsert(UmiProjectConfigurationRegistry *registry, const UmiProjectConfigurationSnapshot *item)
{
    ...
    if (index == SIZE_MAX) {
        if (registry->count >= UMI_PROJECT_CONFIGURATION_CAPACITY) return UMI_STATUS_CAPACITY_EXCEEDED;
        index = registry->count++;
    }
    registry->items[index] = *item;
    registry->items[index].struct_size = (uint32_t)sizeof(registry->items[index]);
    registry->items[index].api_version = UMI_PROJECT_CONFIGURATION_API_VERSION;
    registry->items[index].id[127U] = '\0';
    registry->items[index].project_id[127U] = '\0';
    registry->items[index].name[255U] = '\0';
    registry->items[index].build_type[63U] = '\0';
    registry->items[index].toolchain_id[127U] = '\0';
    registry->items[index].platform[127U] = '\0';
    registry->revision += 1U;
...
size_t umi_project_configuration_registry_count(const UmiProjectConfigurationRegistry *registry) { return registry != NULL ? registry->count : 0U; }
uint64_t umi_project_configuration_registry_revision(const UmiProjectConfigurationRegistry *registry) { return registry != NULL ? registry->revision : 0U; }
