...
struct UmiProjectDependencyRegistry {
    UmiProjectDependencySnapshot items[UMI_PROJECT_DEPENDENCY_CAPACITY];
    size_t count;
    uint64_t revision;
};

/* Provide the find index operation used by this module and its client applications. */
static size_t find_index(const UmiProjectDependencyRegistry *registry, const char *id)
{
    size_t i;
    /*
...
{
    UmiProjectDependencyRegistry *registry;
    /*
     * Protect caller-owned memory by checking that required state is available before it is
     * used.
     */
    ...
    *out_registry = NULL;
    registry = (UmiProjectDependencyRegistry *)calloc(1U, sizeof(*registry));
    /*
     * Protect caller-owned memory by checking that required state is available before it is
     * used.
     */
 ...
 * reused safely.
 */
void umi_project_dependency_registry_destroy(UmiProjectDependencyRegistry *registry) { free(registry); }
...
UmiStatus umi_project_dependency_registry_upsert(UmiProjectDependencyRegistry *registry, const UmiProjectDependencySnapshot *item)
{
    ...
    if (index == SIZE_MAX) {
        /* Keep the operation inside its valid bounds before reading, writing or adding data. */
        if (registry->count >= UMI_PROJECT_DEPENDENCY_CAPACITY) return UMI_STATUS_CAPACITY_EXCEEDED;
        index = registry->count++;
    }
    registry->items[index] = *item;
    registry->items[index].struct_size = (uint32_t)sizeof(registry->items[index]);
    registry->items[index].api_version = UMI_PROJECT_DEPENDENCY_API_VERSION;
    registry->items[index].id[127U] = '\0';
    registry->items[index].project_id[127U] = '\0';
    registry->items[index].name[255U] = '\0';
    registry->items[index].version[127U] = '\0';
    registry->items[index].source[511U] = '\0';
 ...
 * their state.
 */
size_t umi_project_dependency_registry_count(const UmiProjectDependencyRegistry *registry) { return registry != NULL ? registry->count : 0U; }
/*
 * Provide the project dependency registry revision operation used by this module and its
 * client applications.
 */
uint64_t umi_project_dependency_registry_revision(const UmiProjectDependencyRegistry *registry) { return registry != NULL ? registry->revision : 0U; }
/*
 * Release or reset state held by project dependency registry so the same storage can be
 * reused safely.
 */
