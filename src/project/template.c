...
struct UmiProjectTemplateRegistry {
    UmiProjectTemplateSnapshot items[UMI_PROJECT_TEMPLATE_CAPACITY];
    size_t count;
    uint64_t revision;
};

static size_t find_index(const UmiProjectTemplateRegistry *registry, const char *id)
{
    size_t i;
...
{
    UmiProjectTemplateRegistry *registry;
    if (out_registry == NULL) return UMI_STATUS_INVALID_ARGUMENT;
    *out_registry = NULL;
    registry = (UmiProjectTemplateRegistry *)calloc(1U, sizeof(*registry));
...
void umi_project_template_registry_destroy(UmiProjectTemplateRegistry *registry) { free(registry); }
...
UmiStatus umi_project_template_registry_upsert(UmiProjectTemplateRegistry *registry, const UmiProjectTemplateSnapshot *item)
{
    ...
    if (index == SIZE_MAX) {
        if (registry->count >= UMI_PROJECT_TEMPLATE_CAPACITY) return UMI_STATUS_CAPACITY_EXCEEDED;
        index = registry->count++;
    }
    registry->items[index] = *item;
    registry->items[index].struct_size = (uint32_t)sizeof(registry->items[index]);
    registry->items[index].api_version = UMI_PROJECT_TEMPLATE_API_VERSION;
    ...
    registry->items[index].language[63U] = '\0';
    registry->items[index].frontends[255U] = '\0';
    registry->revision += 1U;
...
size_t umi_project_template_registry_count(const UmiProjectTemplateRegistry *registry) { return registry != NULL ? registry->count : 0U; }
uint64_t umi_project_template_registry_revision(const UmiProjectTemplateRegistry *registry) { return registry != NULL ? registry->revision : 0U; }
