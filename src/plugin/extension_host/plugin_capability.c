/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/plugin/extension_host/plugin_capability.c
 *
 * PURPOSE:
...
/*
 * Copy plugin extension host plugin capability into module-owned storage so callers keep
 * ownership of their input values.
 */
static void umi_plugin_extension_host_plugin_capability_copy(char *destination, size_t capacity, const char *source)
{
    size_t i = 0U;
    /*
...
UmiStatus umi_plugin_extension_host_plugin_capability_configure(UmiPluginExtensionHostPluginCapability *value, const char *id, const char *subject, uint32_t version, uint32_t risk, uint64_t flags)
{
    ...
    umi_plugin_extension_host_plugin_capability_init(value);
    umi_plugin_extension_host_plugin_capability_copy(value->id, sizeof(value->id), id);
    umi_plugin_extension_host_plugin_capability_copy(value->subject, sizeof(value->subject), subject);
...
uint64_t umi_plugin_extension_host_plugin_capability_fingerprint(const UmiPluginExtensionHostPluginCapability *value)
{
    uint64_t result;
    /* Preserve the original failure result so the caller can respond to the correct cause. */
    if (umi_plugin_extension_host_plugin_capability_validate(value) != UMI_STATUS_OK) return 0U;
