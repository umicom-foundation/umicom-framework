...
/*
 * Copy plugin extension host recovery plan into module-owned storage so callers keep
 ...
 */
void umi_plugin_extension_host_recovery_plan_init(UmiPluginExtensionHostRecoveryPlan *plan) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if(plan!=NULL) memset(plan,0,sizeof(*plan)); }
/*
