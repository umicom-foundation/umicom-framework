/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/plugin/extension_host/contribution_catalogue.c
 *
 * PURPOSE:
...
/*
 * Copy plugin extension host contribution catalogue into module-owned storage so callers
 * keep ownership of their input values.
 */
static void umi_plugin_extension_host_contribution_catalogue_copy(char *destination, size_t capacity, const char *source)
{
    size_t i = 0U;
    /*
 ...
 */
void umi_plugin_extension_host_contribution_catalogue_init(UmiPluginExtensionHostContributionCatalogue *catalogue) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if (catalogue != NULL) memset(catalogue, 0, sizeof(*catalogue)); }
/*
...
UmiStatus umi_plugin_extension_host_contribution_catalogue_add(UmiPluginExtensionHostContributionCatalogue *catalogue, const char *id, uint32_t state, uint32_t priority, uint64_t evidence)
{
    ...
    i=catalogue->count++; umi_plugin_extension_host_contribution_catalogue_copy(catalogue->items[i].id,sizeof(catalogue->items[i].id),id); catalogue->items[i].state=state; catalogue->items[i].priority=priority; catalogue->items[i].evidence=evidence; ++catalogue->revision; return UMI_STATUS_OK;
}
/*
 ...
 */
size_t umi_plugin_extension_host_contribution_catalogue_count(const UmiPluginExtensionHostContributionCatalogue *catalogue) { return catalogue==NULL?0U:catalogue->count; }
