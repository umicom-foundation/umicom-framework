...
void umi_plugin_extension_host_package_activation_plan_init(UmiPluginExtensionHostPackageActivationPlan *plan) { if(plan!=NULL) memset(plan,0,sizeof(*plan)); }
...
{ size_t i; if(plan==NULL||!umi_plugin_extension_host_text_valid(id,UMI_PLUGIN_EXTENSION_HOST_ID_CAPACITY)||risk>100U) return UMI_STATUS_INVALID_ARGUMENT; if(plan->count>=UMI_PLUGIN_EXTENSION_HOST_SMALL_CAPACITY) return UMI_STATUS_CAPACITY_EXCEEDED; for(i=0U;i<plan->count;++i) if(strcmp(plan->steps[i].id,id)==0) return UMI_STATUS_ALREADY_EXISTS; i=plan->count++; umi_plugin_extension_host_package_activation_plan_copy(plan->steps[i].id,sizeof(plan->steps[i].id),id); plan->steps[i].risk=risk; plan->steps[i].required=required?1:0; ++plan->revision; return UMI_STATUS_OK; }
...
int umi_plugin_extension_host_package_activation_plan_ready(const UmiPluginExtensionHostPackageActivationPlan *plan) { size_t i; if(plan==NULL||plan->count==0U) return 0; for(i=0U;i<plan->count;++i) if(plan->steps[i].required && !plan->steps[i].completed) return 0; return 1; }
uint32_t umi_plugin_extension_host_package_activation_plan_maximum_risk(const UmiPluginExtensionHostPackageActivationPlan *plan) { size_t i; uint32_t r=0U; if(plan==NULL) return 0U; for(i=0U;i<plan->count;++i) if(plan->steps[i].risk>r) r=plan->steps[i].risk; return r; }
