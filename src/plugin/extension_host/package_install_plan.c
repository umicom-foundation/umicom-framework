...
/*
 * Copy plugin extension host package install plan into module-owned storage so callers
 ...
 */
void umi_plugin_extension_host_package_install_plan_init(UmiPluginExtensionHostPackageInstallPlan *plan) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if(plan!=NULL) memset(plan,0,sizeof(*plan)); }
/*
