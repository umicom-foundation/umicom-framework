...
/*
 * Copy plugin extension host package update plan into module-owned storage so callers keep
 ...
 */
void umi_plugin_extension_host_package_update_plan_init(UmiPluginExtensionHostPackageUpdatePlan *plan) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if(plan!=NULL) memset(plan,0,sizeof(*plan)); }
/*
