/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/language/intelligence/declaration_query.c
 *
 * PURPOSE:
 *   Implement represent and score declaration-navigation candidates.
 *
...
void umi_language_intelligence_declaration_query_init(UmiLanguageIntelligenceDeclarationQuery *query, const char *text)
{
    ...
    query->api_version = UMI_LANGUAGE_INTELLIGENCE_DECLARATION_QUERY_API_VERSION;
...
int umi_language_intelligence_declaration_query_matches(
    ...
    const char *candidate)
{
    return umi_language_intelligence_declaration_query_candidate_score(query, candidate) != 0U;
}
