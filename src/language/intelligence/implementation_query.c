/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/language/intelligence/implementation_query.c
 *
 * PURPOSE:
 *   Implement represent and score implementation-navigation candidates.
 *
...
void umi_language_intelligence_implementation_query_init(UmiLanguageIntelligenceImplementationQuery *query, const char *text)
{
    ...
    query->api_version = UMI_LANGUAGE_INTELLIGENCE_IMPLEMENTATION_QUERY_API_VERSION;
...
int umi_language_intelligence_implementation_query_matches(
    ...
    const char *candidate)
{
    return umi_language_intelligence_implementation_query_candidate_score(query, candidate) != 0U;
}
