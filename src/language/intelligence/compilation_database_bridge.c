/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/language/intelligence/compilation_database_bridge.c
 *
 * PURPOSE:
 *   Implement associate projects with compile_commands-style compilation databases.
 *
...
void umi_language_intelligence_compilation_database_bridge_init(UmiLanguageIntelligenceCompilationDatabaseBridge *mapping)
{
    ...
    mapping->api_version = UMI_LANGUAGE_INTELLIGENCE_COMPILATION_DATABASE_BRIDGE_API_VERSION;
    mapping->enabled = 1;
    mapping->revision = 1U;
}
...
{
    /* Preserve the original failure result so the caller can respond to the correct cause. */
    if (umi_language_intelligence_compilation_database_bridge_validate(mapping) != UMI_STATUS_OK ||
