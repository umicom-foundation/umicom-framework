/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/language/formatting.c
 *
 * PURPOSE:
...
struct UmiLanguageFormattingRegistry {
    UmiLanguageFormattingSnapshot items[UMI_LANGUAGE_FORMATTING_CAPACITY];
    size_t count;
    uint64_t revision;
};

static size_t find_index(const UmiLanguageFormattingRegistry *registry, const char *id)
{
    size_t i;
...
{
    UmiLanguageFormattingRegistry *registry;
    if (out_registry == NULL) return UMI_STATUS_INVALID_ARGUMENT;
    *out_registry = NULL;
    registry = (UmiLanguageFormattingRegistry *)calloc(1U, sizeof(*registry));
...
void umi_language_formatting_registry_destroy(UmiLanguageFormattingRegistry *registry) { free(registry); }
...
UmiStatus umi_language_formatting_registry_upsert(UmiLanguageFormattingRegistry *registry, const UmiLanguageFormattingSnapshot *item)
{
    ...
    if (index == SIZE_MAX) {
        if (registry->count >= UMI_LANGUAGE_FORMATTING_CAPACITY) return UMI_STATUS_CAPACITY_EXCEEDED;
        index = registry->count++;
    }
    registry->items[index] = *item;
    registry->items[index].struct_size = (uint32_t)sizeof(registry->items[index]);
    registry->items[index].api_version = UMI_LANGUAGE_FORMATTING_API_VERSION;
    registry->items[index].id[127U] = '\0';
    registry->items[index].document_id[127U] = '\0';
    registry->items[index].provider_id[127U] = '\0';
...
size_t umi_language_formatting_registry_count(const UmiLanguageFormattingRegistry *registry) { return registry != NULL ? registry->count : 0U; }
uint64_t umi_language_formatting_registry_revision(const UmiLanguageFormattingRegistry *registry) { return registry != NULL ? registry->revision : 0U; }
