...
struct UmiLanguageSymbolRegistry {
    UmiLanguageSymbolSnapshot items[UMI_LANGUAGE_SYMBOL_CAPACITY];
    size_t count;
    uint64_t revision;
};

static size_t find_index(const UmiLanguageSymbolRegistry *registry, const char *id)
{
    size_t i;
...
UmiStatus umi_language_symbol_registry_create(UmiLanguageSymbolRegistry **out_registry)
{
    ...
    *out_registry = NULL;
    registry = (UmiLanguageSymbolRegistry *)calloc(1U, sizeof(*registry));
...
void umi_language_symbol_registry_destroy(UmiLanguageSymbolRegistry *registry) { free(registry); }
...
UmiStatus umi_language_symbol_registry_upsert(UmiLanguageSymbolRegistry *registry, const UmiLanguageSymbolSnapshot *item)
{
    ...
    if (index == SIZE_MAX) {
        if (registry->count >= UMI_LANGUAGE_SYMBOL_CAPACITY) return UMI_STATUS_CAPACITY_EXCEEDED;
        index = registry->count++;
    }
    registry->items[index] = *item;
    registry->items[index].struct_size = (uint32_t)sizeof(registry->items[index]);
    registry->items[index].api_version = UMI_LANGUAGE_SYMBOL_API_VERSION;
    registry->items[index].id[127U] = '\0';
    registry->items[index].document_id[127U] = '\0';
    registry->items[index].name[255U] = '\0';
    registry->items[index].kind[63U] = '\0';
    registry->items[index].container[255U] = '\0';
    registry->revision += 1U;
...
size_t umi_language_symbol_registry_count(const UmiLanguageSymbolRegistry *registry) { return registry != NULL ? registry->count : 0U; }
uint64_t umi_language_symbol_registry_revision(const UmiLanguageSymbolRegistry *registry) { return registry != NULL ? registry->revision : 0U; }
