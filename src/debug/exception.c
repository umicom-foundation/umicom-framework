...
struct UmiDebugExceptionRegistry {
    UmiDebugExceptionSnapshot items[UMI_DEBUG_EXCEPTION_CAPACITY];
    size_t count;
    uint64_t revision;
};

static size_t find_index(const UmiDebugExceptionRegistry *registry, const char *id)
{
    size_t i;
...
{
    UmiDebugExceptionRegistry *registry;
    if (out_registry == NULL) return UMI_STATUS_INVALID_ARGUMENT;
    *out_registry = NULL;
    registry = (UmiDebugExceptionRegistry *)calloc(1U, sizeof(*registry));
...
void umi_debug_exception_registry_destroy(UmiDebugExceptionRegistry *registry) { free(registry); }
...
UmiStatus umi_debug_exception_registry_upsert(UmiDebugExceptionRegistry *registry, const UmiDebugExceptionSnapshot *item)
{
    ...
    if (index == SIZE_MAX) {
        if (registry->count >= UMI_DEBUG_EXCEPTION_CAPACITY) return UMI_STATUS_CAPACITY_EXCEEDED;
        index = registry->count++;
    }
    registry->items[index] = *item;
    registry->items[index].struct_size = (uint32_t)sizeof(registry->items[index]);
    registry->items[index].api_version = UMI_DEBUG_EXCEPTION_API_VERSION;
    registry->items[index].id[127U] = '\0';
    registry->items[index].session_id[127U] = '\0';
    registry->items[index].type[255U] = '\0';
    registry->items[index].message[1023U] = '\0';
    registry->items[index].break_mode[63U] = '\0';
    registry->revision += 1U;
...
size_t umi_debug_exception_registry_count(const UmiDebugExceptionRegistry *registry) { return registry != NULL ? registry->count : 0U; }
uint64_t umi_debug_exception_registry_revision(const UmiDebugExceptionRegistry *registry) { return registry != NULL ? registry->revision : 0U; }
