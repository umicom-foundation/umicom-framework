/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/debug/workbench/breakpoint_collection.c
 *
 * PURPOSE:
...
UmiStatus umi_debug_workbench_breakpoint_collection_add(UmiDebugWorkbenchBreakpointCollection *model, const UmiDebugWorkbenchEntry *entry)
{
    ...
    if (umi_debug_workbench_breakpoint_collection_find(model, entry->id) != NULL) return UMI_STATUS_ALREADY_EXISTS;
