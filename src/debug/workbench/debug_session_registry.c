/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/debug/workbench/debug_session_registry.c
 *
 * PURPOSE:
...
UmiStatus umi_debug_workbench_debug_session_registry_add(UmiDebugWorkbenchDebugSessionRegistry *model, const UmiDebugWorkbenchEntry *entry)
{
     ...
     * used.
     */
    if (umi_debug_workbench_debug_session_registry_find(model, entry->id) != NULL) return UMI_STATUS_ALREADY_EXISTS;
    /* Keep the operation inside its valid bounds before reading, writing or adding data. */
