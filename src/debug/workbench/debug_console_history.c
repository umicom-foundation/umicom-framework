/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/debug/workbench/debug_console_history.c
 *
 * PURPOSE:
...
UmiStatus umi_debug_workbench_debug_console_history_add(UmiDebugWorkbenchDebugConsoleHistory *model, const UmiDebugWorkbenchEntry *entry)
{
    ...
    if (umi_debug_workbench_debug_console_history_find(model, entry->id) != NULL) return UMI_STATUS_ALREADY_EXISTS;
