/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/debug/workbench/debug_configuration_group.c
 *
 * PURPOSE:
...
UmiStatus umi_debug_workbench_debug_configuration_group_add(UmiDebugWorkbenchDebugConfigurationGroup *model, const UmiDebugWorkbenchEntry *entry)
{
     ...
     * used.
     */
    if (umi_debug_workbench_debug_configuration_group_find(model, entry->id) != NULL) return UMI_STATUS_ALREADY_EXISTS;
    /* Keep the operation inside its valid bounds before reading, writing or adding data. */
