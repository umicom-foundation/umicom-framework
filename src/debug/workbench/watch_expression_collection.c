/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/debug/workbench/watch_expression_collection.c
 *
 * PURPOSE:
...
UmiStatus umi_debug_workbench_watch_expression_collection_add(UmiDebugWorkbenchWatchExpressionCollection *model, const UmiDebugWorkbenchEntry *entry)
{
    ...
    if (umi_debug_workbench_watch_expression_collection_find(model, entry->id) != NULL) return UMI_STATUS_ALREADY_EXISTS;
