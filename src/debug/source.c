...
struct UmiDebugSourceRegistry {
    UmiDebugSourceSnapshot items[UMI_DEBUG_SOURCE_CAPACITY];
    size_t count;
    uint64_t revision;
};

static size_t find_index(const UmiDebugSourceRegistry *registry, const char *id)
{
    size_t i;
...
{
    UmiDebugSourceRegistry *registry;
    if (out_registry == NULL) return UMI_STATUS_INVALID_ARGUMENT;
    *out_registry = NULL;
    registry = (UmiDebugSourceRegistry *)calloc(1U, sizeof(*registry));
...
void umi_debug_source_registry_destroy(UmiDebugSourceRegistry *registry) { free(registry); }
...
UmiStatus umi_debug_source_registry_upsert(UmiDebugSourceRegistry *registry, const UmiDebugSourceSnapshot *item)
{
    ...
    if (index == SIZE_MAX) {
        if (registry->count >= UMI_DEBUG_SOURCE_CAPACITY) return UMI_STATUS_CAPACITY_EXCEEDED;
        index = registry->count++;
    }
    registry->items[index] = *item;
    registry->items[index].struct_size = (uint32_t)sizeof(registry->items[index]);
    registry->items[index].api_version = UMI_DEBUG_SOURCE_API_VERSION;
...
size_t umi_debug_source_registry_count(const UmiDebugSourceRegistry *registry) { return registry != NULL ? registry->count : 0U; }
uint64_t umi_debug_source_registry_revision(const UmiDebugSourceRegistry *registry) { return registry != NULL ? registry->revision : 0U; }
