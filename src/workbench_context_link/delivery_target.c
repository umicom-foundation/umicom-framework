/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/delivery_target.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the context delivery target.
 *
                                           ...
                                           const char *identity)
{
    ...
    if (identity != NULL) {
        (void)umi_workbench_context_link_copy_text(
            record->target_id, sizeof(record->target_id), identity);
    }
}
    ...
    const UmiWorkbenchContextLinkDeliveryTarget *record)
{
    ...
    if (!umi_workbench_context_link_text_is_valid(
            record->target_id, sizeof(record->target_id)) ||
    ...
    const UmiWorkbenchContextLinkDeliveryTarget *source)
{
    ...
    if (umi_workbench_context_link_delivery_target_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
    ...
    const UmiWorkbenchContextLinkDeliveryTarget *record)
{
    ...
    hash = umi_workbench_context_link_hash_text(
        hash, record->target_id, sizeof(record->target_id));
    hash = umi_workbench_context_link_hash_text(
