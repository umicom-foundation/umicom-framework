/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/conflict.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the context-link conflict record.
 *
                                           ...
                                           const char *identity)
{
    ...
    if (identity != NULL) {
        (void)umi_workbench_context_link_copy_text(
            record->conflict_id, sizeof(record->conflict_id), identity);
    }
}
    ...
    const UmiWorkbenchContextLinkConflict *record)
{
    ...
    if (!umi_workbench_context_link_text_is_valid(
            record->conflict_id, sizeof(record->conflict_id)) ||
        record->conflict_id[0] == '\0') {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    /* Apply this operation only while the related capability or state is available. */
    if (!umi_workbench_context_link_text_is_valid(
    ...
    const UmiWorkbenchContextLinkConflict *source)
{
    ...
    if (umi_workbench_context_link_conflict_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
    ...
    const UmiWorkbenchContextLinkConflict *record)
{
    ...
    hash = umi_workbench_context_link_hash_text(
        hash, record->conflict_id, sizeof(record->conflict_id));
    hash = umi_workbench_context_link_hash_text(
