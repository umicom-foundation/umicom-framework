/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/navigation_entry.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the context navigation stack entry.
 *
    ...
    const UmiWorkbenchContextLinkNavigationEntry *source)
{
    ...
    if (umi_workbench_context_link_navigation_entry_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
