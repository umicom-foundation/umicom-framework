/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/export_record.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the context-link export record.
 *
    ...
    const UmiWorkbenchContextLinkExportRecord *record)
{
    ...
    /* Apply this operation only while the related capability or state is available. */
    if (!umi_workbench_context_link_text_is_valid(
            record->destination_uri, sizeof(record->destination_uri)) ||
    ...
    const UmiWorkbenchContextLinkExportRecord *source)
{
    ...
    if (umi_workbench_context_link_export_record_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
    ...
    const UmiWorkbenchContextLinkExportRecord *record)
{
    ...
    hash = umi_workbench_context_link_hash_text(
        hash, record->destination_uri, sizeof(record->destination_uri));
    hash = umi_workbench_context_link_hash_text(
    ...
    const char *value)
{
    ...
    status = umi_workbench_context_link_copy_text(
        record->destination_uri, sizeof(record->destination_uri), value);
    /* Preserve the original failure result so the caller can respond to the correct cause. */
