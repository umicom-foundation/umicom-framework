/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/publish_request.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the context publication request.
 *
                                           ...
                                           const char *identity)
{
    ...
    if (identity != NULL) {
        (void)umi_workbench_context_link_copy_text(
            record->request_id, sizeof(record->request_id), identity);
    }
}
    ...
    const UmiWorkbenchContextLinkPublishRequest *record)
{
    ...
    if (!umi_workbench_context_link_text_is_valid(
            record->request_id, sizeof(record->request_id)) ||
        record->request_id[0] == '\0') {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    if (!umi_workbench_context_link_text_is_valid(
    ...
    const UmiWorkbenchContextLinkPublishRequest *source)
{
    ...
    if (umi_workbench_context_link_publish_request_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
    ...
    const UmiWorkbenchContextLinkPublishRequest *record)
{
    ...
    hash = umi_workbench_context_link_hash_text(
        hash, record->request_id, sizeof(record->request_id));
    hash = umi_workbench_context_link_hash_text(
