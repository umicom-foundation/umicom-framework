/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/route_trace.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the context route trace.
 *
                                           ...
                                           const char *identity)
{
    ...
    if (identity != NULL) {
        (void)umi_workbench_context_link_copy_text(
            record->trace_id, sizeof(record->trace_id), identity);
    }
}
    ...
    const UmiWorkbenchContextLinkRouteTrace *record)
{
    ...
    if (!umi_workbench_context_link_text_is_valid(
            record->trace_id, sizeof(record->trace_id)) ||
        record->trace_id[0] == '\0') {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    if (!umi_workbench_context_link_text_is_valid(
    ...
    const UmiWorkbenchContextLinkRouteTrace *source)
{
    ...
    if (umi_workbench_context_link_route_trace_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
    ...
    const UmiWorkbenchContextLinkRouteTrace *record)
{
    ...
    hash = umi_workbench_context_link_hash_text(
        hash, record->trace_id, sizeof(record->trace_id));
    hash = umi_workbench_context_link_hash_text(
