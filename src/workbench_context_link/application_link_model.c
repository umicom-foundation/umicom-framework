/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/application_link_model.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the application link summary model.
 *
    ...
    const UmiWorkbenchContextLinkApplicationLinkModel *source)
{
    ...
    if (umi_workbench_context_link_application_link_model_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
