/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/panel_header_model.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the linked-panel header view model.
 *
    ...
    const UmiWorkbenchContextLinkPanelHeaderModel *source)
{
    ...
    if (umi_workbench_context_link_panel_header_model_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
