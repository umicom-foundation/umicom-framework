/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/command_history.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the context-link command history record.
 *
    ...
    const UmiWorkbenchContextLinkCommandHistory *source)
{
    ...
    if (umi_workbench_context_link_command_history_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
