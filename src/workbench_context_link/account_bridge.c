/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/account_bridge.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the account context bridge.
 *
    ...
    const UmiWorkbenchContextLinkAccountBridge *source)
{
    ...
    if (umi_workbench_context_link_account_bridge_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
