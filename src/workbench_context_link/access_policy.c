/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/access_policy.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the context-link access policy.
 *
                                           ...
                                           const char *identity)
{
    ...
    if (identity != NULL) {
        (void)umi_workbench_context_link_copy_text(
            record->policy_id, sizeof(record->policy_id), identity);
    }
}
    ...
    const UmiWorkbenchContextLinkAccessPolicy *record)
{
    ...
    if (!umi_workbench_context_link_text_is_valid(
            record->policy_id, sizeof(record->policy_id)) ||
        record->policy_id[0] == '\0') {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    /* Apply this operation only while the related capability or state is available. */
    if (!umi_workbench_context_link_text_is_valid(
        ...
        !umi_workbench_context_link_text_is_valid(
            record->resource_id, sizeof(record->resource_id))) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    ...
    const UmiWorkbenchContextLinkAccessPolicy *source)
{
    ...
    if (umi_workbench_context_link_access_policy_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
    ...
    const UmiWorkbenchContextLinkAccessPolicy *record)
{
    ...
    hash = umi_workbench_context_link_hash_text(
        hash, record->policy_id, sizeof(record->policy_id));
    hash = umi_workbench_context_link_hash_text(
        hash, record->subject_id, sizeof(record->subject_id));
    hash = umi_workbench_context_link_hash_text(
        hash, record->resource_id, sizeof(record->resource_id));
    ...
    const char *value)
{
    ...
    if (record == NULL || value == NULL) return UMI_STATUS_INVALID_ARGUMENT;
    status = umi_workbench_context_link_copy_text(
        record->resource_id, sizeof(record->resource_id), value);
    /* Preserve the original failure result so the caller can respond to the correct cause. */
    if (status == UMI_STATUS_OK) ++record->revision;
    return status;
}
