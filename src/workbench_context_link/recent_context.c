/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/recent_context.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the recent context record.
 *
                                           ...
                                           const char *identity)
{
    ...
    if (identity != NULL) {
        (void)umi_workbench_context_link_copy_text(
            record->recent_id, sizeof(record->recent_id), identity);
    }
}
    ...
    const UmiWorkbenchContextLinkRecentContext *record)
{
    ...
    if (!umi_workbench_context_link_text_is_valid(
            record->recent_id, sizeof(record->recent_id)) ||
        record->recent_id[0] == '\0') {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    if (!umi_workbench_context_link_text_is_valid(
    ...
    const UmiWorkbenchContextLinkRecentContext *source)
{
    ...
    if (umi_workbench_context_link_recent_context_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
    ...
    const UmiWorkbenchContextLinkRecentContext *record)
{
    ...
    hash = umi_workbench_context_link_hash_text(
        hash, record->recent_id, sizeof(record->recent_id));
    hash = umi_workbench_context_link_hash_text(
