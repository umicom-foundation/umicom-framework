/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/workspace_bridge.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the workspace context bridge.
 *
    ...
    const UmiWorkbenchContextLinkWorkspaceBridge *source)
{
    ...
    if (umi_workbench_context_link_workspace_bridge_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
