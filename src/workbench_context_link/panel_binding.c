/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/panel_binding.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the panel-to-colour-group binding.
 *
    ...
    const UmiWorkbenchContextLinkPanelBinding *source)
{
    ...
    if (umi_workbench_context_link_panel_binding_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
