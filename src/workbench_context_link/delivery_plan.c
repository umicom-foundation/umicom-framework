/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/delivery_plan.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the context delivery plan.
 *
                                           ...
                                           const char *identity)
{
    ...
    if (identity != NULL) {
        (void)umi_workbench_context_link_copy_text(
            record->plan_id, sizeof(record->plan_id), identity);
    }
}
    ...
    const UmiWorkbenchContextLinkDeliveryPlan *record)
{
    ...
    if (!umi_workbench_context_link_text_is_valid(
            record->plan_id, sizeof(record->plan_id)) ||
        record->plan_id[0] == '\0') {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    if (!umi_workbench_context_link_text_is_valid(
    ...
    const UmiWorkbenchContextLinkDeliveryPlan *source)
{
    ...
    if (umi_workbench_context_link_delivery_plan_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
    ...
    const UmiWorkbenchContextLinkDeliveryPlan *record)
{
    ...
    hash = umi_workbench_context_link_hash_text(
        hash, record->plan_id, sizeof(record->plan_id));
    hash = umi_workbench_context_link_hash_text(
