/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/media_bridge.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the media context bridge.
 *
    ...
    const UmiWorkbenchContextLinkMediaBridge *source)
{
    ...
    if (umi_workbench_context_link_media_bridge_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
