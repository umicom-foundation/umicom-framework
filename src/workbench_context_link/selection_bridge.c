/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/selection_bridge.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the selection context bridge.
 *
    ...
    const UmiWorkbenchContextLinkSelectionBridge *source)
{
    ...
    if (umi_workbench_context_link_selection_bridge_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
