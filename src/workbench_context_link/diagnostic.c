/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/diagnostic.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the context-link diagnostic record.
 *
                                           ...
                                           const char *identity)
{
    ...
    if (identity != NULL) {
        (void)umi_workbench_context_link_copy_text(
            record->diagnostic_id, sizeof(record->diagnostic_id), identity);
    }
}
    ...
    const UmiWorkbenchContextLinkDiagnostic *record)
{
    ...
    if (!umi_workbench_context_link_text_is_valid(
            record->diagnostic_id, sizeof(record->diagnostic_id)) ||
        record->diagnostic_id[0] == '\0') {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    if (!umi_workbench_context_link_text_is_valid(
            record->source_id, sizeof(record->source_id)) ||
        !umi_workbench_context_link_text_is_valid(
            record->message, sizeof(record->message))) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    ...
    const UmiWorkbenchContextLinkDiagnostic *source)
{
    ...
    if (umi_workbench_context_link_diagnostic_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
    ...
    const UmiWorkbenchContextLinkDiagnostic *record)
{
    ...
    hash = umi_workbench_context_link_hash_text(
        hash, record->diagnostic_id, sizeof(record->diagnostic_id));
    hash = umi_workbench_context_link_hash_text(
        hash, record->source_id, sizeof(record->source_id));
    hash = umi_workbench_context_link_hash_text(
        hash, record->message, sizeof(record->message));
    ...
    const char *value)
{
    ...
    if (record == NULL || value == NULL) return UMI_STATUS_INVALID_ARGUMENT;
    status = umi_workbench_context_link_copy_text(
        record->message, sizeof(record->message), value);
    if (status == UMI_STATUS_OK) ++record->revision;
    return status;
}
