/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/multi_monitor_link_model.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the multi-monitor link summary model.
 *
    ...
    const UmiWorkbenchContextLinkMultiMonitorLinkModel *record)
{
    ...
    if (!umi_workbench_context_link_text_is_valid(
            record->monitor_id, sizeof(record->monitor_id)) ||
    ...
    const UmiWorkbenchContextLinkMultiMonitorLinkModel *source)
{
    ...
    if (umi_workbench_context_link_multi_monitor_link_model_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
    ...
    const UmiWorkbenchContextLinkMultiMonitorLinkModel *record)
{
    ...
    hash = umi_workbench_context_link_hash_text(
        hash, record->monitor_id, sizeof(record->monitor_id));
    hash = umi_workbench_context_link_hash_text(
    ...
    const char *value)
{
    ...
    status = umi_workbench_context_link_copy_text(
        record->monitor_id, sizeof(record->monitor_id), value);
