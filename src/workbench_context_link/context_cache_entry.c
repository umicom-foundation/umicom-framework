/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/context_cache_entry.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the context cache entry.
 *
    ...
    const UmiWorkbenchContextLinkContextCacheEntry *source)
{
    ...
    if (umi_workbench_context_link_context_cache_entry_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
