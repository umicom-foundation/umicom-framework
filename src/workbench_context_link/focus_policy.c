/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/focus_policy.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the focus propagation policy.
 *
    ...
    const UmiWorkbenchContextLinkFocusPolicy *source)
{
    ...
    if (umi_workbench_context_link_focus_policy_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
