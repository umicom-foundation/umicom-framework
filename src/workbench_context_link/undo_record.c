/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/undo_record.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the context-link undo record.
 *
    ...
    const UmiWorkbenchContextLinkUndoRecord *source)
{
    ...
    if (umi_workbench_context_link_undo_record_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
