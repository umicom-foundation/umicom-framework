/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/active_context.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the active group context.
 *
                                           ...
                                           const char *identity)
{
    ...
    if (identity != NULL) {
        (void)umi_workbench_context_link_copy_text(
            record->active_id, sizeof(record->active_id), identity);
    }
}
    ...
    const UmiWorkbenchContextLinkActiveContext *record)
{
    ...
    if (!umi_workbench_context_link_text_is_valid(
            record->active_id, sizeof(record->active_id)) ||
        record->active_id[0] == '\0') {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    /* Apply this operation only while the related capability or state is available. */
    if (!umi_workbench_context_link_text_is_valid(
    ...
    const UmiWorkbenchContextLinkActiveContext *source)
{
    ...
    if (umi_workbench_context_link_active_context_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
    ...
    const UmiWorkbenchContextLinkActiveContext *record)
{
    ...
    hash = umi_workbench_context_link_hash_text(
        hash, record->active_id, sizeof(record->active_id));
    hash = umi_workbench_context_link_hash_text(
