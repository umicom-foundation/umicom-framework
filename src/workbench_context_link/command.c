/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/command.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the context-link command record.
 *
                                           ...
                                           const char *identity)
{
    ...
    if (identity != NULL) {
        (void)umi_workbench_context_link_copy_text(
            record->command_id, sizeof(record->command_id), identity);
    }
}
    ...
    const UmiWorkbenchContextLinkCommand *source)
{
    ...
    if (umi_workbench_context_link_command_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
