/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/statistics.c
 *
 * PURPOSE:
                                           ...
                                           const char *identity)
{
    ...
    if (identity != NULL) {
        (void)umi_workbench_context_link_copy_text(
            record->statistics_id, sizeof(record->statistics_id), identity);
    }
}
    ...
    const UmiWorkbenchContextLinkStatistics *record)
{
    ...
    if (!umi_workbench_context_link_text_is_valid(
            record->statistics_id, sizeof(record->statistics_id)) ||
        record->statistics_id[0] == '\0') {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    if (!umi_workbench_context_link_text_is_valid(
            record->group_id, sizeof(record->group_id)) ||
        !umi_workbench_context_link_text_is_valid(
            record->scope_id, sizeof(record->scope_id))) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    ...
    const UmiWorkbenchContextLinkStatistics *source)
{
    ...
    if (umi_workbench_context_link_statistics_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
    ...
    const UmiWorkbenchContextLinkStatistics *record)
{
    ...
    hash = umi_workbench_context_link_hash_text(
        hash, record->statistics_id, sizeof(record->statistics_id));
    hash = umi_workbench_context_link_hash_text(
        hash, record->group_id, sizeof(record->group_id));
    hash = umi_workbench_context_link_hash_text(
        hash, record->scope_id, sizeof(record->scope_id));
    ...
    const char *value)
{
    ...
    if (record == NULL || value == NULL) return UMI_STATUS_INVALID_ARGUMENT;
    status = umi_workbench_context_link_copy_text(
        record->scope_id, sizeof(record->scope_id), value);
    if (status == UMI_STATUS_OK) ++record->revision;
    return status;
}
