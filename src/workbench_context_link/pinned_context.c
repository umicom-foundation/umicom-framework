/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/pinned_context.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the pinned context record.
 *
                                           ...
                                           const char *identity)
{
    ...
    if (identity != NULL) {
        (void)umi_workbench_context_link_copy_text(
            record->pin_id, sizeof(record->pin_id), identity);
    }
}
    ...
    const UmiWorkbenchContextLinkPinnedContext *record)
{
    ...
    if (!umi_workbench_context_link_text_is_valid(
            record->pin_id, sizeof(record->pin_id)) ||
        record->pin_id[0] == '\0') {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    /* Apply this operation only while the related capability or state is available. */
    ...
    const UmiWorkbenchContextLinkPinnedContext *source)
{
    ...
    if (umi_workbench_context_link_pinned_context_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
    ...
    const UmiWorkbenchContextLinkPinnedContext *record)
{
    ...
    hash = umi_workbench_context_link_hash_text(
        hash, record->pin_id, sizeof(record->pin_id));
    hash = umi_workbench_context_link_hash_text(
