/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/query.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the context-link query record.
 *
                                           ...
                                           const char *identity)
{
    ...
    if (identity != NULL) {
        (void)umi_workbench_context_link_copy_text(
            record->query_id, sizeof(record->query_id), identity);
    }
}
    ...
    const UmiWorkbenchContextLinkQuery *record)
{
    ...
    if (!umi_workbench_context_link_text_is_valid(
            record->query_id, sizeof(record->query_id)) ||
        record->query_id[0] == '\0') {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    if (!umi_workbench_context_link_text_is_valid(
    ...
    const UmiWorkbenchContextLinkQuery *source)
{
    ...
    if (umi_workbench_context_link_query_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
    ...
    const UmiWorkbenchContextLinkQuery *record)
{
    ...
    hash = umi_workbench_context_link_hash_text(
        hash, record->query_id, sizeof(record->query_id));
    hash = umi_workbench_context_link_hash_text(
