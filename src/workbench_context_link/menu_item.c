/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/menu_item.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the context-link menu item.
 *
    ...
    const UmiWorkbenchContextLinkMenuItem *source)
{
    ...
    if (umi_workbench_context_link_menu_item_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
