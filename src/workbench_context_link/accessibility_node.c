/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/accessibility_node.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the context-link accessibility node.
 *
                                           ...
                                           const char *identity)
{
    ...
    if (identity != NULL) {
        (void)umi_workbench_context_link_copy_text(
            record->node_id, sizeof(record->node_id), identity);
    }
}
    ...
    const UmiWorkbenchContextLinkAccessibilityNode *record)
{
    ...
    if (!umi_workbench_context_link_text_is_valid(
            record->node_id, sizeof(record->node_id)) ||
        record->node_id[0] == '\0') {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    if (!umi_workbench_context_link_text_is_valid(
            record->role, sizeof(record->role)) ||
    ...
    const UmiWorkbenchContextLinkAccessibilityNode *source)
{
    ...
    if (umi_workbench_context_link_accessibility_node_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
    ...
    const UmiWorkbenchContextLinkAccessibilityNode *record)
{
    ...
    hash = umi_workbench_context_link_hash_text(
        hash, record->node_id, sizeof(record->node_id));
    hash = umi_workbench_context_link_hash_text(
        hash, record->role, sizeof(record->role));
    hash = umi_workbench_context_link_hash_text(
    ...
    const char *value)
{
    ...
    status = umi_workbench_context_link_copy_text(
        record->role, sizeof(record->role), value);
