/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/event.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the context-link event record.
 *
                                           ...
                                           const char *identity)
{
    ...
    if (identity != NULL) {
        (void)umi_workbench_context_link_copy_text(
            record->event_id, sizeof(record->event_id), identity);
    }
}
    ...
    const UmiWorkbenchContextLinkEvent *record)
{
    ...
    if (!umi_workbench_context_link_text_is_valid(
            record->event_id, sizeof(record->event_id)) ||
        record->event_id[0] == '\0') {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    if (!umi_workbench_context_link_text_is_valid(
    ...
    const UmiWorkbenchContextLinkEvent *source)
{
    ...
    if (umi_workbench_context_link_event_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
    ...
    const UmiWorkbenchContextLinkEvent *record)
{
    ...
    hash = umi_workbench_context_link_hash_text(
        hash, record->event_id, sizeof(record->event_id));
    hash = umi_workbench_context_link_hash_text(
