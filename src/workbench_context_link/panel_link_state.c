/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/panel_link_state.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the panel link state record.
 *
    ...
    const UmiWorkbenchContextLinkPanelLinkState *source)
{
    ...
    if (umi_workbench_context_link_panel_link_state_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
