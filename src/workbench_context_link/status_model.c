/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/status_model.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the context-link status view model.
 *
    ...
    const UmiWorkbenchContextLinkStatusModel *source)
{
    ...
    if (umi_workbench_context_link_status_model_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
