/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/group_state.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the group runtime state record.
 *
    ...
    const UmiWorkbenchContextLinkGroupState *source)
{
    ...
    if (umi_workbench_context_link_group_state_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
