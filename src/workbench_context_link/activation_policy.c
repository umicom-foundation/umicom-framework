/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/activation_policy.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the panel activation policy.
 *
    ...
    const UmiWorkbenchContextLinkActivationPolicy *source)
{
    ...
    if (umi_workbench_context_link_activation_policy_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
