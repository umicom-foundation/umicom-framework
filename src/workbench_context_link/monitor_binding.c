/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/workbench_context_link/monitor_binding.c
 *
 * PURPOSE:
 *   Implement validation, copying, hashing and mutation for the monitor context binding.
 *
    ...
    const UmiWorkbenchContextLinkMonitorBinding *source)
{
    ...
    if (umi_workbench_context_link_monitor_binding_validate(source) != UMI_STATUS_OK) {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    *destination = *source;
    return UMI_STATUS_OK;
