...
/* Publish an error explanation, but never let a rejected operation mark an
 * existing draft as saved. The controller's other output fields are untrusted
 * on failure; only its validated message and badge are useful here. */
static void PublishFailure(
    UmiApplicationPresentationSurfaceRuntime *runtime,
    UmiApplicationPresentationSurfaceItem *item,
    UmiStatus status,
    const UmiApplicationPresentationSurfaceUpdate *controllerUpdate)
{
    UmiApplicationPresentationSurfaceUpdate failure;
    const int usable = UmiApplicationPresentationSurfaceUpdateValidate(controllerUpdate)
        == UMI_STATUS_OK;
    (void)UmiApplicationPresentationSurfaceFailure(status,
        usable && controllerUpdate->message[0] != '\0' ? controllerUpdate->message : NULL,
        &failure);
    /* Unknown adapter statuses must still produce a defined, bounded update. */
    if (status <= UMI_STATUS_OK || status > UMI_STATUS_BUSY) {
        (void)UmiApplicationPresentationSurfaceFailure(UMI_STATUS_INTERNAL_ERROR,
            "The controller returned an unknown result. Check the application log.",
            &failure);
    }
    if (usable) (void)memcpy(failure.badge, controllerUpdate->badge, sizeof(failure.badge));
    failure.dirty = item->dirty;
    (void)umi_application_presentation_surface_session_apply(
        &runtime->session, item->placement->panel->component_id, &failure);
}

/* Notify presentation for both accepted and rejected operations. A host error
 * never replaces the original controller rejection in the returned result. */
static UmiStatus NotifyAndRecord(
    UmiApplicationPresentationSurfaceRuntime *runtime,
    UmiApplicationPresentationSurfaceItem *item,
    UmiApplicationPresentationSurfaceEvent event, UmiStatus status, int notifyHost)
{
    if (notifyHost && runtime->host_bound && runtime->started) {
        UmiStatus hostStatus = runtime->host.operations->update(runtime->host.context, item);
        if (status == UMI_STATUS_OK) status = hostStatus;
    }
...
static UmiStatus dispatch_update(
    ...
    int notify_host)
{
    UmiApplicationPresentationSurfaceUpdate update = {0};
    UmiStatus status = umi_application_presentation_surface_controller_dispatch(
    ...
    if (status != UMI_STATUS_OK) PublishFailure(runtime, item, status, &update);
    return NotifyAndRecord(runtime, item, event, status, notify_host);
}
...
/* Inspect existing policy without executing an action or changing history. */
...
{
    const UmiApplicationPresentationSurfaceItem *item;
    UmiApplicationPresentationCommandAvailability result = {0};
    const char *reason = "The route is connected. The controller must still validate the request.";
    if (runtime == NULL || component_id == NULL || out_availability == NULL)
        return UMI_STATUS_INVALID_ARGUMENT;
    item = umi_application_presentation_surface_session_find_const(&runtime->session, component_id);
    if (item == NULL) return UMI_STATUS_NOT_FOUND;
    if (item->behavior == NULL || runtime->session.workspace_policy == NULL)
        return UMI_STATUS_INVALID_STATE;
    result.controller_bound = umi_application_presentation_surface_controller_find(
        &runtime->controllers, component_id) != NULL;
    result.status = UMI_STATUS_OK;
    if (!runtime->started) {
        result.status = UMI_STATUS_INVALID_STATE;
        reason = "Open the application workspace before running a panel command.";
    } else if (runtime->background &&
               !runtime->session.workspace_policy->allow_background_commands) {
        result.status = UMI_STATUS_INVALID_STATE;
        reason = "Return to this workspace before running its commands.";
    } else if (item->behavior->command_mode == UMI_APPLICATION_PRESENTATION_COMMAND_READ_ONLY) {
        result.status = UMI_STATUS_PERMISSION_DENIED;
        reason = "This panel is read-only. Use the owning service to change its data.";
    } else if (!result.controller_bound) {
        result.status = item->behavior->command_mode == UMI_APPLICATION_PRESENTATION_COMMAND_GUARDED
            ? UMI_STATUS_PERMISSION_DENIED : UMI_STATUS_NOT_IMPLEMENTED;
        reason = "No command controller is connected to this panel. No request was accepted.";
    } else if (!item->visible || item->state == UMI_APPLICATION_PRESENTATION_STATE_DORMANT) {
        result.status = UMI_STATUS_INVALID_STATE;
        reason = "Open this panel before running its commands.";
    } else if (item->state == UMI_APPLICATION_PRESENTATION_STATE_BUSY ||
               item->state == UMI_APPLICATION_PRESENTATION_STATE_LOADING) {
        result.status = UMI_STATUS_BUSY;
        reason = "Wait for this panel's current operation to finish before trying again.";
    }
    result.can_dispatch = result.status == UMI_STATUS_OK;
    (void)snprintf(result.reason, sizeof(result.reason), "%s", reason);
    *out_availability = result;
    return UMI_STATUS_OK;
}

/* One check is shared by UI enablement, tools and the actual command route. */
    ...
    const char *command_id)
{
    ...
    UmiApplicationPresentationSurfaceItem *item;
    UmiStatus status;
    if (runtime == NULL || component_id == NULL || command_id == NULL || command_id[0] == '\0')
        return UMI_STATUS_INVALID_ARGUMENT;
    status = UmiApplicationPresentationSurfaceCommandCheck(runtime, component_id, &availability);
    if (status != UMI_STATUS_OK) return status;
    item = umi_application_presentation_surface_session_find(&runtime->session, component_id);
    if (availability.can_dispatch)
        return dispatch_update(runtime, item, UMI_APPLICATION_PRESENTATION_EVENT_COMMAND, command_id, 1);
    /* Do not disturb another active operation, a hidden panel or a stopped
     * workspace just to reject a premature call. Keep their state unchanged. */
    if (runtime->started && item->visible &&
        ...
        (void)UmiApplicationPresentationSurfaceFailure(availability.status, availability.reason, &update);
        PublishFailure(runtime, item, availability.status, &update);
        return NotifyAndRecord(runtime, item, UMI_APPLICATION_PRESENTATION_EVENT_COMMAND,
            availability.status, 1);
    }
    umi_application_presentation_surface_journal_record(&runtime->journal,
        UMI_APPLICATION_PRESENTATION_EVENT_COMMAND, component_id, availability.status);
