...
UmiStatus umi_application_suite_layout_register_workbench_profiles_resolved(
    const UmiApplicationExperienceDefinition *experience, UmiUiWorkbench *workbench,
    int activate_default, UmiApplicationSuiteLayoutPaneResolver resolver, void *user_data) {
  ...
  for (index = 0U; index < experience->layout_count; ++index) {
    ...
    status = umi_application_suite_layout_profile_project_resolved(
        experience, &experience->layouts[index], (int32_t)(100U + index), resolver, user_data,
        &profile);
    if (status != UMI_STATUS_OK)
      return status;
    ...
    status = umi_ui_workspace_profile_model_install_built_in(profiles, &profile);
    if (status != UMI_STATUS_OK)
      return status;
  }
  if (activate_default) {
    char profile_id[UMI_UI_ID_CAPACITY];
    int written = snprintf(profile_id, sizeof(profile_id), "%s.%s", experience->application_id,
