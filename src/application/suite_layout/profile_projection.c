 ...
 *
 * PURPOSE:
 *   Build reusable workbench profile snapshots from the same canonical application experience metadata.
 *
    ...
    UmiUiWorkspaceProfileSnapshot *out_profile)
{
    ...
    written = snprintf(out_profile->label, sizeof(out_profile->label), "%s", layout->title);
    ...
    for (index = 0U; index < layout->panel_count; ++index) {
        ...
        else if (placement == UMI_UI_PLACEMENT_RIGHT) out_profile->auxiliary_sidebar_visible = 1;
        else if (placement == UMI_UI_PLACEMENT_BOTTOM) out_profile->bottom_panel_visible = 1;
        ...
        written = snprintf(pane->pane_id, sizeof(pane->pane_id), "%s", panel->panel_id);
