...
static UmiStatus register_component(UmiDeclComponentRegistry *r,const char *type,const char *name,const char *category,int container){UmiDeclComponentDescriptor d;UmiDeclPropertyDescriptor p;UmiStatus s=umi_decl_component_descriptor_init(&d,type,name,category,container);if(s!=UMI_STATUS_OK)return s;(void)umi_decl_property_descriptor_init(&p,"title",UMI_DECL_VALUE_STRING,"",0);(void)umi_decl_component_add_property(&d,&p);(void)umi_decl_property_descriptor_init(&p,"visible",UMI_DECL_VALUE_BOOLEAN,"true",0);(void)umi_decl_component_add_property(&d,&p);return umi_decl_component_registry_register(r,&d);}
...
{"window","Window","Containers",1},{"split","Split","Containers",1},{"stack","Stack","Containers",1},{"tabs","Tabs","Containers",1},{"pane","Pane","Containers",1},{"editor","Editor","Development",0},{"tree","Tree","Data",0},{"table","Table","Data",0},{"chart","Chart","Data",0},{"terminal","Terminal","Development",0},{"output","Output","Development",0},{"problems","Problems","Development",0},{"button","Button","Controls",0},{"label","Label","Controls",0},{"text","Text","Controls",0},{"image","Image","Media",0},{"menu","Menu","Navigation",1},{"toolbar","Toolbar","Navigation",1},{"status","Status","Navigation",1},{"browser","Browser","Web",0}};size_t i;UmiStatus s;if(r==NULL)return UMI_STATUS_INVALID_ARGUMENT;for(i=0U;i<sizeof(items)/sizeof(items[0]);++i){s=register_component(r,items[i].type,items[i].name,items[i].category,items[i].container);if(s!=UMI_STATUS_OK)return s;}return UMI_STATUS_OK;}
