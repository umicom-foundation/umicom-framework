...
{ /* Protect caller-owned memory by checking that required state is available before it is used. */ if (value == NULL) return UMI_STATUS_INVALID_ARGUMENT; value->required_count = number; value->revision += 1U; return UMI_STATUS_OK; }
/*
...
{ /* Protect caller-owned memory by checking that required state is available before it is used. */ if (value == NULL) return UMI_STATUS_INVALID_ARGUMENT; value->optional_count = number; value->revision += 1U; return UMI_STATUS_OK; }
/*
