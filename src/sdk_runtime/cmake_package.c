...
{ if (value == NULL) return UMI_STATUS_INVALID_ARGUMENT; value->target_count = number; value->revision += 1U; return UMI_STATUS_OK; }
UmiStatus umi_sdk_runtime_cmake_package_set_found(UmiSdkRuntimeCmakePackage *value, uint64_t number)
{ if (value == NULL) return UMI_STATUS_INVALID_ARGUMENT; value->found = number; value->revision += 1U; return UMI_STATUS_OK; }
