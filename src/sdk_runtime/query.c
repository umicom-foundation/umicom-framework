...
{ if (value == NULL) return UMI_STATUS_INVALID_ARGUMENT; value->offset = number; value->revision += 1U; return UMI_STATUS_OK; }
UmiStatus umi_sdk_runtime_query_set_limit(UmiSdkRuntimeQuery *value, uint64_t number)
{ if (value == NULL) return UMI_STATUS_INVALID_ARGUMENT; value->limit = number; value->revision += 1U; return UMI_STATUS_OK; }
