...
{ if (value == NULL) return UMI_STATUS_INVALID_ARGUMENT; value->ready_components = number; value->revision += 1U; return UMI_STATUS_OK; }
UmiStatus umi_sdk_runtime_health_set_blocked_components(UmiSdkRuntimeHealth *value, uint64_t number)
{ if (value == NULL) return UMI_STATUS_INVALID_ARGUMENT; value->blocked_components = number; value->revision += 1U; return UMI_STATUS_OK; }
