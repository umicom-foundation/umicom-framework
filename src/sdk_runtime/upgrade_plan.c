/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/sdk_runtime/upgrade_plan.c
 *
 * PURPOSE:
 *   Implement plan sdk/runtime upgrades with abi and rollback evidence.
 *
...
UmiStatus umi_sdk_runtime_upgrade_plan_set_action_count(UmiSdkRuntimeUpgradePlan *value,uint64_t n)
{
    if(value==NULL)return UMI_STATUS_INVALID_ARGUMENT;
...
UmiStatus umi_sdk_runtime_upgrade_plan_set_generation(UmiSdkRuntimeUpgradePlan *value,uint64_t n)
{
    if(value==NULL)return UMI_STATUS_INVALID_ARGUMENT;
    value->generation=n;
    value->revision+=1U;
