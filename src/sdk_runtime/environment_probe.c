/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/sdk_runtime/environment_probe.c
 *
 * PURPOSE:
...
UmiStatus umi_sdk_runtime_environment_probe_set_entry_count(UmiSdkRuntimeEnvironmentProbe *value,uint64_t n)
{
    if(value==NULL)return UMI_STATUS_INVALID_ARGUMENT;
    value->entry_count=n;
    value->revision+=1U;
...
UmiStatus umi_sdk_runtime_environment_probe_set_warning_count(UmiSdkRuntimeEnvironmentProbe *value,uint64_t n)
{
    if(value==NULL)return UMI_STATUS_INVALID_ARGUMENT;
    value->warning_count=n;
    value->revision+=1U;
    return UMI_STATUS_OK;
    }
