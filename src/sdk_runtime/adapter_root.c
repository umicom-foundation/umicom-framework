/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/sdk_runtime/adapter_root.c
 *
 * PURPOSE:
 *   Implement describe installed frontend and provider adapter roots.
 *
...
UmiStatus umi_sdk_runtime_adapter_root_set_adapter_count(UmiSdkRuntimeAdapterRoot *value,uint64_t n)
{
    if(value==NULL)return UMI_STATUS_INVALID_ARGUMENT;
    value->adapter_count=n;
    value->revision+=1U;
    return UMI_STATUS_OK;
    }
UmiStatus umi_sdk_runtime_adapter_root_set_generation(UmiSdkRuntimeAdapterRoot *value,uint64_t n)
{
    if(value==NULL)return UMI_STATUS_INVALID_ARGUMENT;
    value->generation=n;
    value->revision+=1U;
