...
{ if (value == NULL) return UMI_STATUS_INVALID_ARGUMENT; value->minimum_abi = number; value->revision += 1U; return UMI_STATUS_OK; }
UmiStatus umi_sdk_runtime_abi_requirement_set_maximum_abi(UmiSdkRuntimeAbiRequirement *value, uint64_t number)
{ if (value == NULL) return UMI_STATUS_INVALID_ARGUMENT; value->maximum_abi = number; value->revision += 1U; return UMI_STATUS_OK; }
