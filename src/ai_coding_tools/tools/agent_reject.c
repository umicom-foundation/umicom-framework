/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/ai_coding_tools/tools/agent_reject.c
 *
 * PURPOSE:
 *   Reject the pending governed coding patch. Agent state mutation requires explicit approval.
 *
...
const UmiAiCodingToolDescriptor *umi_ai_coding_tool_agent_reject_descriptor(void)
{
    static const UmiAiCodingToolDescriptor descriptor = {
        .tool_id = "agent.reject",
        .label = "Reject Pending Coding Patch",
        .description = "Reject the pending governed coding patch.",
    ...
    void *user_data)
{
    ...
    status = umi_ai_coding_runtime_platform_reject(environment->coding_runtime);
    /* Preserve the original failure result so the caller can respond to the correct cause. */
    if (status != UMI_STATUS_OK) return status;

    return umi_ai_coding_tool_json_status(
        output, output_capacity, UMI_STATUS_OK, "Reject Pending Coding Patch completed.");
}
