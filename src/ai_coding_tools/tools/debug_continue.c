/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/ai_coding_tools/tools/debug_continue.c
 *
 * PURPOSE:
 *   Continue Debuggee through the persistent DAP runtime. Debuggee control always requires
...
const UmiAiCodingToolDescriptor *umi_ai_coding_tool_debug_continue_descriptor(void)
{
    static const UmiAiCodingToolDescriptor descriptor = {
        .tool_id = "debug.continue",
        .label = "Continue Debuggee",
        .description = "Continue Debuggee through the Framework DAP runtime.",
    ...
    void *user_data)
{
    ...
    return umi_ai_coding_tool_json_status(
        output, output_capacity, UMI_STATUS_OK, "Continue Debuggee completed.");
}
