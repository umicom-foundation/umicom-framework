/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/ai_coding_tools/tools/debug_step_over.c
 *
 * PURPOSE:
 *   Step Over through the persistent DAP runtime. Debuggee control always requires
...
const UmiAiCodingToolDescriptor *umi_ai_coding_tool_debug_step_over_descriptor(void)
{
    static const UmiAiCodingToolDescriptor descriptor = {
        .tool_id = "debug.step-over",
        .label = "Step Over",
        .description = "Step Over through the Framework DAP runtime.",
    ...
    void *user_data)
{
    ...
    return umi_ai_coding_tool_json_status(
        output, output_capacity, UMI_STATUS_OK, "Step Over completed.");
}
