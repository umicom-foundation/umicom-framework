/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/ai_coding_tools/tools/source_control_pull.c
 *
 * PURPOSE:
 *   Execute source-control.pull only when network and source-control mutation policy are
...
const UmiAiCodingToolDescriptor *umi_ai_coding_tool_source_control_pull_descriptor(void)
{
    static const UmiAiCodingToolDescriptor descriptor = {
        .tool_id = "source-control.pull",
        .label = "Pull Source Control",
        .description = "Pull Source Control through the Framework source-control controller.",
    ...
    void *user_data)
{
    ...
    status = umi_developer_source_control_pull(
        environment->source_control);
    if (status != UMI_STATUS_OK) return status;

    return umi_ai_coding_tool_json_status(
        output, output_capacity, UMI_STATUS_OK, "Pull Source Control completed.");
}
