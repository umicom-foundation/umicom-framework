...
#include <string.h>
/*
 * Copy teacher lesson catalogue into module-owned storage so callers keep ownership of
 * their input values.
 */
...
/*
 * Initialise teacher lesson catalogue from caller-provided values so later operations
 * receive a known state.
 */
void umi_teacher_lesson_catalogue_init(UmiTeacherLessonCatalogue *catalogue) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if (catalogue != NULL) memset(catalogue, 0, sizeof(*catalogue));
    }
/*
 * Find teacher lesson catalogue while leaving the underlying catalogue or model owned by
 * this module.
 */
...
/*
 * Provide the teacher lesson catalogue upsert operation used by this module and its client
 * applications.
 */
...
/*
 * Return the number of records represented by teacher lesson catalogue eligible without
