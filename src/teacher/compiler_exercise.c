/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/teacher/compiler_exercise.c
 *
 * PURPOSE:
...
/*
 * Copy teacher compiler exercise into module-owned storage so callers keep ownership of
 * their input values.
 */
static void umi_teacher_compiler_exercise_copy(char *destination, size_t capacity, const char *source) {
    size_t i = 0U;
    /*
...
UmiStatus umi_teacher_compiler_exercise_configure(UmiTeacherCompilerExercise *value, const char *id, const char *title, UmiTeacherLanguage language, UmiTeacherLevel level, uint32_t weight, uint32_t required_score) {
    ...
    umi_teacher_compiler_exercise_init(value);
    umi_teacher_compiler_exercise_copy(value->id, sizeof(value->id), id);
    umi_teacher_compiler_exercise_copy(value->title, sizeof(value->title), title);
