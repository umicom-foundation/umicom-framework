...
#include <string.h>
void umi_teacher_teacher_hint_policy_init(UmiTeacherTeacherHintPolicy *policy) { if (policy!=NULL) { memset(policy,0,sizeof(*policy));
...
uint32_t umi_teacher_teacher_hint_policy_deficit(const UmiTeacherTeacherHintPolicy *policy,uint32_t mastery) { if (policy==NULL || mastery>=policy->minimum_mastery) return 0U;
