/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/teacher/exercise_catalogue.c
 *
 * PURPOSE:
...
#include <string.h>
/*
 * Copy teacher exercise catalogue into module-owned storage so callers keep ownership of
 * their input values.
 */
static void umi_teacher_exercise_catalogue_copy(char *destination, size_t capacity, const char *source) {
    size_t i = 0U;
    /*
 ...
 */
void umi_teacher_exercise_catalogue_init(UmiTeacherExerciseCatalogue *catalogue) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if (catalogue != NULL) memset(catalogue, 0, sizeof(*catalogue));
    }
/*
...
UmiStatus umi_teacher_exercise_catalogue_upsert(UmiTeacherExerciseCatalogue *catalogue, const char *id, UmiTeacherLanguage language, UmiTeacherLevel level, uint32_t priority) {
    ...
    i = catalogue->count++;
    umi_teacher_exercise_catalogue_copy(catalogue->entries[i].id, UMI_TEACHER_ID_CAPACITY, id);
