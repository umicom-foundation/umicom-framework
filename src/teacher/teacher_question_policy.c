...
#include <string.h>
void umi_teacher_teacher_question_policy_init(UmiTeacherTeacherQuestionPolicy *policy) { if (policy!=NULL) { memset(policy,0,sizeof(*policy));
...
uint32_t umi_teacher_teacher_question_policy_deficit(const UmiTeacherTeacherQuestionPolicy *policy,uint32_t mastery) { if (policy==NULL || mastery>=policy->minimum_mastery) return 0U;
