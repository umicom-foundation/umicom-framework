/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/teacher/teacher_workspace_bridge.c
 *
 * PURPOSE:
...
/*
 * Copy teacher teacher workspace bridge into module-owned storage so callers keep
 * ownership of their input values.
 */
static void umi_teacher_teacher_workspace_bridge_copy(char *destination, size_t capacity, const char *source) {
    size_t i = 0U;
    /*
...
UmiStatus umi_teacher_teacher_workspace_bridge_configure(UmiTeacherTeacherWorkspaceBridge *value, const char *id, const char *title, UmiTeacherLanguage language, UmiTeacherLevel level, uint32_t weight, uint32_t required_score) {
    ...
    umi_teacher_teacher_workspace_bridge_init(value);
    umi_teacher_teacher_workspace_bridge_copy(value->id, sizeof(value->id), id);
    umi_teacher_teacher_workspace_bridge_copy(value->title, sizeof(value->title), title);
