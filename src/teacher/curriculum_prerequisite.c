...
#include <string.h>
void umi_teacher_curriculum_prerequisite_init(UmiTeacherCurriculumPrerequisite *policy) { if (policy!=NULL) { memset(policy,0,sizeof(*policy));
...
uint32_t umi_teacher_curriculum_prerequisite_deficit(const UmiTeacherCurriculumPrerequisite *policy,uint32_t mastery) { if (policy==NULL || mastery>=policy->minimum_mastery) return 0U;
