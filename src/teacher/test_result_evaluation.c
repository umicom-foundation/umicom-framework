 ...
 */
void umi_teacher_test_result_evaluation_init(UmiTeacherTestResultEvaluation *rubric) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if(rubric!=NULL) { rubric->correctness_weight=60U;
 ...
 */
UmiStatus umi_teacher_test_result_evaluation_configure(UmiTeacherTestResultEvaluation *rubric,uint32_t correctness_weight,uint32_t quality_weight,uint32_t efficiency_weight,uint32_t minimum_score) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if(rubric==NULL || correctness_weight+quality_weight+efficiency_weight!=100U || minimum_score>100U) return UMI_STATUS_INVALID_ARGUMENT;
