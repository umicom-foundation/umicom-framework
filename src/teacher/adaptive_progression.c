 ...
 */
void umi_teacher_adaptive_progression_init(UmiTeacherAdaptiveProgression *model) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if(model!=NULL) { model->remediate_below=45U;
 ...
 * client applications.
 */
UmiStatus umi_teacher_adaptive_progression_configure(UmiTeacherAdaptiveProgression *model,uint32_t remediate_below,uint32_t repeat_below,uint32_t advance_at,uint32_t minimum_confidence) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if(model==NULL||remediate_below>repeat_below||repeat_below>advance_at||advance_at>100U||minimum_confidence>100U) return UMI_STATUS_INVALID_ARGUMENT;
 ...
 * client applications.
 */
UmiTeacherProgressDecision umi_teacher_adaptive_progression_decide(const UmiTeacherAdaptiveProgression *model,uint32_t mastery,uint32_t confidence) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if(model==NULL||confidence<model->minimum_confidence) return UMI_TEACHER_PROGRESS_HOLD;
