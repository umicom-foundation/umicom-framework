 ...
 */
void umi_teacher_teacher_explanation_policy_init(UmiTeacherTeacherExplanationPolicy *policy) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if (policy!=NULL) { memset(policy,0,sizeof(*policy));
 ...
 * its client applications.
 */
uint32_t umi_teacher_teacher_explanation_policy_deficit(const UmiTeacherTeacherExplanationPolicy *policy,uint32_t mastery) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if (policy==NULL || mastery>=policy->minimum_mastery) return 0U;
