...
void umi_teacher_lesson_navigation_init(UmiTeacherLessonNavigation *sequence) { if(sequence!=NULL) memset(sequence,0,sizeof(*sequence));
    }
...
const char *umi_teacher_lesson_navigation_current(const UmiTeacherLessonNavigation *sequence) { if(sequence==NULL||sequence->count==0U||sequence->cursor>=sequence->count) return NULL;
    return sequence->ids[sequence->cursor];
    }
UmiStatus umi_teacher_lesson_navigation_next(UmiTeacherLessonNavigation *sequence) { if(sequence==NULL) return UMI_STATUS_INVALID_ARGUMENT;
    if(sequence->cursor+1U>=sequence->count) return UMI_STATUS_NOT_FOUND;
    ++sequence->cursor;
    return UMI_STATUS_OK;
    }
UmiStatus umi_teacher_lesson_navigation_previous(UmiTeacherLessonNavigation *sequence) { if(sequence==NULL) return UMI_STATUS_INVALID_ARGUMENT;
