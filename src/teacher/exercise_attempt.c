...
#include <string.h>
/*
 * Copy teacher exercise attempt into module-owned storage so callers keep ownership of
 * their input values.
 */
 ...
 */
void umi_teacher_exercise_attempt_init(UmiTeacherExerciseAttempt *attempt) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if(attempt!=NULL) memset(attempt,0,sizeof(*attempt));
    }
/*
 * Provide the teacher exercise attempt begin operation used by this module and its client
 * applications.
 */
UmiStatus umi_teacher_exercise_attempt_begin(UmiTeacherExerciseAttempt *attempt,const char *activity_id,uint32_t attempt_number) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if(attempt==NULL||activity_id==NULL||activity_id[0]=='\0'||attempt_number==0U) return UMI_STATUS_INVALID_ARGUMENT;
 ...
 * applications.
 */
UmiStatus umi_teacher_exercise_attempt_finish(UmiTeacherExerciseAttempt *attempt,uint32_t score,uint32_t hints_used,uint32_t elapsed_seconds,uint32_t pass_score) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if(attempt==NULL||attempt->activity_id[0]=='\0'||score>100U||pass_score>100U) return UMI_STATUS_INVALID_ARGUMENT;
    attempt->score=score;
