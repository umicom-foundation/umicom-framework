...
#include <string.h>
void umi_teacher_exercise_solution_policy_init(UmiTeacherExerciseSolutionPolicy *policy) { if (policy!=NULL) { memset(policy,0,sizeof(*policy));
...
uint32_t umi_teacher_exercise_solution_policy_deficit(const UmiTeacherExerciseSolutionPolicy *policy,uint32_t mastery) { if (policy==NULL || mastery>=policy->minimum_mastery) return 0U;
