...
void umi_teacher_study_session_init(UmiTeacherStudySession *session,const char *id) { if(session==NULL) return;
    memset(session,0,sizeof(*session));
    umi_teacher_study_session_copy(session->id,sizeof(session->id),id);
    session->state=UMI_TEACHER_STATE_READY;
    }
UmiStatus umi_teacher_study_session_record_activity(UmiTeacherStudySession *session,int completed,uint32_t mastery) { if(session==NULL||mastery>100U) return UMI_STATUS_INVALID_ARGUMENT;
...
void umi_teacher_study_session_record_question(UmiTeacherStudySession *session) { if(session!=NULL) { ++session->questions_asked;
    ++session->revision;
    } }
void umi_teacher_study_session_add_minutes(UmiTeacherStudySession *session,uint32_t minutes) { if(session!=NULL) { session->elapsed_minutes+=minutes;
    ++session->revision;
    } }
