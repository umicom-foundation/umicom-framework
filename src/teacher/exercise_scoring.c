/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/teacher/exercise_scoring.c
 *
 * PURPOSE:
 ...
 */
void umi_teacher_exercise_scoring_init(UmiTeacherExerciseScoring *rubric) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if(rubric!=NULL) { rubric->correctness_weight=60U;
 ...
 * client applications.
 */
UmiStatus umi_teacher_exercise_scoring_configure(UmiTeacherExerciseScoring *rubric,uint32_t correctness_weight,uint32_t quality_weight,uint32_t efficiency_weight,uint32_t minimum_score) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if(rubric==NULL || correctness_weight+quality_weight+efficiency_weight!=100U || minimum_score>100U) return UMI_STATUS_INVALID_ARGUMENT;
 ...
 * client applications.
 */
uint32_t umi_teacher_exercise_scoring_compute(const UmiTeacherExerciseScoring *rubric,uint32_t correctness,uint32_t quality,uint32_t efficiency) { uint64_t total;
    /*
 ...
 * applications.
 */
int umi_teacher_exercise_scoring_passes(const UmiTeacherExerciseScoring *rubric,uint32_t score) { return rubric!=NULL && score>=rubric->minimum_score;
    }
