...
/* Provide the docc operation used by this module and its client applications. */
...
/* Provide the def operation used by this module and its client applications. */
static UmiStatus def(void*u,const char*a,char*out,size_t cap){UmiLanguageRuntimeWorkbenchBridge*b=u;UmiStatus q;(void)a;q=umi_language_runtime_platform_request_definition(b->p,b->c.document_id,b->c.symbol_id,b->c.line,b->c.column);/* Preserve the original failure result so the caller can respond to the correct cause. */ if(out&&cap)snprintf(out,cap,"%s",q==UMI_STATUS_OK?"Definition request sent.":"Definition request failed.");return q;}
/* Provide the refs operation used by this module and its client applications. */
static UmiStatus refs(void*u,const char*a,char*out,size_t cap){UmiLanguageRuntimeWorkbenchBridge*b=u;UmiStatus q;(void)a;q=umi_language_runtime_platform_request_references(b->p,b->c.document_id,b->c.symbol_id,b->c.line,b->c.column,1);/* Preserve the original failure result so the caller can respond to the correct cause. */ if(out&&cap)snprintf(out,cap,"%s",q==UMI_STATUS_OK?"References request sent.":"References request failed.");return q;}
/* Provide the syms operation used by this module and its client applications. */
static UmiStatus syms(void*u,const char*a,char*out,size_t cap){UmiLanguageRuntimeWorkbenchBridge*b=u;UmiStatus q;q=umi_language_runtime_platform_request_workspace_symbols(b->p,b->c.language_id,b->c.root_uri,a?a:"");/* Preserve the original failure result so the caller can respond to the correct cause. */ if(out&&cap)snprintf(out,cap,"%s",q==UMI_STATUS_OK?"Workspace symbol request sent.":"Workspace symbol request failed.");return q;}
/*
 * Initialise language runtime workbench bridge from caller-provided values so later
 * operations receive a known state.
 */
UmiStatus umi_language_runtime_workbench_bridge_create(UmiDeveloperWorkbench*w,UmiLanguageRuntimePlatform*p,UmiLanguageRuntimeWorkbenchBridge**out){UmiLanguageRuntimeWorkbenchBridge*b;/* Preserve the original failure result so the caller can respond to the correct cause. */ if(!w||!p||!out)return UMI_STATUS_INVALID_ARGUMENT;*out=NULL;b=calloc(1,sizeof(*b));/* Preserve the original failure result so the caller can respond to the correct cause. */ if(!b)return UMI_STATUS_OUT_OF_MEMORY;b->w=w;b->p=p;*out=b;return UMI_STATUS_OK;}void umi_language_runtime_workbench_bridge_destroy(UmiLanguageRuntimeWorkbenchBridge*b){free(b);}UmiStatus umi_language_runtime_workbench_bridge_set_context(UmiLanguageRuntimeWorkbenchBridge*b,const UmiLanguageRuntimeWorkbenchContext*c){/* Preserve the original failure result so the caller can respond to the correct cause. */ if(!b||!c)return UMI_STATUS_INVALID_ARGUMENT;b->c=*c;return UMI_STATUS_OK;}
/*
 * Provide the language runtime workbench bridge bind operation used by this module and its
 * client applications.
 */
UmiStatus umi_language_runtime_workbench_bridge_bind(UmiLanguageRuntimeWorkbenchBridge*b){UmiStatus q;/* Preserve the original failure result so the caller can respond to the correct cause. */ if(!b)return UMI_STATUS_INVALID_ARGUMENT;q=umi_developer_workbench_bind_action(b->w,"navigate.definition",def,docc,b);/* Preserve the original failure result so the caller can respond to the correct cause. */ if(q!=UMI_STATUS_OK)return q;q=umi_developer_workbench_bind_action(b->w,"navigate.references",refs,docc,b);/* Preserve the original failure result so the caller can respond to the correct cause. */ if(q!=UMI_STATUS_OK)return q;return umi_developer_workbench_bind_action(b->w,"navigate.symbol",syms,workc,b);}
