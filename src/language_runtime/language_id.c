...
UmiStatus umi_language_runtime_normalize_language_id(const char*s,char*out,size_t cap){size_t i;const char*r=s;int n;if(s==NULL||!*s||out==NULL||!cap)return UMI_STATUS_INVALID_ARGUMENT;for(i=0;i<sizeof(a)/sizeof(a[0]);i++)if(strcmp(a[i].s,s)==0){r=a[i].t;break;}n=snprintf(out,cap,"%s",r);return n>=0&&(size_t)n<cap?UMI_STATUS_OK:UMI_STATUS_CAPACITY_EXCEEDED;}
