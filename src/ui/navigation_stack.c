/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/ui/navigation_stack.c
 *
 * PURPOSE:
...
struct UmiUiNavigationEntryRegistry {
    UmiUiNavigationEntrySnapshot items[UMI_UI_NAVIGATION_STACK_CAPACITY];
    size_t count;
    uint64_t revision;
};

static size_t find_index(const UmiUiNavigationEntryRegistry *registry, const char *id)
{
    size_t i;
...
{
    UmiUiNavigationEntryRegistry *registry;
    if (out_registry == NULL) return UMI_STATUS_INVALID_ARGUMENT;
    *out_registry = NULL;
    registry = (UmiUiNavigationEntryRegistry *)calloc(1U, sizeof(*registry));
...
void umi_ui_navigation_stack_registry_destroy(UmiUiNavigationEntryRegistry *registry) { free(registry); }
...
UmiStatus umi_ui_navigation_stack_registry_upsert(UmiUiNavigationEntryRegistry *registry, const UmiUiNavigationEntrySnapshot *item)
{
    ...
    if (index == SIZE_MAX) {
        if (registry->count >= UMI_UI_NAVIGATION_STACK_CAPACITY) return UMI_STATUS_CAPACITY_EXCEEDED;
        index = registry->count++;
    }
    registry->items[index] = *item;
    registry->items[index].struct_size = (uint32_t)sizeof(registry->items[index]);
    registry->items[index].api_version = UMI_UI_NAVIGATION_STACK_API_VERSION;
...
size_t umi_ui_navigation_stack_registry_count(const UmiUiNavigationEntryRegistry *registry) { return registry != NULL ? registry->count : 0U; }
uint64_t umi_ui_navigation_stack_registry_revision(const UmiUiNavigationEntryRegistry *registry) { return registry != NULL ? registry->revision : 0U; }
