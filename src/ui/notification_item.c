/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/ui/notification_item.c
 *
 * PURPOSE:
...
struct UmiUiNotificationItemRegistry {
    UmiUiNotificationItemSnapshot items[UMI_UI_NOTIFICATION_ITEM_CAPACITY];
    size_t count;
    uint64_t revision;
};

static size_t find_index(const UmiUiNotificationItemRegistry *registry, const char *id)
{
    size_t i;
...
{
    UmiUiNotificationItemRegistry *registry;
    if (out_registry == NULL) return UMI_STATUS_INVALID_ARGUMENT;
    *out_registry = NULL;
    registry = (UmiUiNotificationItemRegistry *)calloc(1U, sizeof(*registry));
...
void umi_ui_notification_item_registry_destroy(UmiUiNotificationItemRegistry *registry) { free(registry); }
...
UmiStatus umi_ui_notification_item_registry_upsert(UmiUiNotificationItemRegistry *registry, const UmiUiNotificationItemSnapshot *item)
{
    ...
    if (index == SIZE_MAX) {
        if (registry->count >= UMI_UI_NOTIFICATION_ITEM_CAPACITY) return UMI_STATUS_CAPACITY_EXCEEDED;
        index = registry->count++;
    }
    registry->items[index] = *item;
    registry->items[index].struct_size = (uint32_t)sizeof(registry->items[index]);
    registry->items[index].api_version = UMI_UI_NOTIFICATION_ITEM_API_VERSION;
    registry->items[index].id[127U] = '\0';
    registry->items[index].title[255U] = '\0';
    registry->items[index].message[1023U] = '\0';
    registry->items[index].source[127U] = '\0';
    registry->items[index].action_id[127U] = '\0';
    registry->revision += 1U;
...
size_t umi_ui_notification_item_registry_count(const UmiUiNotificationItemRegistry *registry) { return registry != NULL ? registry->count : 0U; }
uint64_t umi_ui_notification_item_registry_revision(const UmiUiNotificationItemRegistry *registry) { return registry != NULL ? registry->revision : 0U; }
