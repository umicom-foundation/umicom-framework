...
{
    size_t index;
    if (profile == NULL || !umi_ui_id_is_valid(profile->profile_id) ||
    ...
    const UmiUiWorkspaceProfileSnapshot *profile)
{
    ...
    for (pane_index = 0U;
         pane_index < model->items[index].pane_count;
         ++pane_index) {
    ...
    const char *description)
{
    ...
    if (model == NULL || profile_id == NULL || label == NULL ||
        description == NULL || label[0] == '\0') {
        return UMI_STATUS_INVALID_ARGUMENT;
    }
    label_length = strlen(label);
    description_length = strlen(description);
    if (label_length >= UMI_UI_TEXT_CAPACITY ||
        description_length >= UMI_UI_DESCRIPTION_CAPACITY) {
        return UMI_STATUS_CAPACITY_EXCEEDED;
    }
    ...
    (void)memcpy(model->items[index].description,
                 description,
