/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/ui/dock_model.c
 *
 * PURPOSE:
 *   Implement persistent dock areas and dock groups inspired by mature multi-pane workbenches.
 *
...
struct UmiUiDockRegistry {
    UmiUiDockSnapshot items[UMI_UI_DOCK_MODEL_CAPACITY];
    size_t count;
    uint64_t revision;
};

static size_t find_index(const UmiUiDockRegistry *registry, const char *id)
{
    size_t i;
...
{
    UmiUiDockRegistry *registry;
    if (out_registry == NULL) return UMI_STATUS_INVALID_ARGUMENT;
    *out_registry = NULL;
    registry = (UmiUiDockRegistry *)calloc(1U, sizeof(*registry));
...
UmiStatus umi_ui_dock_model_registry_upsert(UmiUiDockRegistry *registry, const UmiUiDockSnapshot *item)
{
    ...
    if (index == SIZE_MAX) {
        if (registry->count >= UMI_UI_DOCK_MODEL_CAPACITY) return UMI_STATUS_CAPACITY_EXCEEDED;
        index = registry->count++;
    }
    registry->items[index] = *item;
    registry->items[index].struct_size = (uint32_t)sizeof(UmiUiDockSnapshot);
