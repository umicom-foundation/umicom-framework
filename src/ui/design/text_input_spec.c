...
UmiStatus umi_design_text_input_spec_init(UmiDesignTextInputSpec *spec, const char *placeholder, uint32_t max_length, int password, int search, int multiline)
{
    ...
    spec->max_length=max_length;spec->password=password?1:0;spec->search=search?1:0;spec->multiline=multiline?1:0;if(placeholder!=NULL){if(umi_design_copy_text(spec->placeholder,sizeof spec->placeholder,placeholder)!=UMI_STATUS_OK)return UMI_STATUS_CAPACITY_EXCEEDED;}
