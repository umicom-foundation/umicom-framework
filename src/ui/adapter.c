...
struct UmiUiAdapterRegistry { UmiUiAdapterV1 adapters[UMI_UI_ADAPTER_MAX]; size_t count; UmiMutex *mutex; };
...
const UmiUiAdapterV1 *umi_ui_adapter_registry_find(const UmiUiAdapterRegistry *r,const char *id){size_t i;if(r==NULL||id==NULL)return NULL;i=find_adapter(r,id);return i!=SIZE_MAX?&r->adapters[i]:NULL;}
