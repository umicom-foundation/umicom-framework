 ...
 * relies on it.
 */
int umi_analytics_chart_annotation_valid(const UmiAnalyticsChartAnnotation *item){/* Protect caller-owned memory by checking that required state is available before it is used. */ if(item==NULL)return 0;return (item->id[0]!='\0'&&umi_analytics_number_valid(item->x)&&umi_analytics_number_valid(item->y))?1:0;}
