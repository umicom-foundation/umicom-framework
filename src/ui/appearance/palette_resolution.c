...
UmiStatus umi_appearance_palette_resolution_override(UmiAppearancePaletteResolution *item,const char *token_id,UmiAppearanceScope scope){if(item==NULL||!umi_appearance_id_valid(token_id))return UMI_STATUS_INVALID_ARGUMENT;if(umi_appearance_copy_text(item->resolved_token_id,sizeof item->resolved_token_id,token_id)!=UMI_STATUS_OK)return UMI_STATUS_CAPACITY_EXCEEDED;item->winning_scope=scope;return UMI_STATUS_OK;}
