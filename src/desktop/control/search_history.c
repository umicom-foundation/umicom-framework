/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/desktop/control/search_history.c
 * PURPOSE: Implement capture bounded global search history identity and policy state.
...
void umi_desktop_search_history_init(UmiDesktopSearchHistorySnapshot *value, const char *id)
{
    ...
    value->api_version = UMI_DESKTOP_SEARCH_HISTORY_API_VERSION;
    value->enabled = true;
    value->revision = 1U;
