...
int umi_rad_layout_constraint_is_valid(const UmiRadLayoutConstraint *item){if(item==NULL)return 0;return item->min_width >= 0 && item->min_height >= 0 && item->max_width >= item->min_width && item->max_height >= item->min_height;}
