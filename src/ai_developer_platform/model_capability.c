/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/ai_developer_platform/model_capability.c
 *
 * PURPOSE:
...
UmiStatus umi_ai_dev_model_capability_configure(UmiAiDevModelCapability *policy, uint64_t allowed_flags, uint64_t denied_flags, UmiAiDevRisk maximum_risk, UmiAiDevLocality locality, int require_approval) {
    ...
    umi_ai_dev_model_capability_init(policy);
...
int umi_ai_dev_model_capability_approval_required(const UmiAiDevModelCapability *policy, UmiAiDevRisk risk) { return policy != NULL && policy->require_approval && risk >= UMI_AI_DEV_RISK_MEDIUM; }
