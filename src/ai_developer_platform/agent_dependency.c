/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/ai_developer_platform/agent_dependency.c
 *
 * PURPOSE:
...
UmiStatus umi_ai_dev_agent_dependency_configure(UmiAiDevAgentDependency *policy, uint64_t allowed_flags, uint64_t denied_flags, UmiAiDevRisk maximum_risk, UmiAiDevLocality locality, int require_approval) {
    ...
    umi_ai_dev_agent_dependency_init(policy);
...
int umi_ai_dev_agent_dependency_approval_required(const UmiAiDevAgentDependency *policy, UmiAiDevRisk risk) { return policy != NULL && policy->require_approval && risk >= UMI_AI_DEV_RISK_MEDIUM; }
