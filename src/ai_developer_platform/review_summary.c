/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/ai_developer_platform/review_summary.c
 *
 * PURPOSE:
 ...
 * client applications.
 */
void umi_ai_dev_review_summary_set_pending(UmiAiDevReviewSummary *value, uint32_t pending) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if (value != NULL) { value->pending = pending; ++value->revision; } }
/*
 ...
 * applications.
 */
int umi_ai_dev_review_summary_ready(const UmiAiDevReviewSummary *value) { return value != NULL && value->state == UMI_AI_DEV_STATE_READY && value->pending == 0U; }
