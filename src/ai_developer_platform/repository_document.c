/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/ai_developer_platform/repository_document.c
 *
 * PURPOSE:
...
/*
 * Copy ai dev repository document into module-owned storage so callers keep ownership of
 * their input values.
 */
static void umi_ai_dev_repository_document_copy(char *dst, size_t cap, const char *src) {
    size_t i = 0U;
    /*
...
UmiStatus umi_ai_dev_repository_document_configure(UmiAiDevRepositoryDocument *value, const char *id, const char *label, uint32_t priority, uint64_t flags) {
    ...
    umi_ai_dev_repository_document_init(value);
    umi_ai_dev_repository_document_copy(value->id, sizeof(value->id), id);
    umi_ai_dev_repository_document_copy(value->label, sizeof(value->label), label);
