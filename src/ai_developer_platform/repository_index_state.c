/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/ai_developer_platform/repository_index_state.c
 *
 * PURPOSE:
 ...
 */
void umi_ai_dev_repository_index_state_set_pending(UmiAiDevRepositoryIndexState *value, uint32_t pending) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if (value != NULL) { value->pending = pending; ++value->revision; } }
/*
 ...
 * client applications.
 */
int umi_ai_dev_repository_index_state_ready(const UmiAiDevRepositoryIndexState *value) { return value != NULL && value->state == UMI_AI_DEV_STATE_READY && value->pending == 0U; }
