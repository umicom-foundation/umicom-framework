/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/ai_developer_platform/memory_scope.c
 *
 * PURPOSE:
...
UmiStatus umi_ai_dev_memory_scope_configure(UmiAiDevMemoryScope *policy, uint64_t allowed_flags, uint64_t denied_flags, UmiAiDevRisk maximum_risk, UmiAiDevLocality locality, int require_approval) {
    ...
    umi_ai_dev_memory_scope_init(policy);
 ...
 * client applications.
 */
int umi_ai_dev_memory_scope_approval_required(const UmiAiDevMemoryScope *policy, UmiAiDevRisk risk) { return policy != NULL && policy->require_approval && risk >= UMI_AI_DEV_RISK_MEDIUM; }
