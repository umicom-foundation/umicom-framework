/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/ai_developer_platform/context_filter.c
 *
 * PURPOSE:
...
UmiStatus umi_ai_dev_context_filter_configure(UmiAiDevContextFilter *policy, uint64_t allowed_flags, uint64_t denied_flags, UmiAiDevRisk maximum_risk, UmiAiDevLocality locality, int require_approval) {
    ...
    umi_ai_dev_context_filter_init(policy);
...
int umi_ai_dev_context_filter_approval_required(const UmiAiDevContextFilter *policy, UmiAiDevRisk risk) { return policy != NULL && policy->require_approval && risk >= UMI_AI_DEV_RISK_MEDIUM; }
