...
void umi_ai_dev_retrieval_result_init(UmiAiDevRetrievalResult *collection) { if (collection!=NULL) memset(collection,0,sizeof(*collection)); }
...
UmiStatus umi_ai_dev_retrieval_result_add(UmiAiDevRetrievalResult *collection, const char *id) { if(collection==NULL||id==NULL||id[0]=='\0') return UMI_STATUS_INVALID_ARGUMENT; if(umi_ai_dev_retrieval_result_contains(collection,id)) return UMI_STATUS_ALREADY_EXISTS; if(collection->count>=UMI_AI_DEV_MEDIUM_CAPACITY) return UMI_STATUS_CAPACITY_EXCEEDED; umi_ai_dev_retrieval_result_copy(collection->ids[collection->count],UMI_AI_DEV_ID_CAPACITY,id); ++collection->count; ++collection->revision; return UMI_STATUS_OK; }
