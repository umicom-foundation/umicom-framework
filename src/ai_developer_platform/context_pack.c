...
#include <string.h>
/*
 * Copy ai dev context pack into module-owned storage so callers keep ownership of their
 * input values.
 */
 ...
 * known state.
 */
void umi_ai_dev_context_pack_init(UmiAiDevContextPack *collection) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if (collection!=NULL) memset(collection,0,sizeof(*collection)); }
/*
...
UmiStatus umi_ai_dev_context_pack_add(UmiAiDevContextPack *collection, const char *id) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if(collection==NULL||id==NULL||id[0]=='\0') return UMI_STATUS_INVALID_ARGUMENT; /* Protect caller-owned memory by checking that required state is available before it is used. */ if(umi_ai_dev_context_pack_contains(collection,id)) return UMI_STATUS_ALREADY_EXISTS; /* Protect caller-owned memory by checking that required state is available before it is used. */ if(collection->count>=UMI_AI_DEV_MEDIUM_CAPACITY) return UMI_STATUS_CAPACITY_EXCEEDED; umi_ai_dev_context_pack_copy(collection->ids[collection->count],UMI_AI_DEV_ID_CAPACITY,id); ++collection->count; ++collection->revision; return UMI_STATUS_OK; }
/*
