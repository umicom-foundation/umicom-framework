/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/ai_developer_platform/authorengine_bridge.c
 *
 * PURPOSE:
 ...
 * MIT
 *---------------------------------------------------------------------------*/
#include "umicom/ai/developer_platform/authorengine_bridge.h"

#include <string.h>

/*
 * Copy ai dev authorengine bridge into module-owned storage so callers keep ownership of
 * their input values.
 */
static void umi_ai_dev_authorengine_bridge_copy(char *dst, size_t cap, const char *src) {
    size_t i = 0U;
    /*
...
UmiStatus umi_ai_dev_authorengine_bridge_configure(UmiAiDevAuthorengineBridge *value, const char *id, const char *label, uint32_t priority, uint64_t flags) {
    ...
    umi_ai_dev_authorengine_bridge_init(value);
    umi_ai_dev_authorengine_bridge_copy(value->id, sizeof(value->id), id);
    umi_ai_dev_authorengine_bridge_copy(value->label, sizeof(value->label), label);
