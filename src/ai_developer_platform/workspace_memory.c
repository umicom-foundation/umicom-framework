/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/ai_developer_platform/workspace_memory.c
 *
 * PURPOSE:
...
#include <string.h>
/*
 * Copy ai dev workspace memory into module-owned storage so callers keep ownership of
 * their input values.
 */
static void umi_ai_dev_workspace_memory_copy(char *dst, size_t cap, const char *src) {
    size_t i = 0U;
    /*
 ...
 */
void umi_ai_dev_workspace_memory_init(UmiAiDevWorkspaceMemory *collection) { /* Protect caller-owned memory by checking that required state is available before it is used. */ if (collection!=NULL) memset(collection,0,sizeof(*collection)); }
/*
