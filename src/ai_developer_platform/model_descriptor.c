/*-----------------------------------------------------------------------------
 * Umicom Framework
 * File: src/ai_developer_platform/model_descriptor.c
 *
 * PURPOSE:
...
/*
 * Copy ai dev model descriptor into module-owned storage so callers keep ownership of
 * their input values.
 */
static void umi_ai_dev_model_descriptor_copy(char *dst, size_t cap, const char *src) {
    size_t i = 0U;
    /*
...
UmiStatus umi_ai_dev_model_descriptor_configure(UmiAiDevModelDescriptor *value, const char *id, const char *label, uint32_t priority, uint64_t flags) {
    ...
    umi_ai_dev_model_descriptor_init(value);
    umi_ai_dev_model_descriptor_copy(value->id, sizeof(value->id), id);
    umi_ai_dev_model_descriptor_copy(value->label, sizeof(value->label), label);
