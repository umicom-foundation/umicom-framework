...
/*
 * Provide the native web workspace strip render operation used by this module and its
 * client applications.
 */
UmiStatus umi_native_web_workspace_strip_render(const UmiNativeWebRenderContext *context,const UmiNativeWebSurface *surface,UmiNativeWebRenderResult *result){/* Preserve the original failure result so the caller can respond to the correct cause. */ if(umi_native_web_render_context_validate(context)!=UMI_STATUS_OK)return UMI_STATUS_INVALID_ARGUMENT;return umi_native_web_render_surface_wrapper(surface,"umicom-workspace-strip","navigation",result);}
/*
 * Provide the native web workspace strip renderer descriptor operation used by this module
