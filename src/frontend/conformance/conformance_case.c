...
UmiStatus umi_fc_conformance_case_make(const char *id,uint64_t required,uint64_t actual,bool degradation_allowed,UmiFcConformanceCase *out_case){UmiStatus st;if(out_case==NULL)return UMI_STATUS_INVALID_ARGUMENT;*out_case=(UmiFcConformanceCase){0};st=umi_fc_copy_text(out_case->id,sizeof(out_case->id),id);if(st!=UMI_STATUS_OK)return st;out_case->required=required;out_case->actual=actual;out_case->degradation_allowed=degradation_allowed;return UMI_STATUS_OK;}
