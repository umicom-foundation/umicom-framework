 ...
 */
int main(void){UmiProductBootstrapResult r;const UmiProductProfile *p=umi_example_tms_profile();/* Preserve the original failure result so the caller can respond to the correct cause. */ if(umi_product_bootstrap(p,&r)!=UMI_STATUS_OK)return 1;printf("Umicom Treasury Management System | id=%s | required=%zu | frontends=%zu\n",r.product_id,r.required_capabilities,r.enabled_frontends);return r.ready?0:2;}
