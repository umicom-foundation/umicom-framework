...
static const char *umi_repo_option_value(int argc,
                                         char **argv,
                                         const char *option)
{
    int index;
...
static int umi_repo_has_flag(int argc, char **argv, const char *option)
{
    int index;
...
static unsigned umi_repo_frontends(const char *text, int argc, char **argv)
{
    char copy[256];
    char *token;
    ...
    if (frontends != 0U) return frontends;
    ...
    while (token != NULL) {
        if (strcmp(token, "console") == 0) frontends |= UMI_FRONTEND_CONSOLE;
        else if (strcmp(token, "gtk") == 0 || strcmp(token, "gtk4") == 0)
            frontends |= UMI_FRONTEND_GTK4;
        else if (strcmp(token, "web") == 0) frontends |= UMI_FRONTEND_WEB;
        else if (strcmp(token, "qt") == 0 || strcmp(token, "qt6") == 0)
            frontends |= UMI_FRONTEND_QT;
        else if (strcmp(token, "wt") == 0) frontends |= UMI_FRONTEND_WT;
...
static UmiFrameworkLinkMode umi_repo_framework_link(int argc, char **argv)
{
...
int umi_cli_command_repository(UmiCliContext *context, int argc, char **argv)
{
    ...
    option_count = argc - 1;
    options = argv + 1;
    parent = umi_repo_option_value(option_count, options, "--parent");
    frontends = umi_repo_option_value(option_count, options, "--frontends");
    selected_frontends = umi_repo_frontends(frontends,
                                            option_count,
                                            options);
    if (parent == NULL) parent = ".";

    if (umi_cli_context_prepare(context,
                                NULL,
                                (selected_frontends & UMI_FRONTEND_GTK4) != 0U,
                                umi_repo_has_flag(option_count,
                                                  options,
                                                  "--remote")) !=
        UMI_STATUS_OK) {
        return 1;
    ...
    request.repository_name = umi_repo_option_value(option_count,
                                                       options,
                                                       "--repo-name");
    if (request.repository_name == NULL) request.repository_name = name;
    request.application_id = umi_repo_option_value(option_count,
                                                   options,
                                                   "--id");
    request.remote_owner = umi_repo_option_value(option_count,
                                                 options,
                                                 "--owner");
    request.remote_description = umi_repo_option_value(option_count,
                                                       options,
                                                       "--description");
    request.framework_url = umi_repo_option_value(option_count,
                                                  options,
                                                  "--framework-url");
    request.framework_branch = umi_repo_option_value(option_count,
                                                     options,
                                                     "--framework-branch");
    if (request.framework_branch == NULL) request.framework_branch = "main";
    request.frontends = selected_frontends;
    request.framework_link = umi_repo_framework_link(option_count, options);
    request.visibility = umi_repo_has_flag(option_count, options, "--public")
        ? UMI_REPOSITORY_PUBLIC
        : UMI_REPOSITORY_PRIVATE;
    request.initialise_git = !umi_repo_has_flag(option_count,
                                                options,
                                                "--no-git");
    request.create_initial_commit = request.initialise_git &&
        !umi_repo_has_flag(option_count, options, "--no-commit");
    request.create_remote = umi_repo_has_flag(option_count,
                                              options,
                                              "--remote");
    request.push_initial = umi_repo_has_flag(option_count,
                                             options,
                                             "--push");
    request.dry_run = umi_repo_has_flag(option_count, options, "--dry-run");
    ...
    status = umi_repository_scaffold_create(&context->discovery.profile,
                                            &context->environment,
                                            &request,
                                            &report);
    ...
    (void)printf("Directories created: %zu\n", report.directories_created);
    if (request.dry_run) (void)puts("Dry run completed; no files changed.");
    return 0;
}
